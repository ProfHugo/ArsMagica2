...
public class LightningBolt extends Particle {
	...
	public LightningBolt(World world, double x1, double y1, double z1, double x, double y, double z, long seed,
	...
	private static AMVector3 getRelativeViewVector(AMVector3 pos) {
		...
		return new AMVector3((float) renderentity.posX - pos.x, (float) renderentity.posY - pos.y,
				(float) renderentity.posZ - pos.z);
	}

	private void renderBolt(VertexBuffer buffer, float partialframe, float cosyaw, float cospitch, float sinyaw,
		...
		int renderlength = (int) ((this.main.particleAge + partialframe + (int) (this.main.length * 3.0F))
				/ (int) (this.main.length * 3.0F) * this.main.numsegments0);
		for (Iterator<Segment> iterator = this.main.segments.iterator(); iterator.hasNext();) {
			...
			if (rendersegment.segmentno <= renderlength) {
				float width = 0.03F * (getRelativeViewVector(rendersegment.startpoint.point).length() / 10.0F + 1.0F)
				...
				// rendersegment.light);
				buffer.pos(rx2 - diff2.x, ry2 - diff2.y, rz2 - diff2.z).tex(0.5D, 0.0D)
						.color(this.particleRed, this.particleGreen, this.particleBlue, mainalpha * rendersegment.light)
						.endVertex();
				buffer.pos(rx1 - diff1.x, ry1 - diff1.y, rz1 - diff1.z).tex(0.5D, 0.0D)
						.color(this.particleRed, this.particleGreen, this.particleBlue, mainalpha * rendersegment.light)
						.endVertex();
				buffer.pos(rx1 + diff1.x, ry1 + diff1.y, rz1 + diff1.z).tex(0.5D, 1.0D)
						.color(this.particleRed, this.particleGreen, this.particleBlue, mainalpha * rendersegment.light)
						.endVertex();
				buffer.pos(rx2 + diff2.x, ry2 + diff2.y, rz2 + diff2.z).tex(0.5D, 1.0D)
						.color(this.particleRed, this.particleGreen, this.particleBlue, mainalpha * rendersegment.light)
						.endVertex();
				if (rendersegment.next == null) {
					AMVector3 roundend = rendersegment.endpoint.point.copy()
							.add(rendersegment.diff.copy().normalize().scale(width));
					float rx3 = (float) (roundend.x - interpPosX);
					float ry3 = (float) (roundend.y - interpPosY);
					float rz3 = (float) (roundend.z - interpPosZ);
					buffer.pos(rx3 - diff2.x, ry3 - diff2.y, rz3 - diff2.z).tex(0.0D, 0.0D).color(this.particleRed,
							this.particleGreen, this.particleBlue, mainalpha * rendersegment.light).endVertex();
					buffer.pos(rx2 - diff2.x, ry2 - diff2.y, rz2 - diff2.z).tex(0.5D, 0.0D).color(this.particleRed,
							this.particleGreen, this.particleBlue, mainalpha * rendersegment.light).endVertex();
					buffer.pos(rx2 + diff2.x, ry2 + diff2.y, rz2 + diff2.z).tex(0.5D, 1.0D).color(this.particleRed,
							this.particleGreen, this.particleBlue, mainalpha * rendersegment.light).endVertex();
					buffer.pos(rx3 + diff2.x, ry3 + diff2.y, rz3 + diff2.z).tex(0.0D, 1.0D).color(this.particleRed,
							this.particleGreen, this.particleBlue, mainalpha * rendersegment.light).endVertex();
				}
				if (rendersegment.prev == null) {
					AMVector3 roundend = rendersegment.startpoint.point.copy()
							.sub(rendersegment.diff.copy().normalize().scale(width));
					float rx3 = (float) (roundend.x - interpPosX);
					float ry3 = (float) (roundend.y - interpPosY);
					float rz3 = (float) (roundend.z - interpPosZ);
					buffer.pos(rx1 - diff1.x, ry1 - diff1.y, rz1 - diff1.z).tex(0.5D, 0.0D).color(this.particleRed,
							this.particleGreen, this.particleBlue, mainalpha * rendersegment.light).endVertex();
					buffer.pos(rx3 - diff1.x, ry3 - diff1.y, rz3 - diff1.z).tex(0.0D, 0.0D).color(this.particleRed,
							this.particleGreen, this.particleBlue, mainalpha * rendersegment.light).endVertex();
					buffer.pos(rx3 + diff1.x, ry3 + diff1.y, rz3 + diff1.z).tex(0.0D, 1.0D).color(this.particleRed,
							this.particleGreen, this.particleBlue, mainalpha * rendersegment.light).endVertex();
					buffer.pos(rx1 + diff1.x, ry1 + diff1.y, rz1 + diff1.z).tex(0.5D, 1.0D).color(this.particleRed,
							this.particleGreen, this.particleBlue, mainalpha * rendersegment.light).endVertex();
				}
			}
		}
	...
	@Override
	public void renderParticle(VertexBuffer worldRendererIn, Entity entityIn, float partialTicks, float cosyaw,
		...
		Minecraft.getMinecraft().renderEngine
				.bindTexture(new ResourceLocation("arsmagica2", "textures/items/particles/smoke.png"));
