...
public class BindingsDefs {
	...
	public static final KeyBinding SHAPE_GROUP = new KeyBinding("key.am2.shape_groups", Keyboard.KEY_C,
			"keybindings.am2");
	public static final KeyBinding AURA_CUSTOMIZATION = new KeyBinding("key.am2.aura_customization", Keyboard.KEY_B,
			"keybindings.am2");
	public static final KeyBinding NIGHT_VISION = new KeyBinding("key.am2.dark_vision", Keyboard.KEY_L,
			"keybindings.am2");
	public static final KeyBinding SPELL_BOOK_PREV = new KeyBinding("key.am2.spellbookprev", Keyboard.KEY_Z,
			"keybindings.am2");
	public static final KeyBinding SPELL_BOOK_NEXT = new KeyBinding("key.am2.spellbooknext", Keyboard.KEY_X,
			"keybindings.am2");
	...
	public void onKeyInput(KeyInputEvent event) {
		...
		if (NIGHT_VISION.isPressed())
			...
			if (curItem != null && curItem.getItem() instanceof ItemSpellBook) {
				AMNetHandler.INSTANCE.sendPacketToServer(AMPacketIDs.SPELLBOOK_CHANGE_ACTIVE_SLOT,
						new AMDataWriter().add(ItemSpellBook.ID_NEXT_SPELL).add(player.getEntityId())
								.add(player.inventory.currentItem).generate());
			}
			...
			if (curItem != null && curItem.getItem() instanceof ItemSpellBook) {
				AMNetHandler.INSTANCE.sendPacketToServer(AMPacketIDs.SPELLBOOK_CHANGE_ACTIVE_SLOT,
						new AMDataWriter().add(ItemSpellBook.ID_PREV_SPELL).add(player.getEntityId())
								.add(player.inventory.currentItem).generate());
			}
		}
		...
		// return;
		// }
		// if (curItem.getItem() == ItemDefs.spellBook || curItem.getItem() ==
		// ItemDefs.arcaneSpellbook){
		// //send packet to server
		// AMNetHandler.INSTANCE.sendSpellbookSlotChange(player,
		// player.inventory.currentItem, ItemSpellBook.ID_NEXT_SPELL);
		// }
		...
		// //send packet to server
		// AMNetHandler.INSTANCE.sendSpellbookSlotChange(player,
		// player.inventory.currentItem, ItemSpellBook.ID_PREV_SPELL);
		// }
		// }
