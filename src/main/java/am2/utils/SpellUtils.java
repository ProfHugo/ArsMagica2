...
public class SpellUtils {
	...
	public static SpellShape getShapeForStage(ItemStack oldIs, int stage) {
		if (oldIs == null || !oldIs.hasTagCompound())
		...
		return SpellRegistry.getShapeFromName(shapeName) != null ? SpellRegistry.getShapeFromName(shapeName)
				: SpellDefs.MISSING_SHAPE;
	}
	...
	public static boolean attackTargetSpecial(ItemStack spellStack, Entity target, DamageSource damagesource,
			float magnitude) {
		...
		if (damagesource.getEntity() != null) {
			if (damagesource.getEntity() instanceof EntityLivingBase) {
				...
				if ((source instanceof EntityLightMage || source instanceof EntityDarkMage)
					...
					return false;
				} else if (source instanceof EntityPlayer && target instanceof EntityPlayer && !target.worldObj.isRemote
		...
		if (target instanceof EntityDragon) {
			success = ((EntityDragon) target).attackEntityFromPart(((EntityDragon) target).dragonPartBody, damagesource,
					magnitude);
		} else {
		...
		if (dmgSrcPlayer != null) {
			if (spellStack != null && target instanceof EntityLivingBase) {
				if (!target.worldObj.isRemote && ((EntityLivingBase) target).getHealth() <= 0
					...
					double chance = SpellUtils.getModifiedDouble_Add(0, spellStack, dmgSrcPlayer, target,
							dmgSrcPlayer.worldObj, SpellModifiers.DISMEMBERING_LEVEL);
	...
	public static KeyValuePair<ArrayList<AbstractSpellPart>, NBTTagCompound> decode(NBTTagCompound toDecode) {
		...
		try {
			...
			return new KeyValuePair<ArrayList<AbstractSpellPart>, NBTTagCompound>(parts,
					toDecode.getCompoundTag(SPELL_DATA));
		} catch (Exception e) {
			return null;
		}
	}

	public static ItemStack createSpellStack(
			ArrayList<KeyValuePair<ArrayList<AbstractSpellPart>, NBTTagCompound>> shapeGroups,
			ArrayList<AbstractSpellPart> spellDef, NBTTagCompound encodedData) {
	...
	public static ItemStack merge(ItemStack spellIn) {
		...
		NBTTagCompound group = (NBTTagCompound) NBTUtils
				.addCompoundList(NBTUtils.getAM2Tag(newStack.getTagCompound()), "ShapeGroups")
				.getCompoundTagAt(NBTUtils.getAM2Tag(newStack.getTagCompound()).getInteger("CurrentShapeGroup")).copy();
		int stageNum = numStages(newStack);
		for (int i = 0; i < stageNum; i++) {
			NBTTagList list = (NBTTagList) NBTUtils
					.addCompoundList(NBTUtils.getAM2Tag(newStack.getTagCompound()), STAGE + i).copy();
			if (i == 0 && !group.getBoolean("LastWasShape")) {
				NBTTagList newList = (NBTTagList) NBTUtils
						.addCompoundList(group, "Stage_" + group.getInteger("StageNum")).copy();
		...
		group.getCompoundTag(SPELL_DATA)
				.merge(NBTUtils.getAM2Tag(newStack.getTagCompound()).getCompoundTag(SPELL_DATA));
		newStack.setTagCompound(NBTUtils.addTag(new NBTTagCompound(), group, "AM2"));
		return newStack;
	}

	public static ItemStack popStackStage(ItemStack is) {
		NBTUtils.getAM2Tag(is.getTagCompound()).setInteger("CurrentGroup",
				NBTUtils.getAM2Tag(is.getTagCompound()).getInteger("CurrentGroup") + 1);
		return is;
	}
	...
	public static float getManaCost(ItemStack stack, Entity caster) {
		...
		try {
			...
			for (int j = 0; j < NBTUtils.getAM2Tag(mergedStack.getTagCompound()).getInteger("StageNum"); j++) {
				NBTTagList stageTag = NBTUtils.addCompoundList(NBTUtils.getAM2Tag(mergedStack.getTagCompound()),
						STAGE + j);
				for (int i = 0; i < stageTag.tagCount(); i++) {
					NBTTagCompound tmp = stageTag.getCompoundTagAt(i);
					String type = tmp.getString(TYPE);
							...
							for (Affinity aff : affinities) {
								for (Affinity aff2 : component.getAffinity()) {
									if (aff == aff2 && pAffinity.getAffinityDepth(aff) > 0) {
										cost = cost - (float) (cost * (0.5f
												* AffinityData.For((EntityLivingBase) caster).getAffinityDepth(aff)));
										break;
									} else {
	...
	public static SpellCastResult applyStackStage(ItemStack stack, EntityLivingBase caster, EntityLivingBase target,
			double x, double y, double z, @Nullable EnumFacing side, World world, boolean consumeMBR, boolean giveXP,
			int ticksUsed) {
		...
		manaCost *= 1F + (float) ((float) EntityExtension.For(caster).getCurrentBurnout()
				/ (float) EntityExtension.For(caster).getMaxBurnout());
		SpellCastEvent.Pre pre = new SpellCastEvent.Pre(caster, stack, manaCost);
		MinecraftForge.EVENT_BUS.post(pre);
		manaCost = pre.manaCost;

		if (consumeMBR) {
			if ((!ext.hasEnoughtMana(manaCost) && (caster instanceof EntityPlayer))
		...
		if (group == 0)
			result = shape.beginStackStage((ItemSpellBase) stack.getItem(), stack2, caster, target, world, x, y, z,
					side, giveXP, ticksUsed);
		else {
			NBTUtils.getAM2Tag(stack.getTagCompound()).setInteger("CurrentGroup", group + 1);
			result = shape.beginStackStage((ItemSpellBase) stack.getItem(), stack, caster, target, world, x, y, z, side,
					giveXP, ticksUsed);
		}
		...
		if (caster instanceof EntityPlayer) {
			if (consumeMBR && !((EntityPlayer) caster).capabilities.isCreativeMode
					&& (result == SpellCastResult.SUCCESS || result == SpellCastResult.SUCCESS_REDUCE_MANA
							|| result == SpellCastResult.MALFORMED_SPELL_STACK)) {
	...
	public static double getModifiedStat(double defaultValue, int operation, ItemStack stack, EntityLivingBase caster,
			Entity target, World world, int stage, SpellModifiers modified) {
			...
			for (int i = 0; i < stageTag.tagCount(); i++) {
				...
				if (tagType.equalsIgnoreCase(TYPE_MODIFIER)) {
					...
					if (mod.getAspectsModified().contains(modified))
						val = makeCalculation(operation, val,
								mod.getModifier(modified, caster, target, world, stack.getTagCompound()));
				}
			}
		} else {
	 ...
	 * @return
	 */
	public static double getModifiedStat(int operation, ItemStack stack, EntityLivingBase caster, Entity target,
			World world, int stage, SpellModifiers modified) {
	...
	public static ArrayList<AbstractSpellPart> getPartsForGroup(ItemStack stack, int group) {
		...
		try {
			NBTTagCompound compound = (NBTTagCompound) NBTUtils
					.addCompoundList(NBTUtils.getAM2Tag(stack.getTagCompound()), "ShapeGroups")
					.getCompoundTagAt(NBTUtils.getAM2Tag(stack.getTagCompound()).getInteger("CurrentShapeGroup"))
					.copy();
	...
	public static ArrayList<SpellComponent> getComponentsForStage(ItemStack stack, int stage) {
		try {
			...
			if (stage != -1) {
				NBTTagList stageTag = NBTUtils.addCompoundList(NBTUtils.getAM2Tag(stack.getTagCompound()),
						STAGE + stage);
				for (int i = 0; i < stageTag.tagCount(); i++) {
					NBTTagCompound tag = stageTag.getCompoundTagAt(i);
					String tagType = tag.getString(TYPE);
	...
	public static SpellCastResult applyStageToGround(ItemStack stack, EntityLivingBase caster, World world,
			BlockPos pos, EnumFacing blockFace, double impactX, double impactY, double impactZ, boolean consumeMBR) {
					...
					if (modifierIsPresent(SpellModifiers.COLOR, stack)) {
						...
						for (SpellModifier mod : mods) {
							if (mod instanceof Colour) {
								color = (int) mod.getModifier(SpellModifiers.COLOR, null, null, null,
										NBTUtils.getAM2Tag(stack.getTagCompound()));
							}
						}
					}
					component.spawnParticles(world, pos.getX(), pos.getY(), pos.getZ(), caster, caster, world.rand,
							color);
				}
			}
		}
	...
	public static SpellCastResult applyStageToEntity(ItemStack stack, EntityLivingBase caster, World world,
			Entity target, boolean shiftAffinityAndXP) {
		SpellShape stageShape = SpellUtils.getShapeForStage(stack, 0);
		if (stageShape == null)
			return SpellCastResult.MALFORMED_SPELL_STACK;

		// if ((!AMCore.config.getAllowCreativeTargets()) && target instanceof
		...
		for (SpellComponent component : components) {
			...
			if (component.applyEffectEntity(stack, world, caster, target)) {
				...
				if (world.isRemote) {
					...
					component.spawnParticles(world, target.posX, target.posY + target.getEyeHeight(), target.posZ,
							caster, target, world.rand, color);
				}
				if (caster instanceof EntityPlayer) {
					AffinityShiftUtils.doAffinityShift(caster, component, stageShape);
				}
	...
	public static int getModifiedInt_Mul(int defaultValue, ItemStack stack, EntityLivingBase caster, Entity target,
			World world, SpellModifiers modified) {
		return (int) getModifiedStat(defaultValue, 2, stack, caster, target, world, -1, modified);
	}

	public static double getModifiedDouble_Mul(double defaultValue, ItemStack stack, EntityLivingBase caster,
			Entity target, World world, SpellModifiers modified) {
		return getModifiedStat(defaultValue, 2, stack, caster, target, world, -1, modified);
	}

	public static double getModifiedDouble_Mul(ItemStack stack, EntityLivingBase caster, Entity target, World world,
			SpellModifiers modified) {
		return getModifiedStat(modified.defaultValue, 2, stack, caster, target, world, -1, modified);
	}

	public static int getModifiedInt_Add(int defaultValue, ItemStack stack, EntityLivingBase caster, Entity target,
			World world, SpellModifiers modified) {
		return (int) getModifiedDouble_Add(defaultValue, stack, caster, target, world, modified);
	}

	public static double getModifiedDouble_Add(double defaultValue, ItemStack stack, EntityLivingBase caster,
			Entity target, World world, SpellModifiers modified) {
		return getModifiedStat(defaultValue, 0, stack, caster, target, world, -1, modified);
	}

	public static double getModifiedDouble_Add(ItemStack stack, EntityLivingBase caster, Entity target, World world,
			SpellModifiers modified) {
		return getModifiedStat(modified.defaultValueInt, 0, stack, caster, target, world, -1, modified);
	}

	public static int getModifiedInt_Add(ItemStack stack, EntityLivingBase caster, EntityLivingBase target, World world,
			SpellModifiers modified) {
	...
	public static ArrayList<AbstractSpellPart> getShapeGroupParts(ItemStack stack, int shapeGroup) {
		try {
			ArrayList<AbstractSpellPart> mods = new ArrayList<AbstractSpellPart>();
			NBTTagCompound tag = NBTUtils.addCompoundList(NBTUtils.getAM2Tag(stack.getTagCompound()), "ShapeGroups")
					.getCompoundTagAt(shapeGroup);
	...
	public static ItemStack createSpellStack(
			ArrayList<KeyValuePair<ArrayList<AbstractSpellPart>, NBTTagCompound>> shapeGroupSetup,
			KeyValuePair<ArrayList<AbstractSpellPart>, NBTTagCompound> curRecipeSetup) {
	...
	public static SpellCastResult applyStackStageOnUsing(ItemStack stack, EntityLivingBase caster,
			EntityLivingBase target, double x, double y, double z, World world, boolean consumeMBR, boolean giveXP,
			int ticks) {
