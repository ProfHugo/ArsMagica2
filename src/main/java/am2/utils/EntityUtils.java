...
public class EntityUtils {
	...
	public static Entity getPointedEntity(World world, EntityLivingBase entityplayer, double range,
			double collideRadius, boolean nonCollide, boolean targetWater) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			RayTraceResult mop = world.rayTraceBlocks(
					...
					new Vec3d(entity.posX, entity.posY + entity.getEyeHeight(), entity.posZ), targetWater, !targetWater,
					false);
	...
	public static int xpBarCap(int experienceLevel) {
		return experienceLevel >= 30 ? 112 + (experienceLevel - 30) * 9
				: (experienceLevel >= 15 ? 37 + (experienceLevel - 15) * 5 : 7 + experienceLevel * 2);// experienceLevel
																										// >=
																										// 30
																										// ?
																										// 62
	...
	public static void makeSummon_PlayerFaction(EntityCreature entityliving, EntityPlayer player,
		...
		if (isAIEnabled(entityliving)
				&& EntityExtension.For(player).getCurrentSummons() < EntityExtension.For(player).getMaxSummons()) {
			if (storeForRevert)
				storedTasks.put(entityliving.getEntityId(),
						new ArrayList<EntityAITasks.EntityAITaskEntry>(entityliving.targetTasks.taskEntries));
			...
			entityliving.targetTasks.addTask(2, new EntityAINearestAttackableTarget<EntityMob>(entityliving,
					EntityMob.class, 0, true, false, SummonEntitySelector.instance));
			entityliving.targetTasks.addTask(2,
					new EntityAINearestAttackableTarget<EntitySlime>(entityliving, EntitySlime.class, true));
			entityliving.targetTasks.addTask(2,
					new EntityAINearestAttackableTarget<EntityGhast>(entityliving, EntityGhast.class, true));
			entityliving.targetTasks.addTask(2,
					new EntityAINearestAttackableTarget<EntityShulker>(entityliving, EntityShulker.class, true));
	...
	public static void makeSummon_MonsterFaction(EntityCreature entityliving, boolean storeForRevert) {
		if (isAIEnabled(entityliving)) {
			...
			entityliving.targetTasks.addTask(1, new EntityAIHurtByTarget(entityliving, true));
			entityliving.targetTasks.addTask(2,
					new EntityAINearestAttackableTarget<EntityPlayer>(entityliving, EntityPlayer.class, true));
			if (!entityliving.worldObj.isRemote && entityliving.getAttackTarget() != null
	...
	public static void setOwner(EntityLivingBase entityliving, EntityLivingBase owner) {
		...
		if (entityliving instanceof EntityCreature) {
			...
			if (speed <= 0)
				speed = 1.0f;
			((EntityCreature) entityliving).tasks.addTask(1,
					new EntityAISummonFollowOwner((EntityCreature) entityliving, speed, 10, 20));
		}
	}
	...
	public static void setSize(EntityLivingBase entityliving, float width, float height) {
		...
		if (ptrSetSize == null) {
			try {
				ptrSetSize = ReflectionHelper.findMethod(Entity.class, entityliving,
						new String[] { "func_70105_a", "setSize" }, Float.TYPE, Float.TYPE);
			} catch (Throwable t) {
				t.printStackTrace();
				return;
			}
