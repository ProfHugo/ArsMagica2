...
public class AMLineSegment {
	...
	public Vec3d closestPointOnLine(Vec3d vPoint) {
		...
		// Create a normalized direction vector from end point vA to end point
		// vB
		...
		// Using the dot product, we project the vVector1 onto the vector
		// vVector2.
		// This essentially gives us the distance from our projected vector from
		// vA.
		double t = vVector2.dotProduct(vVector1);

		// If our projected distance from vA, "t", is less than or equal to 0,
		// it must
		// be closest to the end point vA. We want to return this end point.
		if (t <= 0)
			return a;

		// If our projected distance from vA, "t", is greater than or equal to
		// the magnitude
		// or distance of the line segment, it must be closest to the end point
		// vB. So, return vB.
		if (t >= d)
			return b;
