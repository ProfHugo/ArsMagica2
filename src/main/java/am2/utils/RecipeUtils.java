...
public class RecipeUtils {
	...
	public static IRecipe getRecipeFor(ItemStack item) {
		...
		try {
			...
			for (IRecipe recipe : list) {
				...
				if (output == null)
					continue;
				if (output.getItem() == item.getItem() && (output.getItemDamage() == Short.MAX_VALUE
						|| output.getItemDamage() == item.getItemDamage())) {
	...
	public static ArrayList<ItemStack> getConvRecipe(AbstractSpellPart part) {
		...
		if (recipeItems == null) {
			LogHelper.error("Unable to write recipe to book.  Recipe items are null for part "
