...
public class InventoryUtilities {
		...
		if (inventory instanceof ISidedInventory) {
			...
			for (int i = 0; i < slots.length && stack != null && stack.stackSize > 0; ++i) {
				...
				if (InventoryUtilities.canInsertItemToInventory(sidedInventory, stack, slots[i], side)) {
					...
					if (prvStack == null) {
						// if the stack in the slot is null then get the max
						// value that can be moved and transfer the stack to the
						// inventory
						...
						// if the stack in the slot can be merged with the stack
						// we are trying to move get the max items that can
						// exist in the slot
						// and insert as many as will fit from the stack we are
						// trying to move
						int max = Math.min(stack.getMaxStackSize(), sidedInventory.getInventoryStackLimit());
	...
	public static boolean inventoryHasRoomFor(IInventory inventory, ItemStack stack, int qty) {
		for (int i = 0; i < inventory.getSizeInventory(); ++i) {
			...
			if (compareItemStacks(invStack, stack, true, false, true, true)
					&& invStack.getMaxStackSize() - invStack.stackSize >= qty)
				return true;
		}
		return false;
	}
	...
	public static int getInventorySlotIndexFor(IInventory inventory, Item item, int metadata) {
		for (int i = 0; i < inventory.getSizeInventory(); ++i) {
			...
			if (stack != null && stack.getItem() == item
					&& (stack.getItemDamage() == metadata || metadata == Short.MAX_VALUE))
				return i;
		}
		return -1;
	}
	...
	public static boolean canInsertItemToInventory(IInventory inventory, ItemStack itemStack, int slot,
			EnumFacing side) {
		return !inventory.isItemValidForSlot(slot, itemStack) ? false
				: !(inventory instanceof ISidedInventory)
						|| ((ISidedInventory) inventory).canInsertItem(slot, itemStack, side);
	}

	private static boolean canExtractItemFromInventory(IInventory inventory, ItemStack itemStack, int slot,
			EnumFacing side) {
	...
	public static GetFirstStackStartingFromSlotResult getFirstStackStartingFromSlot(IInventory inventory,
			ItemStack itemStack, int slot, EnumFacing side) {
		if (inventory instanceof ISidedInventory) {
