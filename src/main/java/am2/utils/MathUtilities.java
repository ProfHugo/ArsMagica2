...
public class MathUtilities {
	...
	public static Vec3d floorToI(Vec3d old) {
		return new Vec3d((float) Math.floor(old.xCoord), (float) Math.floor(old.yCoord),
				(float) Math.floor(old.zCoord));
	}

	public static Vec3d roundToI(Vec3d old) {
		return new Vec3d((float) Math.round(old.xCoord), (float) Math.round(old.yCoord),
				(float) Math.round(old.zCoord));
	}

	public static Vec3d[] GetHorizontalBlocksInFrontOfCharacter(EntityLivingBase entity, int numBlocks, int x, int y,
			int z) {
		float speed = 0.1F;
	...
	public static Entity[] GetEntitiesInAngleNearEntity(World world, EntityLivingBase source, int degrees, int radius,
			Class<? extends Entity> filterClass, boolean includeSource) {
		...
		List<? extends Entity> distanceFilter = world.getEntitiesWithinAABB(filterClass,
				new AxisAlignedBB((float) source.posX - radius, source.posY - radius, (float) source.posZ - radius,
		...
		// calculate the min and max angle that the mob can be within the
		// player's view
	...
	public static boolean isInFieldOfVision(Entity e1, EntityLivingBase e2, float angleX, float angleY) {
		...
		boolean flag1 = (yawFOVMin < 0F && (rotationYawPrime >= yawFOVMin + 360F || rotationYawPrime <= yawFOVMax))
				...
				|| (yawFOVMax < 360F && yawFOVMin >= 0F && rotationYawPrime <= yawFOVMax
						&& rotationYawPrime >= yawFOVMin);
		boolean flag2 = (pitchFOVMin <= -180F
				...
				|| (pitchFOVMax < 180F && pitchFOVMin >= -180F && rotationPitchPrime <= pitchFOVMax
	...
	public static Entity getPointedEntity(World world, EntityLivingBase entityplayer, double range,
			double collideRadius, boolean nonCollide) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			RayTraceResult mop = world.rayTraceBlocks(
					...
					new Vec3d(entity.posX, entity.posY + entity.getEyeHeight(), entity.posZ), false);
