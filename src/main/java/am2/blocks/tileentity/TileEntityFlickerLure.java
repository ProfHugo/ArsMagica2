...
public class TileEntityFlickerLure extends TileEntityAMPower {
	...
	public void update() {
		...
		if (worldObj.isBlockIndirectlyGettingPowered(pos) > 0) {
			if (worldObj.rand.nextDouble() < 0.05f && PowerNodeRegistry.For(worldObj).checkPower(this, 20)) {
				...
				PowerNodeRegistry.For(worldObj).consumePower(this,
						PowerNodeRegistry.For(worldObj).getHighestPowerType(this), 20);
			}
		}
		...
		if (ticksExisted % 200 == 0) {
			...
			List<EntityFlicker> flickers = worldObj.getEntitiesWithinAABB(EntityFlicker.class,
					new AxisAlignedBB(pos).expandXyz(5));
			for (EntityFlicker flicker : flickers) {
				...
				if (inventory != null) {
					...
					for (int i = 0; i < inventory.getSizeInventory(); i++) {
						...
						if (is == null || is.getItem() != ItemDefs.flickerJar
								|| is.getItemDamage() != ArsMagicaAPI.getAffinityRegistry().getId(Affinity.NONE))
							continue;
						is.stackSize--;
						if (is.stackSize <= 0)
							is = null;
					...
					if (jar != null) {
						...
						if (!placed) {
							EntityItem item = new EntityItem(worldObj, pos.getX() + 0.5D, pos.getY() + 1.5D,
									pos.getZ() + 0.5D, jar);
