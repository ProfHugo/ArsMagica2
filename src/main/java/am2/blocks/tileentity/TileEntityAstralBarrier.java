...
public class TileEntityAstralBarrier extends TileEntityAMPower
		implements IInventory, IKeystoneLockable<TileEntityAstralBarrier> {
	...
	public boolean IsActive() {
		return PowerNodeRegistry.For(this.worldObj).checkPower(this, 0.35f * getRadius())
				&& worldObj.isBlockIndirectlyGettingPowered(pos) > 0 && getRadius() > 0;
	}
	...
	public void update() {
		...
		if (IsActive()) {
			PowerNodeRegistry.For(this.worldObj).consumePower(this,
					PowerNodeRegistry.For(worldObj).getHighestPowerType(this), 0.35f * radius);
		}

		if (worldObj.isRemote) {
			if (IsActive()) {
				if (displayAura) {
					AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "symbols",
							pos.getX(), pos.getY() + 0.5, pos.getZ());
					if (effect != null) {
						effect.setIgnoreMaxAge(false);
						effect.setMaxAge(100);
						effect.setParticleScale(0.5f);
						effect.AddParticleController(new ParticleOrbitPoint(effect, pos.getX() + 0.5, pos.getY() + 0.5,
								pos.getZ() + 0.5, 1, false).SetOrbitSpeed(0.03).SetTargetDistance(radius));
					}
				}
				...
				if (particleTickCounter >= 15) {
					...
					AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "sparkle",
							pos.getX() + 0.5, pos.getY() + 0.1 + worldObj.rand.nextDouble() * 0.5, pos.getZ() + 0.5);
					if (effect != null) {
						effect.setIgnoreMaxAge(false);
						effect.setMaxAge(100);
						effect.setParticleScale(0.5f);
						effect.AddParticleController(new ParticleOrbitPoint(effect, pos.getX() + 0.5, pos.getY() + 0.5,
								pos.getZ() + 0.5, 1, false).SetOrbitSpeed(0.005)
