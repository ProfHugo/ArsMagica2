...
public class TileEntityCrystalMarkerSpellExport extends TileEntityCrystalMarker {
	...
	private void spawnParticles() {
		for (int i = 0; i < 15; ++i) {
			AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "sparkle2", pos.getX(),
					pos.getY(), pos.getZ());
			if (effect != null) {
				effect.AddParticleController(
						new ParticleFloatUpward(effect, 0, worldObj.rand.nextFloat() * 0.1f, 1, false));
				effect.AddParticleController(
						new ParticleFadeOut(effect, 2, false).setFadeSpeed(0.035f).setKillParticleOnFinish(true));
