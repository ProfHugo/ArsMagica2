...
public class TileEntityArcaneDeconstructor extends TileEntityAMPower
		implements IInventory, ISidedInventory, IKeystoneLockable<TileEntityArcaneDeconstructor> {

	private int particleCounter;
	private static final float DECONSTRUCTION_POWER_COST = 1.25f; // power cost
																	// per tick
	private static final int DECONSTRUCTION_TIME = 200; // how long does it take
														// to deconstruct
														// something?
	private int current_deconstruction_time = 0; // how long have we been
	...
	public void update() {
		...
		if (worldObj.isRemote) {
			if (particleCounter == 0 || particleCounter++ > 1000) {
				particleCounter = 1;
				radiant = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "radiant", pos.getX() + 0.5f,
			...
			if (!isActive()) {
				...
				if (inventory[0] == null) {
					...
					// worldObj.getBlockState(pos), worldObj.getBlockState(pos),
					// 2);
				} else {
		...
		if (checkStack.getItem() == ItemDefs.spell) {
					...
					for (Object o : componentParts) {
						...
						if (stack != null) {
							if (stack.getItem() == ItemDefs.bindingCatalyst) {
								stack.setItemDamage(((Binding) ArsMagicaAPI.getSpellRegistry()
										.getObject(new ResourceLocation("arsmagica2", "binding")))
						...
						for (Object o : componentParts) {
							...
							if (stack != null) {
								if (stack.getItem() == ItemDefs.crystalPhylactery) {
									ItemDefs.crystalPhylactery.setSpawnClass(stack,
											((Summon) ArsMagicaAPI.getSpellRegistry()
													.getObject(new ResourceLocation("arsmagica2", "summon")))
			...
			if (recipeParts != null && checkStack != null && recipe.getRecipeOutput() != null) {
				if (recipe.getRecipeOutput().getItem() == checkStack.getItem()
						...
						&& recipe.getRecipeOutput().stackSize > 1)
					return false;
