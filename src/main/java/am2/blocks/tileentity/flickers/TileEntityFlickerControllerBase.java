...
public class TileEntityFlickerControllerBase extends TileEntityAMPower
	...
	protected void setOperator(AbstractFlickerFunctionality operator) {
		if (this.operator != null) {
			this.operator.RemoveOperator(worldObj, this,
					PowerNodeRegistry.For(worldObj).checkPower(this, this.operator.PowerPerOperation()), nearbyList);
		}
	...
	private EnumFacing getNeighboringEnumFacing(TileEntity neighbor) {
		if (neighbor.getPos().getX() == this.pos.getX() && neighbor.getPos().getY() == this.pos.getY()
				&& neighbor.getPos().getZ() == this.pos.getZ() + 1)
			return EnumFacing.SOUTH;
		else if (neighbor.getPos().getX() == this.pos.getX() && neighbor.getPos().getY() == this.pos.getY()
				&& neighbor.getPos().getZ() == this.pos.getZ() - 1)
			return EnumFacing.NORTH;
		else if (neighbor.getPos().getX() == this.pos.getX() + 1 && neighbor.getPos().getY() == this.pos.getY()
				&& neighbor.getPos().getZ() == this.pos.getZ())
			return EnumFacing.EAST;
		else if (neighbor.getPos().getX() == this.pos.getX() - 1 && neighbor.getPos().getY() == this.pos.getY()
				&& neighbor.getPos().getZ() == this.pos.getZ())
			return EnumFacing.WEST;
		else if (neighbor.getPos().getX() == this.pos.getX() && neighbor.getPos().getY() == this.pos.getY() + 1
				&& neighbor.getPos().getZ() == this.pos.getZ())
	...
	public void update() {
		...
		// if redstone powered, increment the tick counter (so that operator
		// time still continues), but do nothing else.
		// this allows a redstone signal to effectively turn off any flicker
		// habitat.
		...
		if (operator != null) {
			...
			// this allows individual upgrades to be turned off by providing
			// them with a redstone signal.
				...
				if ((powered && operator.RequiresPower()) || !operator.RequiresPower()) {
					...
					if (success || operator.RequiresPower())
						PowerNodeRegistry.For(worldObj).consumePower(this,
								PowerNodeRegistry.For(worldObj).getHighestPowerType(this),
								operator.PowerPerOperation());
	...
	public void readFromNBT(NBTTagCompound par1nbtTagCompound) {
		...
		NBTTagList sigilMetaStore = par1nbtTagCompound.getTagList("sigil_metadata_collection",
