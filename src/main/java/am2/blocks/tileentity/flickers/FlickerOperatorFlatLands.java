...
public class FlickerOperatorFlatLands extends AbstractFlickerFunctionality {
	...
	public boolean DoOperation(World worldObj, IFlickerController<?> habitat, boolean powered) {
		...
		if (!worldObj.isRemote) {
			...
			for (int i = 0; i < searchesPerLoop && !actionPerformed; ++i) {
				...
				if (block != null && !worldObj.isAirBlock(effectPos) && block.isOpaqueCube()
						&& block.getBlock() != BlockDefs.invisibleUtility) {
					if (ForgeEventFactory.doPlayerHarvestCheck(new DummyEntityPlayer(worldObj), block, true)) {
						if (block.getBlock().removedByPlayer(block, worldObj, effectPos,
								new DummyEntityPlayer(worldObj), true)) {
	...
	public Object[] getRecipe() {
		return new Object[] { "S P", "ENI", " R ", Character.valueOf('S'), Items.IRON_SHOVEL, Character.valueOf('P'),
				Items.IRON_PICKAXE, Character.valueOf('E'),
				...
				Character.valueOf('I'),
				new ItemStack(ItemDefs.flickerJar, 1, ArsMagicaAPI.getAffinityRegistry().getId(Affinity.ICE)),
