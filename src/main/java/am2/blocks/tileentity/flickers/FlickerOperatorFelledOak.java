...
public class FlickerOperatorFelledOak extends AbstractFlickerFunctionality {
	...
	void beginTreeFelling(World world, BlockPos pos) {
		...
		if (wood.getBlock().isWood(world, pos)) {
			...
			while (true) {
				...
				if (!isLog(world, new BlockPos(pos.getX(), height, pos.getZ())))
					break;
			}
			height--;
			int numLeaves = 0;
			...
			// if (!world.isRemote)
			// world.playAuxSFX(2001, pos, Block.getIdFromBlock(wood.getBlock())
			// + (WorldUtils.getBlockMeta(wood) << 12));
		}
	}
	...
	private AMVector3 getPlantLocation(World worldObj, IFlickerController<?> habitat, ItemStack sapling) {
		...
		if (data == null || data.length == 0) {
			offset = new AMVector3(te.getPos().getX() - radius_horiz, te.getPos().getY() - radius_vert,
					te.getPos().getZ() - radius_horiz);
		} else {
		...
		for (int i = (int) offset.x; i <= te.getPos().getX() + radius_horiz; i += 2) {
			for (int k = (int) offset.z; k <= te.getPos().getZ() + radius_horiz; k += 2) {
				for (int j = (int) offset.y; j <= te.getPos().getY() + radius_vert; ++j) {
					...
					if (block.getBlock().isReplaceable(worldObj, newPos)
							&& treeBlock.canPlaceBlockAt(worldObj, newPos)) {
	...
	private ItemStack getSaplingFromNearbyChest(World worldObj, IFlickerController<?> habitat) {
		for (EnumFacing dir : EnumFacing.values()) {
			...
			if (inv == null)
				continue;
			int index = InventoryUtilities.getInventorySlotIndexFor(inv,
	...
	/**
	 * Gets an instance of the adjacent IInventory at direction offset. Returns
	 * null if not found or invalid type adjacent.
	 */
	...
	public Object[] getRecipe() {
		return new Object[] { "WG ", "NCL", " OW", Character.valueOf('W'), BlockDefs.witchwoodLog,
