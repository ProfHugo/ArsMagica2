...
public class FlickerOperatorItemTransport extends AbstractFlickerFunctionality {
	...
	public boolean DoOperation(World worldObj, IFlickerController<?> controller, boolean powered, Affinity[] flickers) {
		...
		for (int inIterator = habitat.getInListPosition(); inIterator < habitat.getInListSize(); ++inIterator) {
			// Gets the tile entity for the attached crystal marker at the
			// specified location
	...
	private boolean moveItem(World worldObj, IInventory inventory, TileEntityCrystalMarker crystalMarkerTE,
			TileEntityFlickerHabitat habitat, int toMove) {
		boolean itemFound = false;
		ItemStack orgStack = null;
		int currentSlot = 0;
		GetFirstStackStartingFromSlotResult result = InventoryUtilities.getFirstStackStartingFromSlot(inventory,
				orgStack, currentSlot, crystalMarkerTE.getFacing());
		...
		while (orgStack != null) {
			if (orgStack != null && InputCanMove(crystalMarkerTE, orgStack, inventory)) {
				...
				if (FindOutput(worldObj, habitat, mergeStack, inventory)) {
					// if a valid item was found in input, and a valid output
					// for it was found remove the item from the source
			...
			result = InventoryUtilities.getFirstStackStartingFromSlot(inventory, orgStack, currentSlot,
					crystalMarkerTE.getFacing());
			orgStack = result.stack;
			currentSlot = result.slot;
			if (currentSlot == -1) {
				orgStack = null;
	...
	/**
	 * Ensures that the stack can be moved through the crystalMarker from the
	 * inventory
	 *
	 * @param crystalMarker
	 *            The crystal marker doing the move
	 * @param stack
	 *            The ItemStack to move
	 * @param inventory
	 *            The Inventory the item is being moved from
	...
	private boolean InputCanMove(TileEntityCrystalMarker crystalMarker, ItemStack stack, IInventory inventory) {
		...
		case BlockCrystalMarker.META_REGULATE_MULTI:
			return crystalMarker.filterHasItem(stack)
					&& crystalMarker.getFilterCount(stack) < InventoryUtilities.getLikeItemCount(inventory, stack);
	...
	private boolean FindOutput(World worldObj, TileEntityFlickerHabitat attuner, ItemStack stack, IInventory source) {
		...
		for (int priority = 0; priority <= TileEntityFlickerHabitat.PRIORITY_FINAL; ++priority) {
			...
			if (attuner.getOutListPosition(priority) >= attuner.getOutListSize(priority)) {
				// if the out list position has gone outside the list size reset
				// it to 0
			...
			while (!fullLoop) {
				...
				if (te != null && te instanceof IInventory) {
					...
					itemMoved = outputItem(markerType, new IInventory[] { inventory }, stack, crystalMarkerTE,
							new IInventory[] { source });
				...
				if (!itemMoved && te instanceof TileEntityChest) {
					...
					if (adjacent != null) {
						...
						itemMoved = outputItem(markerType, new IInventory[] { inventory, (IInventory) te }, stack,
								crystalMarkerTE, new IInventory[] { source });
	 ...
	 *
	 * @param markerType
	 *            The type of marker attached to the inventory we are trying to
	 *            move into
	 * @param inventory
	 *            The inventory we are trying to move into
	 * @param stack
	 *            The item stack we are trying to move
	 * @return Returns true if successful, false otherwise
	 */
	private boolean outputItem(int markerType, IInventory[] outputs, ItemStack stack,
			TileEntityCrystalMarker crystalMarkerTE, IInventory[] sources) {
		...
		case BlockCrystalMarker.META_FINAL_DEST:
			...
			for (IInventory inventory : outputs) {
				if (InventoryUtilities.inventoryHasRoomFor(inventory, stack, 1, side)
						&& isDestinationDifferentFromSource(outputs, sources)) {
		...
		case BlockCrystalMarker.META_LIKE_EXPORT:
			// One of the destination inventories must already contain at least
			// 1 of the item we are trying to move and
			...
			for (IInventory inventory : outputs) {
				if (atLeastOneContains && InventoryUtilities.inventoryHasRoomFor(inventory, stack, 1, side)
						&& isDestinationDifferentFromSource(outputs, sources)) {
					return InventoryUtilities.mergeIntoInventory(inventory, stack, stack.stackSize, side);
				}
			}
		...
		case BlockCrystalMarker.META_SET_EXPORT:
			// Check that the item exists in the item filter and that the
			// inventory has room for the item
			for (IInventory inventory : outputs) {
				if (crystalMarkerTE.filterHasItem(stack)
						&& InventoryUtilities.inventoryHasRoomFor(inventory, stack, 1, side)
						&& isDestinationDifferentFromSource(outputs, sources)) {
					return InventoryUtilities.mergeIntoInventory(inventory, stack, stack.stackSize, side);
				}
			}
		...
		case BlockCrystalMarker.META_REGULATE_EXPORT:
			// Check that the item exists in the item filter and that the
			// inventory doesn't have more then the allocated amount already
			// and that the inventory has room for the item
			for (IInventory inventory : outputs) {
				if (crystalMarkerTE.filterHasItem(stack)
						&& InventoryUtilities.getLikeItemCount(inventory, stack, side) < crystalMarkerTE
								.getFilterCount(stack)
						&& InventoryUtilities.inventoryHasRoomFor(inventory, stack, 1, side)
						&& isDestinationDifferentFromSource(outputs, sources)) {
					return InventoryUtilities.mergeIntoInventory(inventory, stack, stack.stackSize, side);
				}
	 ...
	 *
	 * @param worldObj
	 *            The world object to search through
	 * @param xy
	 * @param y
	 * @param z
	 * @return
	...
	private TileEntity GetAttachedCrystalMarkerTileEntity(World worldObj, TileEntityCrystalMarker crystalMarkerTE,
			AMVector3 cmVector) {
		TileEntity te = null;
	...
	public Object[] getRecipe() {
		return new Object[] { " B ", "CAC", " B ", Character.valueOf('A'),
