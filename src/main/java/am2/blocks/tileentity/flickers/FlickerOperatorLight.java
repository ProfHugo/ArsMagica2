...
public class FlickerOperatorLight extends AbstractFlickerFunctionality {
		...
		if (!worldObj.isRemote) {
			...
			for (int i = 0; i < checksPerOperation; ++i) {
				...
				if (light < 10 && worldObj.isAirBlock(checkPos)) {
					worldObj.setBlockState(checkPos, BlockDefs.invisibleUtility.getDefaultState()
							.withProperty(BlockInvisibleUtility.TYPE, EnumInvisibleType.SPECIAL_ILLUMINATED), 2);
				}

				checkPos.east();
				if (checkPos.getX() > ((TileEntity) habitat).getPos().getX() + radius) {
					checkPos = new BlockPos(((TileEntity) habitat).getPos().getX() - radius, checkPos.getY(),
							checkPos.getZ());
					checkPos = checkPos.up();
					if (checkPos.getY() > ((TileEntity) habitat).getPos().getY() + yRadius) {
						checkPos = new BlockPos(checkPos.getX(), ((TileEntity) habitat).getPos().getY() - yRadius,
								checkPos.getZ());
						checkPos = checkPos.south();
						if (checkPos.getZ() > ((TileEntity) habitat).getPos().getZ() + yRadius) {
							checkPos = new BlockPos(checkPos.getX(), checkPos.getY(),
									((TileEntity) habitat).getPos().getZ() - radius);
						}
					}
				}
			}
			...
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "sparkle",
					((TileEntity) habitat).getPos().getX() + 0.5, ((TileEntity) habitat).getPos().getY() + 1,
	...
	public void RemoveOperator(World worldObj, IFlickerController<?> habitat, boolean powered) {
		...
		if (!worldObj.isRemote) {
			...
			for (int i = ((TileEntity) habitat).getPos().getX() - radius; i <= ((TileEntity) habitat).getPos().getX()
					+ radius; ++i) {
				for (int j = ((TileEntity) habitat).getPos().getY() - yRadius; j <= ((TileEntity) habitat).getPos()
						.getY() + yRadius; ++j) {
					for (int k = ((TileEntity) habitat).getPos().getY() - radius; k <= ((TileEntity) habitat).getPos()
							.getY() + radius; ++k) {
						...
						if (block == BlockDefs.invisibleUtility) {
							if (BlockInvisibleUtility.getType(
									worldObj.getBlockState(removePos)) == EnumInvisibleType.SPECIAL_ILLUMINATED) {
	...
	public Object[] getRecipe() {
		return new Object[] { "ISI", "F L", "ISI", Character.valueOf('F'),
				...
				Character.valueOf('L'),
				new ItemStack(ItemDefs.flickerJar, 1, ArsMagicaAPI.getAffinityRegistry().getId(Affinity.LIGHTNING)),
