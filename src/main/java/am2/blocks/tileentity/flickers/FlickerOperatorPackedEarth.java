...
public class FlickerOperatorPackedEarth extends AbstractFlickerFunctionality {
	...
	public boolean DoOperation(World worldObj, IFlickerController<?> habitat, boolean powered) {
		...
		if (!worldObj.isRemote) {
			...
			for (int i = 0; i < searchesPerLoop && !actionPerformed; ++i) {
				...
				BlockPos effectPos = ((TileEntity) habitat).getPos().add(-radius, -1, -radius).add(
						worldObj.rand.nextInt(diameter), worldObj.rand.nextInt(diameter),
						worldObj.rand.nextInt(radius));
				...
				if (worldObj.isAirBlock(effectPos) || block.isReplaceable(worldObj, effectPos)) {
					...
					if (inventoryIndex > -1) {
						...
						worldObj.setBlockState(effectPos,
								Block.getBlockFromItem(stack.getItem()).getStateFromMeta(stack.getItemDamage()));
	...
	public Object[] getRecipe() {
		return new Object[] { "DDD", "RFR", " E ", Character.valueOf('D'), Blocks.DIRT, Character.valueOf('R'),
				new ItemStack(ItemDefs.rune, 1, EnumDyeColor.BLACK.getDyeDamage()), Character.valueOf('E'),
				AffinityShiftUtils.getEssenceForAffinity(Affinity.EARTH), Character.valueOf('F'),
