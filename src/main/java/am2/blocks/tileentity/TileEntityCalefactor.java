...
public class TileEntityCalefactor extends TileEntityAMPower
		implements IInventory, ISidedInventory, IKeystoneLockable<TileEntityCalefactor>, ITileEntityAMBase {

	private ItemStack calefactorItemStacks[];
	private float rotationX, rotationY, rotationZ;
	private float rotationStepX;
	private final short baseCookTime = 220; // default to the same as a standard
											// furnace
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			...
			if (this.calefactorItemStacks[0].getItem() instanceof ItemFood
					|| this.calefactorItemStacks[0].getItem() instanceof ItemBlock
					|| this.calefactorItemStacks[0].getItem() == ItemDefs.itemOre) {
		...
		if (this.worldObj.isRemote) {
			...
			if (this.isCooking) {
				...
				if (particleCount <= 0) {
					...
					ArsMagica2.proxy.particleManager.BeamFromPointToPoint(worldObj, rStartX, rStartY, rStartZ, endX,
							endY, endZ, 0xFF8811);
					if (worldObj.rand.nextBoolean()) {
						AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "smoke", endX,
								endY, endZ);
						if (effect != null) {
						...
						AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "explosion_2",
								endX, endY, endZ);
						if (effect != null) {
							effect.setIgnoreMaxAge(false);
							effect.setMaxAge(10);
							effect.setParticleScale(0.04f);
							effect.addVelocity(worldObj.rand.nextDouble() * 0.2f - 0.1f, 0.2f,
									worldObj.rand.nextDouble() * 0.2f - 0.1f);
		...
		if (this.canSmelt() && this.isSmelting() && powerCheck) {
			...
			if (this.timeSpentCooking >= getModifiedCookTime()) {
				if (!this.worldObj.isRemote) {
					...
					worldObj.playSound(pos.getX(), pos.getY(), pos.getZ(), AMSounds.CALEFACTOR_BURN,
							SoundCategory.BLOCKS, 0.2f, 1.0f, true);
				}
			...
			if (!worldObj.isRemote) {
				if (PowerNodeRegistry.For(worldObj).checkPower(this, PowerTypes.DARK, getCookTickPowerCost())
					...
					PowerNodeRegistry.For(this.worldObj).consumePower(this,
							PowerNodeRegistry.For(this.worldObj).getHighestPowerType(this), getCookTickPowerCost());
				}
			}
