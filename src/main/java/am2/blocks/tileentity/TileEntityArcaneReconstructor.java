...
public class TileEntityArcaneReconstructor extends TileEntityAMPower
		implements IInventory, ISidedInventory, IKeystoneLockable<TileEntityArcaneReconstructor> {
	...
	public void update() {
		if (isFirstTick) {
			outerRingRotationSpeeds = new AMVector3(worldObj.rand.nextDouble() * 4 - 2,
					worldObj.rand.nextDouble() * 4 - 2, worldObj.rand.nextDouble() * 4 - 2);
			middleRingRotationSpeeds = new AMVector3(worldObj.rand.nextDouble() * 4 - 2,
					worldObj.rand.nextDouble() * 4 - 2, worldObj.rand.nextDouble() * 4 - 2);
			innerRingRotationSpeeds = new AMVector3(worldObj.rand.nextDouble() * 4 - 2,
					worldObj.rand.nextDouble() * 4 - 2, worldObj.rand.nextDouble() * 4 - 2);
			isFirstTick = false;
		}

		if (PowerNodeRegistry.For(this.worldObj).checkPower(this, this.getRepairCost())) {// has
																							// enough
																							// power
			if ((repairCounter++ % getRepairRate() == 0) && (!queueRepairableItem())) {// has
																						// ticked
																						// and
																						// already
																						// has
				...
				if (performRepair()) {// something to repair
					if (!worldObj.isRemote) {
						PowerNodeRegistry.For(this.worldObj).consumePower(this,
								PowerNodeRegistry.For(worldObj).getHighestPowerType(this), this.getRepairCost());
					}
				}
			}
			deactivationDelayTicks = 0;
		} else if (!worldObj.isRemote && active) {// out of power, on server and
													// active
		...
		if (worldObj.isRemote) {
			...
			if (shouldRenderItemStack()) {
				AMParticle p = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "sparkle2",
						pos.getX() + 0.2 + (worldObj.rand.nextDouble() * 0.6), pos.getY() + 0.4,
						pos.getZ() + 0.2 + (worldObj.rand.nextDouble() * 0.6));
				if (p != null) {
					...
					p.AddParticleController(
							new ParticleFadeOut(p, 1, false).setFadeSpeed(0.035f).setKillParticleOnFinish(true));
	...
	private boolean performRepair() {
		...
		if (inventory[SLOT_ACTIVE].isItemDamaged()) {
			...
			worldObj.playSound(pos.getX(), pos.getY(), pos.getZ(), AMSounds.RECONSTRUCTOR_COMPLETE,
