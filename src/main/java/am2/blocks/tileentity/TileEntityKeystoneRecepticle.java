...
public class TileEntityKeystoneRecepticle extends TileEntityAMPower
		implements IInventory, IMultiblockStructureController, IKeystoneLockable<TileEntityKeystoneRecepticle> {
	...
	public void initMultiblock() {
		...
		map.put(6, Blocks.STONE_BRICK_STAIRS.getDefaultState().withProperty(FACING, EnumFacing.SOUTH).withProperty(HALF,
				EnumHalf.TOP));
		map.put(5, Blocks.STONE_BRICK_STAIRS.getDefaultState().withProperty(FACING, EnumFacing.WEST).withProperty(HALF,
				EnumHalf.TOP));
		map.put(4, Blocks.STONE_BRICK_STAIRS.getDefaultState().withProperty(FACING, EnumFacing.EAST).withProperty(HALF,
				EnumHalf.TOP));
		map.put(8, Blocks.STONEBRICK.getDefaultState());
		map.put(9, Blocks.STONEBRICK.getDefaultState().withProperty(BlockStoneBrick.VARIANT,
				BlockStoneBrick.EnumType.CHISELED));
		MultiblockGroup keystone = new MultiblockGroup("keystoneRecepticle",
				Lists.newArrayList(
						BlockDefs.keystoneRecepticle.getDefaultState().withProperty(BlockKeystoneReceptacle.FACING,
								EnumFacing.EAST),
						BlockDefs.keystoneRecepticle.getDefaultState().withProperty(BlockKeystoneReceptacle.FACING,
								EnumFacing.WEST)),
				false);
		...
		// secondary
		// row 0
		MultiblockGroup keystone_alt = new MultiblockGroup("keystoneRecepticle_alt",
				Lists.newArrayList(
						BlockDefs.keystoneRecepticle.getDefaultState().withProperty(BlockKeystoneReceptacle.FACING,
								EnumFacing.NORTH),
						BlockDefs.keystoneRecepticle.getDefaultState().withProperty(BlockKeystoneReceptacle.FACING,
	...
	public void setActive(long key) {
		...
		if (!this.worldObj.isRemote) {
			...
			worldObj.playSound(pos.getX(), pos.getY(), pos.getZ(), AMSounds.GATEWAY_OPEN, SoundCategory.BLOCKS, 1.0f,
					1.0f, true);
		}
	}
	...
	public void update() {
		...
		AxisAlignedBB bb = new AxisAlignedBB(pos.getX() + 0.3, pos.getY() - 3, pos.getZ() + 0.3, pos.getX() + 0.7,
				pos.getY(), pos.getZ() + 0.7);
		...
		if (this.isActive) {
			...
			if (entities.size() == 1 && worldObj.canBlockSeeSky(pos)) {
				...
				if (entity instanceof EntityPlayer) {
					...
					if (player.isPotionActive(PotionEffectsDefs.haste)
							&& player.isPotionActive(Potion.getPotionFromResourceLocation("speed"))
							&& player.isSprinting()) {
						// if (worldObj.isRemote)
						// player.addStat(ArsMagica2.achievements.EightyEightMilesPerHour,
						// 1);
						this.key = 0;
						if (!worldObj.isRemote) {
							EntityLightningBolt elb = new EntityLightningBolt(worldObj, pos.getX(), pos.getY(),
									pos.getZ(), false);
	...
	private void doTeleport(Entity entity) {
		...
		AMVector3 newLocation = ArsMagica2.proxy.blocks.getNextKeystonePortalLocation(this.worldObj, pos, false,
				this.key);
		...
		if (ArsMagica2.config.getHazardousGateways()) {
			// uh-oh! Not enough power! The teleporter will still send you
			// though, but I wonder where...
			float charge = PowerNodeRegistry.For(this.worldObj).getHighestPower(this);
			if (charge < essenceCost) {
				essenceCost = charge;
				// get the distance that our charge *will* take us towards the
				// next point
				...
				// interpolate the distance at that angle - this is the new
				// position
			...
			this.worldObj.playSound(newLocation.x, newLocation.y, newLocation.z, SoundEvents.ENTITY_ENDERMEN_TELEPORT,
		...
		entity.setPositionAndRotation(newLocation.x + 0.5, newLocation.y - entity.height, newLocation.z + 0.5,
				newRotation, entity.rotationPitch);

		PowerNodeRegistry.For(this.worldObj).consumePower(this,
				PowerNodeRegistry.For(this.worldObj).getHighestPowerType(this), essenceCost);

		this.worldObj.playSound(myLocation.x, myLocation.y, myLocation.z, SoundEvents.ENTITY_ENDERMEN_TELEPORT,
				SoundCategory.BLOCKS, 1.0F, 1.0F, true);
		this.worldObj.playSound(newLocation.x, newLocation.y, newLocation.z, SoundEvents.ENTITY_ENDERMEN_TELEPORT,
				SoundCategory.BLOCKS, 1.0F, 1.0F, true);
	}
	...
	public void readFromNBT(NBTTagCompound nbttagcompound) {
		...
		ArsMagica2.proxy.blocks.registerKeystonePortal(pos,
	...
	public AxisAlignedBB getRenderBoundingBox() {
		return new AxisAlignedBB(pos.getX() - 3, pos.getY() - 3, pos.getZ() - 3, pos.getX() + 3, pos.getY() + 3,
				pos.getZ() + 3);
	}
