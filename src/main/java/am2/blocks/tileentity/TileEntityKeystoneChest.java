...
public class TileEntityKeystoneChest extends TileEntityLockableLoot
		implements IInventory, ITickable, IKeystoneLockable<TileEntityKeystoneChest> {
	...
	public void update() {
		...
		if (numPlayersUsing > 0) {
			if (getLidAngle() == 0) {
				this.worldObj.playSound(this.getPos().getX() + 0.5D, this.getPos().getY() + 0.5D,
						this.getPos().getZ() + 0.5D, SoundEvents.BLOCK_CHEST_OPEN, SoundCategory.BLOCKS, 0.5F,
			...
			if (getLidAngle() == 1.0f) {
				this.worldObj.playSound(this.getPos().getX() + 0.5D, this.getPos().getY() + 0.5D, this.getPos().getZ(),
