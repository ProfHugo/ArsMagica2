...
public class TileEntityManaDrain extends TileEntityAMPower implements IMultiblockStructureController {
	...
	private void setupMultiblock() {
		MultiblockGroup walls = new MultiblockGroup("walls", Lists.newArrayList(BlockDefs.magicWall.getDefaultState()),
				false);
		...
		MultiblockGroup pillars = new MultiblockGroup("pillars", Lists.newArrayList(
		...
		MultiblockGroup caps = new MultiblockGroup("caps", Lists.newArrayList(BlockDefs.blocks.getDefaultState()
				.withProperty(BlockArsMagicaBlock.BLOCK_TYPE, EnumBlockType.BLUETOPAZ)), false);
		caps.addBlock(new BlockPos(-2, 3, 0));
		caps.addBlock(new BlockPos(2, 3, 0));
		caps.addBlock(new BlockPos(0, 3, -2));
		caps.addBlock(new BlockPos(0, 3, 2));

		MultiblockGroup drain = new MultiblockGroup("drain", Lists.newArrayList(BlockDefs.manaDrain.getDefaultState()),
				true);
	...
	public void update() {
		...
		if (!isMultiblockComplete())
			return;
		if (PowerNodeRegistry.For(worldObj).getPower(this, PowerTypes.NEUTRAL) == capacity)
			return;
		List<EntityLivingBase> entities = worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
				new AxisAlignedBB(pos.add(-1, 1, -1), pos.add(2, 4, 2)));
			...
			if (worldObj.isBlockIndirectlyGettingPowered(pos) == 0) {
				...
				if (!worldObj.isRemote) {
					...
					if (ext.getCurrentMana() >= drain) {
						PowerNodeRegistry.For(worldObj).setPower(this, PowerTypes.NEUTRAL,
								PowerNodeRegistry.For(worldObj).getPower(this, PowerTypes.NEUTRAL) + drain * 10);
						...
						PowerNodeRegistry.For(worldObj).setPower(this, PowerTypes.NEUTRAL,
								PowerNodeRegistry.For(worldObj).getPower(this, PowerTypes.NEUTRAL) + 40);
					}
				}
			} else {
		...
		if (worldObj.isRemote && isWorking) {
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "sparkle2",
					pos.getX() - 1 + worldObj.rand.nextDouble() * 3, pos.getY() + 2,
					pos.getZ() - 1 + worldObj.rand.nextDouble() * 3);
			if (particle != null) {
				particle.setRGBColorI(0x00AAFF);
				particle.AddParticleController(new ParticleFloatUpward(particle, 0.1f,
						worldObj.isBlockIndirectlyGettingPowered(pos) == 0 ? -0.05f : 0.05f, 1, false));
			}
		}
	}
