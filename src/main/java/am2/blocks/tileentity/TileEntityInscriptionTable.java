...
public class TileEntityInscriptionTable extends TileEntity implements IInventory, ITickable, ITileEntityAMBase {
	...
	public void setInUse(EntityPlayer player) {
		...
		if (!this.worldObj.isRemote) {
			this.markDirty();
			// worldObj.markAndNotifyBlock(pos,
			// worldObj.getChunkFromBlockCoords(pos),
		...
		if (!worldObj.isRemote) {
			...
			if (shouldSet)
				this.worldObj.setBlockState(pos,
						worldObj.getBlockState(pos).withProperty(BlockInscriptionTable.TIER_1, getUpgradeState() >= 1)
								.withProperty(BlockInscriptionTable.TIER_2, getUpgradeState() >= 2)
								.withProperty(BlockInscriptionTable.TIER_3, getUpgradeState() >= 3),
						2);
		}
		this.markDirty();
		// worldObj.markAndNotifyBlock(pos,
		// worldObj.getChunkFromBlockCoords(pos), worldObj.getBlockState(pos),
	...
	private void candleUpdate() {
		...
		if (isRenderingLeft()) {
			if (ticksToNextParticle == 0 || ticksToNextParticle == 15) {
				...
				AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "fire", particleX,
						getPos().getY() + 1.32, particleZ);
				if (effect != null) {
				...
				if (worldObj.rand.nextInt(100) > 80) {
					AMParticle smoke = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "smoke", particleX,
							getPos().getY() + 1.4, particleZ);
					if (smoke != null) {
	...
	private void parseTagCompound(NBTTagCompound par1NBTTagCompound) {
		...
		for (int i = 0; i < shapeGroups.tagCount(); i++) {
			...
			for (int j = 0; j < tmplist.tagCount(); j++) {
				...
				parts.add(tmp.getInteger("Slot"),
						ArsMagicaAPI.getSpellRegistry().getObject(new ResourceLocation(tmp.getString("ID"))));
			}
		...
		for (int i = 0; i < recipe.tagCount(); i++) {
			...
			currentRecipe.add(tmp.getInteger("Slot"),
					ArsMagicaAPI.getSpellRegistry().getObject(new ResourceLocation(tmp.getString("ID"))));
		}
	...
	private void countModifiers() {
		...
		// ArrayList<AbstractSpellPart> currentStage = stages.get(stages.size()
		// - 1);
		// countModifiersInList(currentStage);
	}
	...
	public void createSpellForPlayer(EntityPlayer player) {
		if (worldObj.isRemote) {
			...
			KeyValuePair<ArrayList<AbstractSpellPart>, NBTTagCompound> curRecipeSetup = new KeyValuePair<>(
					currentRecipe, new NBTTagCompound());

			for (ArrayList<AbstractSpellPart> arr : shapeGroups) {
				shapeGroupSetup
						.add(new KeyValuePair<ArrayList<AbstractSpellPart>, NBTTagCompound>(arr, new NBTTagCompound()));
			}
	...
	public ItemStack writeRecipeAndDataToBook(ItemStack bookstack, EntityPlayer player, String title) {
		if (bookstack.getItem() == Items.WRITTEN_BOOK && this.currentRecipe != null) {
			...
			else if (bookstack.getTagCompound().getBoolean("spellFinalized")) // don't
																				// overwrite
																				// a
																				// completed
																				// spell
			...
			for (AbstractSpellPart part : allRecipeItems) {
				...
				SpellRecipeItemsEvent event = new SpellRecipeItemsEvent(SpellRegistry.getSkillFromPart(part).getID(),
						recipeItems);
				MinecraftForge.EVENT_BUS.post(event);
				recipeItems = event.recipeItems;

				if (recipeItems == null) {
					LogHelper.error("Unable to write recipe to book.  Recipe items are null for part %s!",
							SpellRegistry.getSkillFromPart(part).getName());
					return bookstack;
				}
			...
			materialsList.put(ItemDefs.spellParchment.getItemStackDisplayName(new ItemStack(ItemDefs.spellParchment)),
					1);
			...
			// worldObj.playSound(getPos().getX(), getPos().getY(),
			// getPos().getZ(), "arsmagica2:misc.inscriptiontable.takebook",
			// 1.0f, 1.0f, true);
			this.markDirty();
			// worldObj.markAndNotifyBlock(pos,
			// worldObj.getChunkFromBlockCoords(pos),
			// worldObj.getBlockState(pos), worldObj.getBlockState(pos), 2);
		}
		return bookstack;
	}
	...
	public void incrementUpgradeState() {
		this.numStageGroups++;
		if (!this.worldObj.isRemote) {
			List<EntityPlayerMP> players = this.worldObj.getEntitiesWithinAABB(EntityPlayerMP.class,
					new AxisAlignedBB(pos).expand(256, 256, 256));
