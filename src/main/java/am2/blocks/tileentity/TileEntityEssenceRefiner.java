...
public class TileEntityEssenceRefiner extends TileEntityAMPower
		implements IInventory, IKeystoneLockable<TileEntityEssenceRefiner>, ISidedInventory {
			...
			if (isRefining()) {
				...
				if (PowerNodeRegistry.For(this.worldObj).checkPower(this, TICK_REFINE_COST)) {
					...
					if (remainingRefineTime % 10 == 0)
						worldObj.markAndNotifyBlock(pos, worldObj.getChunkFromBlockCoords(pos),
					...
					PowerNodeRegistry.For(this.worldObj).consumePower(this,
							PowerNodeRegistry.For(this.worldObj).getHighestPowerType(this), TICK_REFINE_COST);
				}
			} else {
				setActiveTexture();
			}
	...
	private void setActiveTexture() {
		if (this.getWorld().getBlockState(pos).getBlock() != BlockDefs.essenceRefiner) {
			this.invalidate();
			return;
		}
		if (worldObj.getBlockState(pos).getValue(BlockEssenceRefiner.ACTIVE) == isRefining() || worldObj.isRemote)
			return;
		if (isRefining()) {
			if (!worldObj.isRemote) {
				worldObj.setBlockState(pos, worldObj.getBlockState(pos).withProperty(BlockEssenceRefiner.ACTIVE, true),
						3);
			}
		} else {
			if (!worldObj.isRemote) {
				worldObj.setBlockState(pos, worldObj.getBlockState(pos).withProperty(BlockEssenceRefiner.ACTIVE, false),
						3);
			}
		}
	}
	...
	public List<PowerTypes> getValidPowerTypes() {
		return Lists.newArrayList(PowerTypes.NEUTRAL, PowerTypes.DARK);
	}
