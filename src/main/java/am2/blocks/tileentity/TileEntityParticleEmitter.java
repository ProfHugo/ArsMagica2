...
public class TileEntityParticleEmitter extends TileEntity implements ITickable {
	...
	public void update() {
		...
		if (!show && !worldObj.isRemote && ((forceShow && showTicks++ > 100) || !forceShow)) {
			showTicks = 0;
			forceShow = false;
			for (EntityPlayer player : worldObj.getEntitiesWithinAABB(EntityPlayer.class,
					new AxisAlignedBB(pos).expandXyz(8D))) {
				if (player != null && player.inventory.getCurrentItem() != null
						&& player.inventory.getCurrentItem().getItem() == ItemDefs.crystalWrench) {
			...
			worldObj.setBlockState(getPos(),
					worldObj.getBlockState(pos).withProperty(BlockParticleEmitter.HIDDEN, !forceShow), 3);
		}
	...
	private void doSpawn() {
		...
		AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj,
				AMParticle.particleTypes[particleType], x, y, z);
		if (particle != null) {
			particle.AddParticleController(ArsMagica2.proxy.particleManager.createDefaultParticleController(
					particleBehaviour, particle, new AMVector3(x, y, z).toVec3D(), speed, getBlockMetadata()));
			...
			if (!defaultColor) {
				if (!randomColor)
					particle.setRGBColorF(((particleColor >> 16) & 0xFF) / 255f, ((particleColor >> 8) & 0xFF) / 255f,
							(particleColor & 0xFF) / 255f);
				else
	...
	public void setShow(boolean show) {
		...
		if (worldObj.isRemote && show) {
			...
			if (localPlayer != null && localPlayer.inventory.getCurrentItem() != null
					&& localPlayer.inventory.getCurrentItem().getItem() == ItemDefs.crystalWrench) {
				AMVector3 myLoc = new AMVector3(pos);
		...
		// worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, show ?
		// oldMeta & ~0x8 : oldMeta | 0x8, 2);
	}
