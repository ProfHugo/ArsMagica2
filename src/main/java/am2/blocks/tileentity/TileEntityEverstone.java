...
public class TileEntityEverstone extends TileEntity implements ITickable {
	...
	public void setFacade(IBlockState facade) {
		...
		if (!worldObj.isRemote) {
			List<EntityPlayerMP> players = worldObj.getEntitiesWithinAABB(EntityPlayerMP.class,
					new AxisAlignedBB(pos).expand(64, 64, 64));
	...
	private void propagatePoweredByEverstone(boolean powered, ArrayList<BlockPos> completedUpdates) {
		...
		if (worldObj.isRemote) {
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "radiant",
	...
	public void update() {
		...
		worldObj.setBlockState(pos,
				worldObj.getBlockState(pos).withProperty(BlockEverstone.HAS_FACADE, getFacade() != null)
						.withProperty(BlockEverstone.IS_SOLID, isSolid()),
				2);
		...
		if (worldObj.isBlockIndirectlyGettingPowered(pos) == 0 && !poweredFromEverstone) {
			reconstructTimer--;
			if (worldObj.isRemote) { //
				// worldObj.scheduleBlockUpdateWithPriority(xCoord, yCoord,
				// zCoord, BlocksCommonProxy.everstone.blockID, 0, 0);
				worldObj.markAndNotifyBlock(pos, worldObj.getChunkFromBlockCoords(pos), worldObj.getBlockState(pos),
						worldObj.getBlockState(pos), 3);
				if (reconstructTimer < reconstructMax - 20 && reconstructTimer > 20 && worldObj.rand.nextInt(10) < 8) {
					// TODO ArsMagica2.proxy.addDigParticle(worldObj, pos,
					// getFacade() == null ? BlockDefs.everstone : getFacade(),
					// getFacadeMeta());
				}
			}
		}
