...
public class TileEntitySummoner extends TileEntityAMPower implements IInventory, IKeystoneLockable<TileEntitySummoner> {
	...
	private static final int powerPadding = 500; // extra power to charge before
													// summoning so that it can
					...
					if (ent == null && canSummon()) {
						...
						if (ent != null) {
							PowerNodeRegistry.For(this.worldObj).consumePower(this,
									PowerNodeRegistry.For(this.worldObj).getHighestPowerType(this), maintainCost);
						}
					}
				} else {
					unsummonCreature();
	...
	public boolean canSummon() {
		...
		return summonCooldown == 0
				&& PowerNodeRegistry.For(this.worldObj).checkPower(this, getSummonCost() + powerPadding);
	}
	...
	private void summonCreature() {
		...
		EntityLiving summon = ((Summon) ArsMagicaAPI.getSpellRegistry()
				.getObject(new ResourceLocation("arsmagica2:summon"))).summonCreature(inventory[SUMMON_SLOT],
						dummyCaster, dummyCaster, worldObj, pos.getX(), pos.getY() + 1, pos.getZ());
		if (summon != null) {
			...
			PowerNodeRegistry.For(this.worldObj).consumePower(this,
					PowerNodeRegistry.For(this.worldObj).getHighestPowerType(this), summonCost);
