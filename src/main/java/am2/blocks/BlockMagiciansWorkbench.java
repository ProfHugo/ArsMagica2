...
public class BlockMagiciansWorkbench extends BlockAMSpecialRenderContainer {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (te != null && te instanceof TileEntityMagiciansWorkbench) {
			...
			if (KeystoneUtilities.instance.canPlayerAccess((IKeystoneLockable<?>) te, playerIn,
					KeystoneAccessType.USE)) {
				...
				if (heldItem != null && heldItem.getItem() == ItemDefs.workbenchUpgrade) {
					...
					if (!worldIn.isRemote) {
						super.onBlockActivated(worldIn, pos, state, playerIn, hand, heldItem, side, hitX, hitY, hitZ);
						FMLNetworkHandler.openGui(playerIn, ArsMagica2.instance, IDDefs.GUI_MAGICIANS_WORKBENCH,
