...
public class BlockEssenceRefiner extends BlockAMPowered {
	...
	public BlockEssenceRefiner() {
		super(Material.WOOD);
		setHardness(2.0f);
		setDefaultState(blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(ACTIVE,
	...
	public IBlockState getStateFromMeta(int meta) {
		return getDefaultState().withProperty(FACING, EnumFacing.getHorizontal(meta & 0x3)).withProperty(ACTIVE,
				(meta & 0x4) == 0x4);
	}

	@Override
	public boolean onBlockActivated(World par1World, BlockPos pos, IBlockState state, EntityPlayer par5EntityPlayer,
			EnumHand hand, ItemStack heldItem, EnumFacing par6, float par7, float par8, float par9) {
		if (this.HandleSpecialItems(par1World, par5EntityPlayer, pos)) {
			return true;
		}
		if (!par1World.isRemote) {
			if (KeystoneUtilities.HandleKeystoneRecovery(par5EntityPlayer,
					((IKeystoneLockable<?>) par1World.getTileEntity(pos))))
				return true;
			if (KeystoneUtilities.instance.canPlayerAccess((IKeystoneLockable<?>) par1World.getTileEntity(pos),
					par5EntityPlayer, KeystoneAccessType.USE)) {
				super.onBlockActivated(par1World, pos, state, par5EntityPlayer, hand, heldItem, par6, par7, par8, par9);
				FMLNetworkHandler.openGui(par5EntityPlayer, ArsMagica2.instance, IDDefs.GUI_ESSENCE_REFINER, par1World,
