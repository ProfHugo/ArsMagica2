...
public class BlockDesertNova extends BlockAMFlower {
	...
	public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
		...
		if (blockSands == null) {// sand is defined by Forge, hence only first
									// call will be 'true'
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random par5Random) {
		...
		for (int i = 0; i < 360; i += increment) {
			...
			AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(world, "explosion_2",
					pos.getX() + 0.5, posY, pos.getZ() + 0.5);
			if (effect != null) {
				effect.setIgnoreMaxAge(true);
				effect.AddParticleController(
						new ParticleExpandingCollapsingRingAtPoint(effect, posX, posY, posZ, 0.3, 3, 0.2, 1, false)
								.setExpanding());
				// effect.AddParticleController(new ParticleOrbitPoint(effect,
				// x+0.5, y, z+0.5, 1, false).SetTargetDistance(0.35f +
				// par5Random.nextFloat() *
				// 0.2f).SetOrbitSpeed(0.5f).setIgnoreYCoordinate(true));
				// effect.AddParticleController(new ParticleFloatUpward(effect,
				// 0, 0.035f, 1, false));
				effect.AddParticleController(
						new ParticleFadeOut(effect, 2, false).setFadeSpeed(0.05f).setKillParticleOnFinish(true));
