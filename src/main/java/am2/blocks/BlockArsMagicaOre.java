...
public class BlockArsMagicaOre extends BlockAM {
	...
	public int quantityDropped(IBlockState state, int fortune, Random random) {
		if (state.getValue(ORE_TYPE) == EnumOreType.VINTEUM)
			return 1;
	...
	public IBlockState getStateFromMeta(int meta) {
		return getDefaultState().withProperty(ORE_TYPE,
				EnumOreType.values()[MathHelper.clamp_int(meta, 0, EnumOreType.values().length - 1)]);
	}
	...
	public static enum EnumOreType implements IStringSerializable {
		VINTEUM, CHIMERITE, BLUETOPAZ, MOONSTONE, SUNSTONE;
