...
public class BlockKeystoneChest extends BlockAMSpecialRenderContainer {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!worldIn.isRemote) {
			...
			/*
			 * PacketDispatcher.sendPacketToPlayer(AMCore.instance.proxy.
			 * packetSender.createArsMagicaClientPacket(AMPacketIDs.
			 * KEYSTONE_CHEST_GUI_OPEN, new
			 * AMDataWriter().add(par2).add(par3).add(par4).generate()),
			 * (Player)entityplayer);
			 */

			FMLNetworkHandler.openGui(playerIn, ArsMagica2.instance, IDDefs.GUI_KEYSTONE_CHEST, worldIn, pos.getX(),
					pos.getY(), pos.getZ());
	...
	public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player) {
		...
		if (KeystoneUtilities.instance.canPlayerAccess(receptacle, player, KeystoneAccessType.BREAK)) {
			for (int i = receptacle.getSizeInventory() - 3; i < receptacle.getSizeInventory(); i++) {
				...
				// arbitrary number, just in case rune stack sizes increase in
				// the future
				// yes, it's hard-coded; yes, it's also less computationally
				// intensive than a stack size lookup
			}
		}
	}
