...
public class BlockInlay extends BlockRailBase {
	...
	public void randomDisplayTick(IBlockState stateIn, World world, BlockPos pos, Random rand) {
		if (world.isRemote
				&& world.getBlockState(pos.down()).getBlock().isAir(world.getBlockState(pos.down()), world, pos.down())
				&& ArsMagica2.config.FullGFX() && rand.nextInt(10) < 8) {
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(world,
					ArsMagica2.config.FullGFX() ? "radiant" : "sparkle2", pos.getX() + rand.nextFloat(), pos.getY(),
	...
	private boolean checkForIceEffigy(World world, BlockPos pos) {
		if (world.getBlockState(pos.add(1, 0, 1)).getBlock() == BlockDefs.blocks.getDefaultState()
				.withProperty(BlockArsMagicaBlock.BLOCK_TYPE, EnumBlockType.BLUETOPAZ).getBlock()) {
			if (world.getBlockState(pos.add(1, 1, 1)).getBlock() == BlockDefs.blocks.getDefaultState()
					.withProperty(BlockArsMagicaBlock.BLOCK_TYPE, EnumBlockType.BLUETOPAZ).getBlock()) {
				if (world.getBlockState(pos.add(1, 2, 1)).getBlock() == BlockDefs.iceEffigy) {
					...
					if (iceMeta > 0) {
						ArsMagica2.proxy.particleManager.RibbonFromPointToPoint(world, pos.getX() + 1.5, pos.getY() + 2,
								pos.getZ() + 1.5, pos.getX() + 2, pos.getY() + 3, pos.getZ() + 2);
					}
					if (iceMeta >= 3) {
						BossSpawnHelper.instance.onIceEffigyBuilt(world, pos.add(1, 0, 1));
					} else {
						List<EntitySnowman> snowmen = world.getEntitiesWithinAABB(EntitySnowman.class,
								new AxisAlignedBB(pos.getX() - 9, pos.getY() - 2, pos.getZ() - 9, pos.getX() + 10,
										pos.getY() + 4, pos.getZ() + 10));
						if (snowmen.size() > 0) {
							...
							ArsMagica2.proxy.particleManager.BeamFromEntityToPoint(world, snowmen.get(0),
		...
		if (world.getBlockState(pos.add(1, 0, 1)).getBlock() == BlockDefs.manaBattery) {
			if (world.getBlockState(pos.add(1, 1, 1)).getBlock() == Blocks.IRON_BARS) {
				if (world.getBlockState(pos.add(1, 2, 1)).getBlock() == BlockDefs.lightningEffigy) {
					...
					if (!world.isRemote) {
						for (int i = 0; i < 5; ++i)
							ArsMagica2.proxy.particleManager.BoltFromPointToPoint(world, pos.getX() + 1.5,
									pos.getY() + 30, pos.getZ() + 1.5, pos.getX() + 1.5, pos.getY() + 2,
									pos.getZ() + 1.5);
						for (int i = 0; i < fenceMeta; ++i)
							world.playSound(pos.getX(), pos.getY(), pos.getZ(), SoundEvents.ENTITY_LIGHTNING_THUNDER,
									SoundCategory.AMBIENT, 1.0f, world.rand.nextFloat() + 0.5f, false);
					}
					if (fenceMeta >= 3) {
	...
	public void onMinecartPass(World world, EntityMinecart cart, BlockPos pos) {
		...
		if (this.material == TYPE_REDSTONE) {
			...
			EnumRailDirection teleportMeta = teleportLocation != null
					? world.getBlockState(teleportLocation.toBlockPos()).getValue(SHAPE) : null;
			long time = System.currentTimeMillis();
			boolean cooldownPassed = (time - millisSinceLastTeleport) > 5000;
			if (teleportLocation != null
					&& (teleportMeta == EnumRailDirection.EAST_WEST || teleportMeta == EnumRailDirection.NORTH_SOUTH)
					&& cooldownPassed) {
				world.playSound(teleportLocation.x, teleportLocation.y, teleportLocation.z,
						SoundEvents.ENTITY_ENDERMEN_TELEPORT, SoundCategory.BLOCKS, 1.0F, 1.0F, false);
				world.playSound(cart.posX, cart.posY, cart.posZ, SoundEvents.ENTITY_ENDERMEN_TELEPORT,
