...
public class BlockManaBattery extends BlockAMPowered {
		...
		if (!worldIn.isRemote) {
			...
			if (te != null) {
				if (ArsMagica2.config.colourblindMode()) {
					playerIn.addChatMessage(new TextComponentString(String.format("Charge Level: %.2f %% [%s]",
							PowerNodeRegistry.For(worldIn).getPower(te, te.getPowerType()) / te.getCapacity() * 100,
							getColorNameFromPowerType(te.getPowerType()))));
				} else {
					playerIn.addChatMessage(new TextComponentString(String.format("Charge Level: %s%.2f \u00A7f%%",
							te.getPowerType().getChatColor(),
							PowerNodeRegistry.For(worldIn).getPower(te, te.getPowerType()) / te.getCapacity() * 100)));
				}
			}
		}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer,
		...
		if (stack != null) {
			...
			if (stack.getTagCompound() != null) {
				if (stack.getTagCompound().hasKey("mana_battery_charge")
						&& stack.getTagCompound().hasKey("mana_battery_powertype"))
					PowerNodeRegistry.For(worldIn).setPower(te,
							PowerTypes.getByID(stack.getTagCompound().getInteger("mana_battery_powertype")),
							stack.getTagCompound().getFloat("mana_battery_charge"));
				else
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		if (te instanceof TileEntityManaBattery && stack != null) {
			if (PowerNodeRegistry.For((World) world).getPower((TileEntityManaBattery) te,
					((TileEntityManaBattery) te).getPowerType()) != 0.0F) {
				stack.setTagCompound(new NBTTagCompound());
				stack.getTagCompound().setFloat("mana_battery_charge", PowerNodeRegistry.For((World) world)
						.getPower((TileEntityManaBattery) te, ((TileEntityManaBattery) te).getPowerType()));
				stack.getTagCompound().setInteger("mana_battery_powertype",
						((TileEntityManaBattery) te).getPowerType().ID());
			}
		}
		return drops;
	}
	...
	public int getComparatorInputOverride(IBlockState blockState, World worldIn, BlockPos pos) {
		...
		// the only time they have more, is when they are at zero, but then it
		// doesn't matter
	...
	// @Override
	// public int colorMultiplier(IBlockAccess blockAccess, int x, int y, int
	// z){
