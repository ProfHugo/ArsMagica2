...
/**
 * This is my invisible utility block. I use it for illumination (meta 0-2), as
 * well as invisible walls (meta 3-10). Meta 0: low illuminated Meta 1: med
 * illuminated Meta 2: high illuminated Meta 3: only collidable from +x Meta 4:
 * only collidable from -x Meta 5: only collidable from +z Meta 6: only
 * collidable from -z Meta 7: only collidable from +/- x Meta 8: only collidable
 * from +/- z Meta 9: fully collidable Meta 10: special illuminated (light
 * sigil)
 *
 * @author Mithion
 */
public class BlockInvisibleUtility extends BlockAM {
		...
		if (meta.type == EnumType.COLLISION) {
			...
			case COLLISION_POSITIVE_X: // +x
				if (entity.posX > pos.getX() + distanceThreshold) {
					collidingBoxes.add(new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1.25,
							pos.getY() + 1.6, pos.getZ() + 1.25));
					isCollided = true;
				}
				break;
	...
	private void spawnBlockParticles(World world, BlockPos pos) {
		AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(world, "symbols", pos.getX() + 0.5,
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
		if (getType(state).type == EnumType.LIGHT) {
			...
			List<EntityLivingBase> ents = world.getEntitiesWithinAABB(EntityLivingBase.class,
					new AxisAlignedBB(pos).expandXyz(r));
			boolean buffNearby = false;
			for (EntityLivingBase ent : ents) {
				buffNearby |= ent.isPotionActive(PotionEffectsDefs.illumination)
						|| (ent instanceof EntityPlayer && ((EntityPlayer) ent).inventory.getCurrentItem() != null
	...
	public static enum EnumInvisibleType implements IStringSerializable {
		LOW_ILLUMINATED(EnumType.LIGHT), MED_ILLUMINATED(EnumType.LIGHT), HIGH_ILLUMINATED(
				EnumType.LIGHT), COLLISION_POSITIVE_X(EnumType.COLLISION), COLLISION_NEGATIVE_X(
						EnumType.COLLISION), COLLISION_POSITIVE_Z(EnumType.COLLISION), COLLISION_NEGATIVE_Z(
								EnumType.COLLISION), COLLISION_ALL_X(EnumType.COLLISION), COLLISION_ALL_Z(
										EnumType.COLLISION), COLLISION_ALL(
												EnumType.COLLISION), SPECIAL_ILLUMINATED(EnumType.LIGHT);
