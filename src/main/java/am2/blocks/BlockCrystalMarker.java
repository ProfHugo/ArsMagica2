...
public class BlockCrystalMarker extends BlockAMContainer {
	...
	public static final String[] crystalMarkerTypes = { "cm_import", "cm_export", "cm_likeExport", "cm_setExport",
			"cm_regulateExport", "cm_regulateMulti", "cm_setImport", "cm_final", "cm_spellExport" };
		...
		if (player.getHeldItemMainhand() != null && player.getHeldItemMainhand().getItem() == ItemDefs.crystalWrench) {
			...
			if (crystalMarkerTE != null) {
				...
				if (elementalAttunerVector != null) {
					// if we are here, there is an attached attuner to this
					// marker. Resolve the location into the tile entity.
					elementalAttunerTE = GetElementalAttunerTileEntity(world, elementalAttunerVector.toBlockPos());
					if (elementalAttunerTE != null) {
						// Notify the attuner that this crystal now has a new
						// priority
						elementalAttunerTE.switchMarkerPriority(new AMVector3(pos), oldPriority,
								crystalMarkerTE.getPriority());
					}
				}

				// finally, notify the player of the new priority
				player.addChatMessage(new TextComponentString(String.format(I18n.translateToLocal("am2.tooltip.priSet"),
						String.format("%d", crystalMarkerTE.getPriority()) // need
																			// to
																			// put
																			// this
																			// as
			...
			return false;
		} else if (operandType == META_SET_EXPORT || operandType == META_REGULATE_EXPORT
				|| operandType == META_REGULATE_MULTI || operandType == META_SET_IMPORT) {
			FMLNetworkHandler.openGui(player, ArsMagica2.instance, IDDefs.GUI_CRYSTAL_MARKER, world, pos.getX(),
					pos.getY(), pos.getZ());
			return true;
		}
	...
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
		...
		case UP: // Bottom, Inventory is above
			return new AxisAlignedBB(0.35f, (float) ((1 + cm.GetConnectedBoundingBox().minY)) - 0.1f, 0.32f, 0.65f,
					(float) ((1 + cm.GetConnectedBoundingBox().minY)), 0.68f);
		case DOWN: // Top, Inventory is below
			return new AxisAlignedBB(0.35f, (float) (-1 * (1 - cm.GetConnectedBoundingBox().maxY)), 0.3f, 0.65f,
					(float) (-1 * (1 - cm.GetConnectedBoundingBox().maxY)) + 0.1f, 0.7f);
		case SOUTH: // North, Inventory is to the south
			return new AxisAlignedBB(0.35f, 0.32f, (float) (1 + (1 - cm.GetConnectedBoundingBox().maxZ)) - 0.1f, 0.65f,
					0.68f, (float) (1 + (1 - cm.GetConnectedBoundingBox().maxZ)));
		case NORTH: // South, Inventory is to the north
			return new AxisAlignedBB(0.35f, 0.32f, (float) (0 - cm.GetConnectedBoundingBox().minZ), 0.65f, 0.68f,
					(float) (0 - cm.GetConnectedBoundingBox().minZ) + 0.1f);
		case EAST: // West, Inventory is to the east
			return new AxisAlignedBB((float) (1 + cm.GetConnectedBoundingBox().minX) - 0.1f, 0.32f, 0.35f,
					(float) (1 + cm.GetConnectedBoundingBox().minX), 0.68f, 0.65f);
		case WEST: // East, Inventory is to the west
			return new AxisAlignedBB((float) (-1 * (1 - cm.GetConnectedBoundingBox().maxX)), 0.32f, 0.35f,
					(float) (-1 * (1 - cm.GetConnectedBoundingBox().maxX)) + 0.1f, 0.68f, 0.65f);
		}
		return null;
	}
	...
	public static boolean isOutputMarker(int operandType) {
		return operandType == META_OUT || operandType == META_LIKE_EXPORT || operandType == META_SET_EXPORT
				|| operandType == META_REGULATE_EXPORT || operandType == META_REGULATE_MULTI
				|| operandType == META_FINAL_DEST || operandType == META_SPELL_EXPORT;
	}

	public static boolean isInputMarker(int operandType) {
		return operandType == META_IN || operandType == META_REGULATE_MULTI || operandType == META_SET_IMPORT
