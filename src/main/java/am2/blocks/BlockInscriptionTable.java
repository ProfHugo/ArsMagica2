...
public class BlockInscriptionTable extends BlockAMSpecialRenderContainer {
	...
	public BlockInscriptionTable() {
		...
		setDefaultState(blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(LEFT, false)
				.withProperty(TIER_1, false).withProperty(TIER_2, false).withProperty(TIER_3, false));
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (te != null) {
			if (!state.getValue(LEFT)) {
				...
				tealt = (TileEntityInscriptionTable) worldIn
						.getTileEntity(pos.offset(state.getValue(FACING).getOpposite()));
			}
		}
		...
		FMLNetworkHandler.openGui(playerIn, ArsMagica2.instance, IDDefs.GUI_INSCRIPTION_TABLE, worldIn, pos.getX(),
				pos.getY(), pos.getZ());
	...
	public IBlockState getStateFromMeta(int meta) {
		return getDefaultState().withProperty(FACING, EnumFacing.values()[2 + (meta % 4)]).withProperty(LEFT,
				(meta & 0x8) == 0x8);
	}
