...
public class TileEssenceConduitRenderer extends TileEntitySpecialRenderer<TileEntityEssenceConduit> {
	...
	public TileEssenceConduitRenderer() {
		...
		// unPowered = new ResourceLocation("arsmagica2",
		// "textures/blocks/custom/essenceConduitUnpowered.png");
	}
	...
	public void renderTileEntityAt(TileEntityEssenceConduit tile, double d, double d1, double d2, float f,
			int destroyStage) {
		if (tile.getWorld() == null)
			return;
		if (tile.getWorld().getBlockState(tile.getPos()).getBlock() != BlockDefs.essenceConduit)
			return;
		createModel();
		...
		GL11.glScalef(1.0F, -1F, -1F); // if you read this, comment out this
		...
		model.renderModel(tile.getRotationX(), tile.getRotationY(), tile.getRotationZ(), 0, 0.0625F); // renders
																										// and
																										// yes
																										// 0.0625
																										// is
		...
		Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelRenderer().renderModel(getWorld(),
				bakedModel, getWorld().getBlockState(tile.getPos()), tile.getPos(), tesselator.getBuffer(), false);
		tesselator.draw();
