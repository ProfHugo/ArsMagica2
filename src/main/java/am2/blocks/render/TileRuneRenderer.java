...
public class TileRuneRenderer extends TileEntitySpecialRenderer<TileEntityGroundRuneSpell> {
	...
	@Override
	public void renderTileEntityAt(TileEntityGroundRuneSpell te, double x, double y, double z, float partialTicks,
			int destroyStage) {
		if (te.getSpellStack() != null) {
			Affinity mainAff = AffinityShiftUtils.getMainShiftForStack(te.getSpellStack());
			TextureAtlasSprite sprite = Minecraft.getMinecraft().getTextureMapBlocks()
					.getAtlasSprite(new ResourceLocation(mainAff.getRegistryName().getResourceDomain(),
							"blocks/runes/rune_" + mainAff.getRegistryName().getResourcePath()).toString());
