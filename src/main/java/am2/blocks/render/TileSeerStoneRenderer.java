...
public class TileSeerStoneRenderer extends TileEntitySpecialRenderer<TileEntitySeerStone> {
	...
	public void renderTileEntityAt(TileEntitySeerStone tile, double d, double d1, double d2, float f,
			int destroyStage) {
		...
		case UP:
			...
			GlStateManager.rotate(180, 1.0F, 0.0F, 0.0F); // rotate based on
															// metadata
			break;
		case DOWN:
		...
		case SOUTH:
			GlStateManager.translate((float) d + 0.5f, (float) d1 + 0.5F, (float) d2 - 0.5F); // size
			GlStateManager.rotate(270, 1.0F, 0.0F, 0.0F); // rotate based on
															// metadata
			break;
		case NORTH:
			GlStateManager.translate((float) d + 0.5f, (float) d1 + 0.5F, (float) d2 + 1.5F); // size
			GlStateManager.rotate(90, 1.0F, 0.0F, 0.0F); // rotate based on
															// metadata
			break;
		case EAST:
			GlStateManager.translate((float) d - 0.5f, (float) d1 + 0.5F, (float) d2 + 0.5F); // size
			GlStateManager.rotate(90, 0.0F, 0.0F, 1.0F); // rotate based on
															// metadata
			break;
		case WEST:
			GlStateManager.translate((float) d + 1.5F, (float) d1 + 0.5F, (float) d2 + 0.5F); // size
			GlStateManager.rotate(270, 0.0F, 0.0F, 1.0F); // rotate based on
															// metadata
			break;
		}

		GlStateManager.scale(1.0F, -1F, -1F); // if you read this comment out
	...
	private void renderArsMagicaEffect(Tessellator tessellator, int i, EnumFacing meta) {
		...
		// tessellator.getBuffer().putBrightness4(15728864, 15728864, 15728864,
		// 15728864);
