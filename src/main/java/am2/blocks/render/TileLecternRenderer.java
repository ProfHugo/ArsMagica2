...
public class TileLecternRenderer extends TileEntitySpecialRenderer<TileEntityLectern> {
	RenderEntityItem renderItem = new RenderEntityItem(Minecraft.getMinecraft().getRenderManager(),
	...
	public void renderTileEntityArchmagePodiumAt(TileEntityLectern podium, double x, double y, double z, float f1)
			throws Exception {
		...
		Minecraft.getMinecraft().renderEngine
				.bindTexture(new ResourceLocation("arsmagica2", "textures/blocks/custom/archmagePodium.png"));
	...
	private void renderHelperIcon(TileEntityLectern podium, double x, double y, double z, float f) {
		...
		// try {
		// renderItem.doRender(AMGuiHelper.instance.dummyItem, x + 0.5f, y +
		// 1.4f, z + 0.5f, AMGuiHelper.instance.dummyItem.rotationYaw, f);
	...
	private void renderRadiant(Tessellator tessellator, float partialFrame, TileEntityLectern podium) {
		// RenderHelper.disableStandardItemLighting();
		// float var4 = (podium.particleAge + partialFrame) /
		// podium.particleMaxAge;
		// float var5 = 0.0F;
		//
		// if (var4 > 0.8F){
		// var5 = (var4 - 0.8F) / 0.2F;
	...
	private void RenderBook(TileEntityLectern podium, double x, double y, double z, float partialTicks, int meta) {
		...
		// float var14 = podium.bookSpreadPrev + (podium.bookSpread -
		// podium.bookSpreadPrev) * partialTicks;
	...
	@Override
	public void renderTileEntityAt(TileEntityLectern te, double x, double y, double z, float partialTicks,
			int destroyStage) {
		GlStateManager.pushMatrix();
		try {
	...
	/*
	 * GL11.glPushMatrix(); GL11.glTranslatef((float)x + 0.5F, (float)y + 0.75F,
	 * (float)z + 0.5F); float f1 = (float)podium.field_145926_a + partialTicks;
	 * GL11.glTranslatef(0.0F, 0.1F + MathHelper.sin(f1 * 0.1F) * 0.01F, 0.0F);
	 * float f2;
	 * 
	 * for (f2 = podium.field_145928_o - podium.field_145925_p; f2 >=
	 * (float)Math.PI; f2 -= ((float)Math.PI * 2F)) { ; }
	 * 
	 * while (f2 < -(float)Math.PI) { f2 += ((float)Math.PI * 2F); }
	 * 
	 * float f3 = podium.field_145925_p + f2 * partialTicks; GL11.glRotatef(-f3
	 * * 180.0F / (float)Math.PI, 0.0F, 1.0F, 0.0F); GL11.glRotatef(80.0F, 0.0F,
	 * 0.0F, 1.0F); this.bindTexture(book); float f4 = podium.field_145931_j +
	 * (podium.field_145933_i - podium.field_145931_j) * partialTicks + 0.25F;
	 * float f5 = podium.field_145931_j + (podium.field_145933_i -
	 * podium.field_145931_j) * partialTicks + 0.75F; f4 = (f4 -
	 * (float)MathHelper.truncateDoubleToInt((double)f4)) * 1.6F - 0.3F; f5 =
	 * (f5 - (float)MathHelper.truncateDoubleToInt((double)f5)) * 1.6F - 0.3F;
	 * 
	 * if (f4 < 0.0F) { f4 = 0.0F; }
	 * 
	 * if (f5 < 0.0F) { f5 = 0.0F; }
	 ...
	 * if (f5 > 1.0F) { f5 = 1.0F; }
	 * 
	 * float f6 = podium.field_145927_n + (podium.field_145930_m -
	 * podium.field_145927_n) * partialTicks; GL11.glEnable(GL11.GL_CULL_FACE);
