...
public class TileEverstoneRenderer extends TileEntitySpecialRenderer<TileEntityEverstone> {
	...
	@Override
	public void renderTileEntityAt(TileEntityEverstone te, double x, double y, double z, float partialTicks,
			int destroyStage) {
		...
		if (te.isSolid()) {
			if (te.getFacade() != null) {
				...
				Minecraft.getMinecraft().getBlockRendererDispatcher().renderBlock(te.getFacade(), te.getPos(),
				...
				if (destroyStage >= 0) {
					...
					IBakedModel ibakedmodel = Minecraft.getMinecraft().getBlockRendererDispatcher()
							.getBlockModelShapes().getModelForState(te.getFacade());
					IBakedModel ibakedmodel1 = net.minecraftforge.client.ForgeHooksClient.getDamageModel(ibakedmodel,
							sprite, te.getFacade(), te.getWorld(), te.getPos());
					Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelRenderer().renderModel(
							te.getWorld(), ibakedmodel1, te.getFacade(), te.getPos(),
