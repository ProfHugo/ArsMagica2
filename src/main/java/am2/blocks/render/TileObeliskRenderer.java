...
public class TileObeliskRenderer extends TileEntitySpecialRenderer<TileEntityObelisk> {
	...
	@Override
	public void renderTileEntityAt(TileEntityObelisk te, double x, double y, double z, float partialTicks,
			int destroyStage) {
		if (!te.getWorld().isBlockLoaded(te.getPos(), false)
				|| te.getWorld().getBlockState(te.getPos()).getBlock() != BlockDefs.obelisk)
			return;
		...
		if (te.hasWorldObj())
			Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelRenderer().renderModel(te.getWorld(),
					getBakedModel(te), te.getWorld().getBlockState(te.getPos()), te.getPos(), tessellator.getBuffer(),
					false);
		else
			Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelRenderer().renderModel(
					Minecraft.getMinecraft().theWorld, getBakedModel(te), BlockDefs.obelisk.getDefaultState(),
					new BlockPos(0, 0, 0), tessellator.getBuffer(), false);
		tessellator.draw();
		if (te.isActive()) {
			...
			TextureAtlasSprite sprite = Minecraft.getMinecraft().getTextureMapBlocks()
					.getAtlasSprite("arsmagica2:blocks/custom/obelisk_runes");
			GlStateManager.scale(1 / (sprite.getMaxU() - sprite.getMinU()), 1 / (sprite.getMaxV() - sprite.getMinV()),
					1);
			Minecraft.getMinecraft().renderEngine
					.bindTexture(new ResourceLocation("arsmagica2:textures/blocks/custom/obelisk_runes.png"));
			tessellator.getBuffer().begin(GL11.GL_QUADS, DefaultVertexFormats.BLOCK);
			...
			Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelRenderer().renderModel(te.getWorld(),
					runesBakedModel, te.getWorld().getBlockState(te.getPos()), te.getPos(), tessellator.getBuffer(),
					false);
			tessellator.draw();
			GL11.glPopMatrix();
