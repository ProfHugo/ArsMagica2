...
public class TileOtherworldAuraRenderer extends TileEntitySpecialRenderer<TileEntityOtherworldAura> {
	...
	public void renderTileEntityAt(TileEntityOtherworldAura tile, double d1, double d2, double d3, float f,
			int destroyStage) {
	...
	private void renderArsMagicaEffect(Tessellator tessellator, float offset, float scale, float smooth,
			boolean rotate) {
		GlStateManager.pushMatrix();
		if (rotate) {
		...
		GlStateManager.rotate((Minecraft.getMinecraft().thePlayer.ticksExisted + smooth) * (scale * 2) * offset, 0, 0,
				1);
