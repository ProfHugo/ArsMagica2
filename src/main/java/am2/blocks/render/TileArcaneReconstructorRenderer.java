...
public class TileArcaneReconstructorRenderer extends TileEntitySpecialRenderer<TileEntityArcaneReconstructor> {
	...
	private void bake() {
		try {
			mainModel = ModelLoaderRegistry
					.getModel(new ResourceLocation("arsmagica2", "block/reconstructor/main.obj"));
			ring1Model = ModelLoaderRegistry
					.getModel(new ResourceLocation("arsmagica2", "block/reconstructor/ring1.obj"));
			ring2Model = ModelLoaderRegistry
					.getModel(new ResourceLocation("arsmagica2", "block/reconstructor/ring2.obj"));
			ring3Model = ModelLoaderRegistry
					.getModel(new ResourceLocation("arsmagica2", "block/reconstructor/ring3.obj"));
		} catch (Exception e) {
	...
	@Override
	public void renderTileEntityAt(TileEntityArcaneReconstructor te, double x, double y, double z, float partialTicks,
			int destructionStage) {
		if (renderItem == null)
			renderItem = new RenderItemNoBob(Minecraft.getMinecraft().getRenderManager());
		bake();
		float floatingOffset = te.getOffset();// (float)
												// (Math.sin(te.getOffset()) *
												// (Math.PI/ 180F) * 1.4f);
		...
		if (te.shouldRenderRotateOffset()) {
			...
			RenderRotatedModelGroup(te, ring3BakedModel, te.getInnerRingRotation().copy()
					.sub(te.getInnerRingRotationSpeed().copy().scale(te.getRotateOffset())));
			RenderRotatedModelGroup(te, ring1BakedModel, te.getMiddleRingRotation().copy()
					.sub(te.getMiddleRingRotationSpeed().copy().scale(te.getRotateOffset())));
			RenderRotatedModelGroup(te, ring2BakedModel, te.getOuterRingRotation().copy()
					.sub(te.getOuterRingRotationSpeed().copy().scale(te.getRotateOffset())));

			GL11.glColor4f(1.0f, 1.0f, 1.0f, 0.15f);

			RenderRotatedModelGroup(te, ring3BakedModel, te.getInnerRingRotation().copy()
					.sub(te.getInnerRingRotationSpeed().copy().scale(te.getRotateOffset() * 2)));
			RenderRotatedModelGroup(te, ring1BakedModel, te.getMiddleRingRotation().copy()
					.sub(te.getMiddleRingRotationSpeed().copy().scale(te.getRotateOffset() * 2)));
			RenderRotatedModelGroup(te, ring2BakedModel, te.getOuterRingRotation().copy()
					.sub(te.getOuterRingRotationSpeed().copy().scale(te.getRotateOffset() * 2)));
		}
	...
	private void RenderItemAtCoords(ItemStack item, double x, double y, double z, float partialTick) {
		...
		renderItem.doRender(AMGuiHelper.instance.dummyItem, x, y, z, AMGuiHelper.instance.dummyItem.rotationYaw,
				partialTick);
	}
}
