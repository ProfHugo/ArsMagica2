...
public abstract class BlockWitchwoodSlabs extends BlockSlab {
	...
	public BlockWitchwoodSlabs() {
		...
		setDefaultState(isDouble() ? blockState.getBaseState().withProperty(SLAB_TYPE, EnumSlabType.WITCHWOOD)
				: blockState.getBaseState().withProperty(SLAB_TYPE, EnumSlabType.WITCHWOOD).withProperty(HALF,
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		drops.add(
				new ItemStack(BlockDefs.witchwoodSingleSlab, isDouble() ? 2 : 1, state.getValue(SLAB_TYPE).ordinal()));
		return drops;
	}
