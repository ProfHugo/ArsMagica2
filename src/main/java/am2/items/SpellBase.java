...
public class SpellBase extends ItemSpellBase {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List<String> list, boolean par4) {
		...
		manaCost *= 1F + (float) ((float) EntityExtension.For(player).getCurrentBurnout()
				/ (float) EntityExtension.For(player).getMaxBurnout());
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack stack, World world, EntityPlayer caster, EnumHand hand) {
		...
		if (!stack.hasDisplayName()) {
			if (!world.isRemote)
				FMLNetworkHandler.openGui(caster, ArsMagica2.instance, IDDefs.GUI_SPELL_CUSTOMIZATION, world,
						(int) caster.posX, (int) caster.posY, (int) caster.posZ);
		} else {
	...
	public void onPlayerStoppedUsing(ItemStack stack, World world, EntityLivingBase player, int timeLeft) {
		...
		if (shape != null) {
			if (!shape.isChanneled())
				SpellUtils.applyStackStage(stack, player, null, player.posX, player.posY, player.posZ, EnumFacing.UP,
						world, true, true, 0);
			if (world.isRemote && shape.isChanneled()) {
				// SoundHelper.instance.stopSound(shape.getSoundForAffinity(SpellUtils.instance.mainAffinityFor(stack),
				// stack, null));
			}
		}
	}
	...
	public void onUsingTick(ItemStack stack, EntityLivingBase caster, int count) {
		SpellShape shape = SpellUtils.getShapeForStage(stack, 0);
		if (shape.isChanneled())
			SpellUtils.applyStackStage(stack, caster, null, caster.posX, caster.posY, caster.posZ, EnumFacing.UP,
					caster.worldObj, true, true, count - 1);
		super.onUsingTick(stack, caster, count);
	}

	@Override
	public RayTraceResult getMovingObjectPosition(EntityLivingBase caster, World world, double range,
		...
		if (entityPos != null && mop != null) {
			if (mop.hitVec.distanceTo(new RayTraceResult(caster).hitVec) < entityPos.hitVec
					.distanceTo(new RayTraceResult(caster).hitVec)) {
				return mop;
			} else {
				return entityPos;
			}
	...
	public int getHarvestLevel(ItemStack stack, String toolClass, @Nullable EntityPlayer player,
		...
		return SpellUtils.getModifiedInt_Add(2, stack, (EntityLivingBase) player, (EntityLivingBase) player,
				player.getEntityWorld(), SpellModifiers.MINING_POWER);
	}
}
