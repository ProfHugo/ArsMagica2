		...
		if (EntityExtension.For(player).getCurrentLevel() > 0) {
			...
			if (player.worldObj.isRemote) {
				player.addChatMessage(
						new TextComponentString(I18n.translateToLocal("am2.tooltip.infOrb" + type.toString())));
			}
			...
			if (player.worldObj.isRemote) {
				...
				player.addChatMessage(
						new TextComponentString(I18n.translateToLocal("am2.tooltip.infOrbFail" + message)));
			}
		}
		return stack;
	}
