...
public class ItemCandle extends ItemArsMagica {
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos,
		...
		if (!stack.hasTagCompound() || !stack.getTagCompound().hasKey("search_block")) {
			...
			if (playerIn.isSneaking() && block != null && block.getBlockHardness(worldIn, pos) > 0f
					&& worldIn.getTileEntity(pos) == null) {
				if (!worldIn.isRemote) {
					setSearchBlock(block, stack);
					worldIn.setBlockToAir(pos);
				} else {
					AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldIn, "radiant",
	...
	// world, int x, int y, int z, int side, float hitX, float hitY, float
	// hitZ){
	//
	// if (!stack.hasTagCompound() ||
	// !stack.getTagCompound().hasKey("search_block")){
	// Block block = world.getBlock(x, y, z);
	// if (player.isSneaking() && block != null && block.getBlockHardness(world,
	// x, y, z) > 0f && world.getTileEntity(x, y, z) == null){
	// if (!world.isRemote){
	// setSearchBlock(world.getBlock(x, y, z), world.getBlockMetadata(x, y, z),
	// stack);
	// world.setBlockToAir(x, y, z);
	// }else{
	// AMParticle particle =
	// (AMParticle)AMCore.proxy.particleManager.spawn(world, "radiant", x + 0.5,
	// y + 0.5, z + 0.5);
	// if (particle != null){
	// particle.AddParticleController(new ParticleHoldPosition(particle, 20, 1,
	// false));
	// particle.setRGBColorF(0, 0.5f, 1);
	// }
	// }
	...
	// ChatComponentText(I18n.translateToLocal("am2.tooltip.candlecantplace")));
	// return false;
	// }
	//
	// switch (side){
		...
		for (int i = -radius; i <= radius; ++i) {
			for (int j = -radius; j <= radius; ++j) {
				for (int k = -radius; k <= radius; ++k) {
					...
					if (state == f_block) {
						if (dist <= immediate_radius) {// &&
														// player.getCurrentArmor(3)
														// != null &&
														// ArmorHelper.isInfusionPreset(player.getCurrentArmor(3),
														// GenericImbuement.pinpointOres)){
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int indexInInventory, boolean isCurrentlyHeld) {
		if (entity instanceof EntityPlayer) {
			...
			if (isCurrentlyHeld && !world.isRemote && ArsMagica2.config.candlesAreRovingLights()
					&& world.isAirBlock(player.getPosition())
					&& world.getLightFor(EnumSkyBlock.BLOCK, player.getPosition()) < 14) {
				world.setBlockState(player.getPosition(), BlockDefs.invisibleUtility.getDefaultState().withProperty(
						BlockInvisibleUtility.TYPE, BlockInvisibleUtility.EnumInvisibleType.HIGH_ILLUMINATED), 2);
			}
			if (isCurrentlyHeld) {
				if (!world.isRemote && stack.hasTagCompound() && stack.getItemDamage() % 40 == 0) {
					search(player, stack, world, player.getPosition(),
							Block.getStateById(stack.getTagCompound().getInteger("search_block")));
				}

				if (!player.capabilities.isCreativeMode)
					stack.damageItem(1, player);
				if (!world.isRemote && stack.getItemDamage() >= this.getMaxDamage())
					player.setItemStackToSlot((player.getHeldItemOffhand() == stack ? EntityEquipmentSlot.OFFHAND
							: EntityEquipmentSlot.MAINHAND), null);
			}
		}
	}
	...
	public String getItemStackDisplayName(ItemStack stack) {
		...
		if (stack.hasTagCompound() && stack.getTagCompound().hasKey("search_block")) {
			...
			if (tempItem == null) {
				name += " (" + stack.getTagCompound().getInteger("search_block") + ":"
	...
	public boolean shouldCauseReequipAnimation(ItemStack oldStack, ItemStack newStack, boolean slotChanged) {
		...
		if (newStack.getTagCompound() == null)
			return slotChanged;
		if (oldStack.getTagCompound().equals(newStack.getTagCompound()))
			return false;
		return slotChanged;
	}
