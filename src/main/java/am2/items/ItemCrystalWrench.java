...
public class ItemCrystalWrench extends ItemArsMagicaRotated {
	...
	public EnumActionResult onItemUseFirst(ItemStack stack, EntityPlayer player, World world, BlockPos pos,
		...
		if (te != null && !(te instanceof IPowerNode || te instanceof TileEntityParticleEmitter)
				&& cMode == MODE_DISCONNECT) {
		...
		if (te != null && te instanceof IPowerNode) {
			if (cMode == MODE_DISCONNECT) {
				doDisconnect((IPowerNode<?>) te, world, pos.getX() + hitX, pos.getY() + hitY, pos.getZ() + hitZ,
						player);
				return EnumActionResult.FAIL;
			}
		...
		} else if (te != null && te instanceof TileEntityCrystalMarker && stack.getTagCompound() != null
				&& stack.getTagCompound().hasKey(HAB_PAIRLOC)) {
	...
	private void handleCMPair(ItemStack stack, World world, EntityPlayer player, TileEntity te, double hitX,
	...
	private void storePairLocation(World world, TileEntity te, ItemStack stack, EntityPlayer player, double hitX,
			double hitY, double hitZ) {
	...
	private void doPairNodes(World world, BlockPos pos, ItemStack stack, EntityPlayer player, double hitX, double hitY,
			double hitZ, TileEntity te) {
		...
		if (sourceTE != null && sourceTE instanceof IPowerNode && !world.isRemote) {
			player.addChatMessage(new TextComponentString(
					PowerNodeRegistry.For(world).tryPairNodes((IPowerNode<?>) sourceTE, (IPowerNode<?>) te)));
	...
	private void doDisconnect(IPowerNode<?> node, World world, double hitX, double hitY, double hitZ,
			EntityPlayer player) {
	...
	private void spawnLinkParticles(World world, double hitX, double hitY, double hitZ, boolean disconnect) {
		for (int i = 0; i < 10; ++i) {
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(world, "none_hand", hitX, hitY,
					hitZ);
			if (particle != null) {
				if (disconnect) {
				...
				particle.AddParticleController(new ParticleMoveOnHeading(particle, world.rand.nextInt(360),
						world.rand.nextInt(360), world.rand.nextDouble() * 0.2, 1, false));
