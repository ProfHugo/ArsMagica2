...
public class ItemSpellBook extends ItemArsMagica {
	...
	// private final String[] npc_textureFiles = {"affinity_tome_general",
	// "affinity_tome_ice", "affinity_tome_life", "affinity_tome_fire",
	// "affinity_tome_lightning", "affinity_tome_ender"};
	// private final String[] player_textureFiles = {"spell_book_cover",
	// "spell_book_decoration"};
	...
	public String getItemStackDisplayName(ItemStack par1ItemStack) {
		...
		if (activeSpell != null) {
			return String.format("\2477%s (" + activeSpell.getDisplayName() + "\2477)",
					I18n.translateToLocal("item.arsmagica2:spellbook.name"));
		}
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn,
		...
		if (playerIn.isSneaking()) {
			FMLNetworkHandler.openGui(playerIn, ArsMagica2.instance, IDDefs.GUI_SPELL_BOOK, worldIn,
	...
	public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft) {
		if (entityLiving.isSneaking()) {
			FMLNetworkHandler.openGui((EntityPlayer) entityLiving, ArsMagica2.instance, IDDefs.GUI_SPELL_BOOK, worldIn,
					(int) entityLiving.posX, (int) entityLiving.posY, (int) entityLiving.posZ);
		} else {
	...
	public ItemStack[] ReadFromStackTagCompound(ItemStack itemStack) {
		...
		/*
		 * for (int i = 0; i < items.length; ++i){ if
		 * (!itemStack.stackTagCompound.hasKey("spellbookitem" + i) ||
		 * itemStack.stackTagCompound.getInteger("spellbookitem" + i) == -1){
		 * items[i] = null; continue; } int id =
		 * itemStack.stackTagCompound.getInteger("spellbookitem" + i); int meta
		 * = 0; NBTTagCompound compound = null;
		 * 
		 * if (itemStack.stackTagCompound.hasKey("spellbookmeta" + i)) meta =
		 * itemStack.stackTagCompound.getInteger("spellbookmeta" + i); if
		 * (itemStack.stackTagCompound.hasKey("spellbooktag" + i)) compound =
		 * itemStack.stackTagCompound.getCompoundTag("spellbooktag" + i);
		 * items[i] = new ItemStack(Item.itemsList[id], 1, meta); if (compound
		 * != null){ items[i].stackTagCompound = compound; } }
		 */
