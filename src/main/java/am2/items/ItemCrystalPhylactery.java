...
public class ItemCrystalPhylactery extends ItemArsMagica {
	...
	public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List<String> par3List,
		...
		if (par1ItemStack.hasTagCompound()) {
			...
			if (className != null) {
				par3List.add(String.format(I18n.translateToLocal("am2.tooltip.phyEss"),
						I18n.translateToLocal("entity." + className + ".name")));
	...
	public void getSpawnableEntities(World world) {
		for (Object clazz : EntityList.CLASS_TO_NAME.keySet()) {
			if (EntityCreature.class.isAssignableFrom((Class<?>) clazz)) {
				try {
					EntityCreature temp = (EntityCreature) ((Class<?>) clazz).getConstructor(World.class)
							.newInstance(world);
