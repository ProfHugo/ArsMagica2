...
public class ServerTickHandler {
	...
	private void gameTick_Start() {
		...
		if (firstTick) {
			ItemDefs.crystalPhylactery
					.getSpawnableEntities(FMLCommonHandler.instance().getMinecraftServerInstance().worldServers[0]);
			firstTick = false;
		}
	...
	// Iterator<Entry<EntityLiving, EntityLivingBase>> it =
	// targetsToSet.entrySet().iterator();
	// while (it.hasNext()){
	...
	public void blackoutArmorPiece(EntityPlayerMP player, EntityEquipmentSlot slot, int cooldown) {
		AMNetHandler.INSTANCE.sendPacketToClientPlayer(player, AMPacketIDs.FLASH_ARMOR_PIECE,
				new AMDataWriter().add(slot.getIndex()).add(cooldown).generate());
	}
