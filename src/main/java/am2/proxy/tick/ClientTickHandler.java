			...
			if (arcSpawnCounter++ >= arcSpawnFrequency) {
				...
				if (paths != null) {
					for (PowerTypes type : paths.keySet()) {
						...
						String texture = type == PowerTypes.LIGHT ? "textures/blocks/oreblockbluetopaz.png"
						...
						for (LinkedList<Vec3d> individualPath : pathList) {
							for (int i = 0; i < individualPath.size() - 1; ++i) {
								...
								if (start.squareDistanceTo(playerPos.toVec3D()) > 2500
										|| end.squareDistanceTo(playerPos.toVec3D()) > 2500) {
									continue;
								}

								TileEntity teStart = Minecraft.getMinecraft().theWorld
										.getTileEntity(new BlockPos(start));
								TileEntity teEnd = Minecraft.getMinecraft().theWorld.getTileEntity(new BlockPos(end));

								if (teEnd == null || !(teEnd instanceof IPowerNode))
									break;

								double startX = start.xCoord + ((teStart != null && teStart instanceof IPowerNode)
										? ((IPowerNode<?>) teStart).particleOffset(0) : 0.5f);
								double startY = start.yCoord + ((teStart != null && teStart instanceof IPowerNode)
										? ((IPowerNode<?>) teStart).particleOffset(1) : 0.5f);
								double startZ = start.zCoord + ((teStart != null && teStart instanceof IPowerNode)
										? ((IPowerNode<?>) teStart).particleOffset(2) : 0.5f);
								...
								AMLineArc arc = (AMLineArc) ArsMagica2.proxy.particleManager.spawn(
										Minecraft.getMinecraft().theWorld, texture, startX, startY, startZ, endX, endY,
										endZ);
			...
			if (checkForTKMove(stack)) {
				...
				if (this.mouseWheelValue != 0) {
					...
					AMNetHandler.INSTANCE.sendPacketToServer(AMPacketIDs.SPELLBOOK_CHANGE_ACTIVE_SLOT,
							new AMDataWriter().add(subID).add(Minecraft.getMinecraft().thePlayer.getEntityId())
									.add(Minecraft.getMinecraft().thePlayer.inventory.currentItem).generate());
				}
			}
			this.currentSlot = -1;
