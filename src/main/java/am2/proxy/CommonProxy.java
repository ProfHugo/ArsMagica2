...
public class CommonProxy implements IGuiHandler {
	...
	public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		...
		case GUI_INSCRIPTION_TABLE:
			return new ContainerInscriptionTable(
					(TileEntityInscriptionTable) world.getTileEntity(new BlockPos(x, y, z)), player.inventory);
		case GUI_ARMOR_INFUSION:
			return new ContainerArmorInfuser(player,
					(TileEntityArmorImbuer) world.getTileEntity(new BlockPos(x, y, z)));
		case GUI_KEYSTONE:
			...
			return new ContainerKeystone(player.inventory, player.getHeldItemMainhand(), runeBag,
					keystone.ConvertToInventory(keystoneStack),
					runeBag == null ? null : ItemDefs.runeBag.ConvertToInventory(runeBag), runeBagSlot);
		...
		case GUI_RUNE_BAG:
			...
			return new ContainerRuneBag(player.inventory, player.getHeldItemMainhand(),
		...
		case GUI_ESSENCE_BAG:
			...
			return new ContainerEssenceBag(player.inventory, player.getHeldItemMainhand(),
					essenceBag.ConvertToInventory(bagStack));
		}
		return null;
	}
	...
	public void preInit() {
		...
		CapabilityManager.INSTANCE.register(IEntityExtension.class, new IEntityExtension.Storage(),
				new IEntityExtension.Factory());
		CapabilityManager.INSTANCE.register(IAffinityData.class, new IAffinityData.Storage(),
				new IAffinityData.Factory());
		...
		CapabilityManager.INSTANCE.register(IArcaneCompendium.class, new IArcaneCompendium.Storage(),
				new IArcaneCompendium.Factory());
		CapabilityManager.INSTANCE.register(IDataSyncExtension.class, new IDataSyncExtension.Storage(),
				new IDataSyncExtension.Factory());
	}
	...
	public void init() {
		if (ArsMagica2.config.getEnableWitchwoodForest()) {
			...
			int id = (BiomeWitchwoodForest.getBiomeId() != -1) ? BiomeWitchwoodForest.getBiomeId()
					: BiomeWitchwoodForest.getNextFreeBiomeId();
	...
	public void postInit() {
		...
		for (AbstractSpellPart part : ArsMagicaAPI.getSpellRegistry().getValues()) {
			if (ArsMagicaAPI.getSkillRegistry().getValue(part.getRegistryName()) == null)
				throw new IllegalStateException("Spell Part " + part.getRegistryName()
						+ " is missing a skill, this would cause severe problems");
		}
	...
	private void registerFlickerOperators() {
		GameRegistry.register(FlickerOperatorItemTransport.instance,
				new ResourceLocation("arsmagica2", "item_transport"));
		...
		GameRegistry.register(FlickerOperatorMoonstoneAttractor.instance,
				new ResourceLocation("arsmagica2", "moonstone_attractor"));
		GameRegistry.register(FlickerOperatorNaturesBounty.instance,
				new ResourceLocation("arsmagica2", "natures_bounty"));
	...
	private void initOreDict() {
		...
		OreDictionary.registerOre("blockBlueTopaz",
				new ItemStack(BlockDefs.blocks, 1, EnumBlockType.BLUETOPAZ.ordinal()));
		OreDictionary.registerOre("blockVinteum", new ItemStack(BlockDefs.blocks, 1, EnumBlockType.VINTEUM.ordinal()));
		OreDictionary.registerOre("blockChimerite",
				new ItemStack(BlockDefs.blocks, 1, EnumBlockType.CHIMERITE.ordinal()));
		OreDictionary.registerOre("blockMoonstone",
				new ItemStack(BlockDefs.blocks, 1, EnumBlockType.MOONSTONE.ordinal()));
		OreDictionary.registerOre("blockSunstone",
				new ItemStack(BlockDefs.blocks, 1, EnumBlockType.SUNSTONE.ordinal()));
