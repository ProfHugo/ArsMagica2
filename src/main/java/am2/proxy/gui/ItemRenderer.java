...
public class ItemRenderer {
	private static final ResourceLocation sickleLocation = new ResourceLocation("arsmagica2",
			"textures/mobs/bosses/plant_guardian.png");
	private static final ResourceLocation arcaneBookLocation = new ResourceLocation("arsmagica2",
			"textures/mobs/bosses/arcane_guardian.png");
	private static final ResourceLocation winterArmLocation = new ResourceLocation("arsmagica2",
			"textures/mobs/bosses/ice_guardian.png");
	private static final ResourceLocation airLocation = new ResourceLocation("arsmagica2",
			"textures/mobs/bosses/air_guardian.png");
	private static final ResourceLocation waterLocation = new ResourceLocation("arsmagica2",
			"textures/mobs/bosses/water_guardian.png");
	private static final ResourceLocation fireLocation = new ResourceLocation("arsmagica2",
			"textures/mobs/bosses/fire_guardian.png");
	private static final ResourceLocation earthLocation = new ResourceLocation("arsmagica2",
			"textures/mobs/bosses/earth_guardian.png");
	private static final ResourceLocation broomLocation = new ResourceLocation("arsmagica2", "textures/mobs/broom.png");
	private static final ResourceLocation candleLocation = new ResourceLocation("arsmagica2",
			"textures/blocks/custom/candle.png");
	...
	public void renderItemEvent(RenderingItemEvent event) {
		...
		if (event.getStack().getItem() == ItemDefs.BoundShield) {
			Minecraft.getMinecraft().getTextureManager()
					.bindTexture(new ResourceLocation("arsmagica2:textures/models/bound_shield.png"));
			GlStateManager.pushMatrix();
			IItemPropertyGetter getter = ItemDefs.BoundShield.getPropertyGetter(new ResourceLocation("blocking"));
			ModelUtils.renderShield(event.getStack(),
					getter.apply(event.getStack(), Minecraft.getMinecraft().theWorld, event.getEntity()) == 1.0f,
					event.getCameraTransformType(), event.getEntity());
			GlStateManager.popMatrix();
			Minecraft.getMinecraft().renderEngine.bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
		}
		if (event.getStack() == null || event.getStack().getItem() == null
				|| Block.getBlockFromItem(event.getStack().getItem()) == null)
			return;
		Block block = Block.getBlockFromItem(event.getStack().getItem());
		if (!(block instanceof ITileEntityProvider))
			return;
		TileEntity te = ((ITileEntityProvider) block).createNewTileEntity(Minecraft.getMinecraft().theWorld,
				event.getStack().getItemDamage());
		if (te == null)
			return;
		...
		TRSRTransformation transform = ModelUtils.DEFAULT_BLOCK_STATE
				.apply(Optional.fromNullable(event.getCameraTransformType())).orNull();
		if (transform != null) {
			GlStateManager.translate(transform.getTranslation().x, transform.getTranslation().y,
					transform.getTranslation().z);
			GlStateManager.scale(transform.getScale().x, transform.getScale().y, transform.getScale().z);
			GlStateManager.rotate(new Quaternion(transform.getLeftRot().x, transform.getLeftRot().y,
					transform.getLeftRot().z, transform.getLeftRot().w));
		}
		// ModelUtils.transform(ModelUtils.DEFAULT_BLOCK_STATE,
		// event.getCameraTransformType(),
		// event.getCameraTransformType() ==
	...
	public void renderItem(TransformType type, ItemStack item, Object... data) {
		...
		GlStateManager.pushMatrix();
		// ModelUtils.transform(ModelUtils.DEFAULT_ITEM_STATE, type, type ==
		// TransformType.FIRST_PERSON_LEFT_HAND || type ==
		// TransformType.THIRD_PERSON_LEFT_HAND);
		...
		if (item.hasEffect()) {
			...
			GlStateManager.pushMatrix();
			{
				...
				GlStateManager.pushMatrix();
				{
					renderModelByItem(item);// this.renderModel(model,
											// -8372020);
				}
				GlStateManager.popMatrix();
				GlStateManager.matrixMode(5890);
		...
		if (item.getItem() == ItemDefs.wardingCandle && (type == TransformType.THIRD_PERSON_RIGHT_HAND
				|| type == TransformType.FIRST_PERSON_RIGHT_HAND || type == TransformType.FIRST_PERSON_LEFT_HAND
	...
	private void renderCandleFlame(TransformType type, ItemStack item, Object... data) {
		...
		if (item.hasTagCompound()) {
			GlStateManager.color(item.getTagCompound().getFloat("flame_red"),
					item.getTagCompound().getFloat("flame_green"), item.getTagCompound().getFloat("flame_blue"));
		}
		GlStateManager.pushMatrix();
	...
	private void setupItemRender(TransformType type, ItemStack stack) {
		...
		// }
		// if (type == TransformType.FIRST_PERSON_LEFT_HAND || type ==
		// TransformType.FIRST_PERSON_RIGHT_HAND || type ==
		// TransformType.THIRD_PERSON_LEFT_HAND || type ==
