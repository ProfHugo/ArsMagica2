...
public class ItemFrameWatcher {
		...
		for (int i = -radius; i <= radius; ++i) {
			for (int j = -radius; j <= radius; ++j) {
				for (int k = -radius; k <= radius; ++k) {
					...
					if (frame.worldObj.getBlockState(frame.getPosition().add(i, j, k))
							.getBlock() == BlockDefs.liquid_essence.getBlock()) {
						if (time == null) {
							time = 0;
						}
						time++;
	...
	private boolean frameIsValid(EntityItemFrame frame) {
		return frame != null && !frame.isDead && frame.getDisplayedItem() != null
	...
	private void updateQueuedChanges() {
		...
		for (EntityItemFrameComparator comp : toAdd) {
			if (comp.frame != null && (comp.frame.getDisplayedItem() == null
					|| comp.frame.getDisplayedItem().getItem() != ItemDefs.arcaneCompendium))
		...
		for (EntityItemFrameComparator comp : toRemove) {
			...
			if (time != null && time >= processTime && comp.frame != null && !comp.frame.isDead
					&& comp.frame.worldObj.isRemote
					&& (comp.frame.getDisplayedItem() != null && (comp.frame.getDisplayedItem().getItem() == Items.BOOK
							|| comp.frame.getDisplayedItem().getItem() == ItemDefs.arcaneCompendium))) {
	...
	public void spawnCompendiumProgressParticles(EntityItemFrame frame, BlockPos pos) {
		...
		AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(frame.worldObj, "symbols",
				pos.getX() + rand.nextDouble(), pos.getY() + rand.nextDouble(), pos.getZ() + rand.nextDouble());
		if (particle != null) {
			particle.setIgnoreMaxAge(true);
			particle.AddParticleController(
					new ParticleApproachEntity(particle, frame, 0.02f, 0.04f, 1, false).setKillParticleOnFinish(true));
			// particle.AddParticleController(new ParticleArcToEntity(particle,
			// 1, frame, false).SetSpeed(0.02f).setKillParticleOnFinish(true));
	...
	public void spawnCompendiumCompleteParticles(EntityItemFrame frame) {
		AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(frame.worldObj, "radiant", frame.posX,
				frame.posY, frame.posZ);
		if (particle != null) {
			particle.setIgnoreMaxAge(false);
			particle.setMaxAge(40);
			particle.setParticleScale(0.3f);
			// particle.AddParticleController(new
			// ParticleApproachEntity(particle, frame, 0.02f, 0.04f, 1,
