...
public class PowerNodeRegistry {
	...
	// private static final HashMap<Integer, PowerNodeRegistry>
	// dimensionPowerManagers = new HashMap<Integer, PowerNodeRegistry>();
	...
	static final int POWER_SEARCH_RADIUS = 10; // The literal power search
												// radius
	static final int POWER_SEARCH_RADIUS_SQ = 100; // The power search radius
													// squared. This is used for
													// faster vector math.
	static final int MAX_POWER_SEARCH_RADIUS = 10000; // The maximum power
														// search radius
														// (squared)
	...
	PowerNodeEntry registerPowerNodeInternal(IPowerNode<?> node) {
		...
		if (powerNodes.containsKey(chunk)) {
			...
			LogHelper
					.trace(String.format("Located Power Node list for chunk %d, %d", chunk.chunkXPos, chunk.chunkZPos));
		} else {
			...
			if (compound == null || !compound.hasKey("AM2PowerData")) {
				...
				LogHelper.trace("Prior node list not found.  Created Power Node list for chunk %d, %d", chunk.chunkXPos,
						chunk.chunkZPos);
			} else {
		...
		LogHelper.trace("Successfully registered power node at {%d, %d, %d}", ((TileEntity) node).getPos().getX(),
				((TileEntity) node).getPos().getY(), ((TileEntity) node).getPos().getZ());
	...
	/**
	 * Attempts to pair the source and destination nodes by either a direct link
	 * or by going through conduits.
	 *
	 * @param powerSource
	 *            The power source
	 * @param destination
	 *            The destination point
	 * @return A localized message to return to the entity attempting to pair
	 *         the nodes, either of success or why it failed.
	 */
	public String tryPairNodes(IPowerNode<?> powerSource, IPowerNode<?> destination) {
		...
		// Can the power source provide any of the valid power types for the
		// destination?
		...
		// construct a list of all valid power types common between the source
		// and destination
		int successes = 0;

		for (PowerTypes type : typesProvided) {
			LinkedList<Vec3d> powerPath = new LinkedList<Vec3d>();
			PowerNodePathfinder pathfinder = new PowerNodePathfinder(((TileEntity) powerSource).getWorld(),
					sourceLocation, destLocation, type);
	...
	private void removePowerNode(ChunkPos chunk, Vec3d location) {
		...
		if (powerNodes.containsKey(chunk)) {
			...
			LogHelper.error("Power Node removal requested in a non-tracked chunk (%d, %d)!", chunk.chunkXPos,
					chunk.chunkZPos);
		}
	}
	...
	public float requestPower(IPowerNode<?> destination, PowerTypes type, float amount) {
		...
		float requested = data.requestPower(((TileEntity) destination).getWorld(), type, amount,
	 ...
	 * @param power
	 *            The power type that the provider needs to supply or accept to
	 *            be considered valid
	...
	public IPowerNode<?>[] getAllNearbyNodes(World world, Vec3d location, PowerTypes power) {
		...
		for (ChunkPos pair : search) {
			...
			if (nodesInChunk != null) {
				// Add only vectors that are less than or equal to
				// POWER_SEARCH_RADIUS_SQ away
	...
	private ChunkPos[] getSearchChunks(Vec3d location) {
		...
		for (int i = -1; i <= 1; ++i) {
			for (int j = -1; j <= 1; ++j) {
				// don't include the home chunk, this has already been added.
				// We're only concerned with neighboring chunks.
				if (i == 0 && j == 0)
					continue;
				// create the new CCIP at the offset location
				ChunkPos newPair = new ChunkPos(chunkX + i, chunkZ + j);
				// offset the x/z locations by POWER_SEARCH_RADIUS * i/j
				// respectively and calculate the chunk that would fall in.
				// If it matches the new chunk coordinates, we add that chunk to
				// the list of chunks to search.
				if (((int) location.xCoord + (POWER_SEARCH_RADIUS * i)) >> 4 == newPair.chunkXPos
						&& ((int) location.zCoord + (POWER_SEARCH_RADIUS * j)) >> 4 == newPair.chunkZPos) {
	...
	private ChunkPos getChunkFromNode(IPowerNode<?> node) {
		...
		// return
		// te.getWorld().getChunkFromBlockCoords(te.getPos()).getChunkCoordIntPair();
	}

	// private ChunkPos getChunkFromPosition(World world, Vec3d location){
	// return new ChunkPos((int)location.xCoord >> 4, (int)location.zCoord >>
	// 4);
	// }
	...
	public void LoadChunkFromNBT(ChunkPos chunk, NBTTagCompound compound) {
		...
		for (int i = 0; i < powerNodeTagList.tagCount(); ++i) {
			...
			Vec3d nodeLocation = new Vec3d(nodeCompound.getInteger("xCoord"), nodeCompound.getInteger("yCoord"),
	...
	//
	// if (te1.xCoord == te2.xCoord && te1.zCoord == te2.zCoord && te1.yCoord ==
	// te2.yCoord)
	// return 0;
	//
