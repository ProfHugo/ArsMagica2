...
public class PowerNodeEntry {
	...
	private boolean validatePath(World world, LinkedList<Vec3d> path) {
		for (Vec3d vec : path) {
			...
			// set a marker block to say that a conduit or other power relay of
			// some sort was here and is now not
			if (!world.isRemote && world.isAirBlock(new BlockPos(vec))) {
				// world.setBlock(new BlockPos(vec),
				// BlocksCommonProxy.brokenLinkBlock);
			}
	...
	public NBTTagCompound saveToNBT() {
		...
		for (PowerTypes type : nodePaths.keySet()) {
			...
			// AMCore.log.info("Saved %d node paths for %s etherium.",
			// nodePaths.get(type).size(), type.name());

			// append this entry in the power path list to the list of power
			// path entries
		...
		if (powerPathList != null) {
			for (int i = 0; i < powerPathList.tagCount(); ++i) {
				...
				if (pathNodes != null) {
					...
					// ArsMagica2.LOGGER.info(String.format("Loaded %d node
					// paths for %s etherium.", pathsList.size(), type.name()));
				}
			}
		}
	}
