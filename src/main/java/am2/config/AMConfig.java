...
public class AMConfig extends Configuration {
	...
	public void init() {
		...
		DamageMultiplier = (float) get(CATEGORY_GENERAL, KEY_DamageMultiplier, 1.0,
				"How much the damage in Ars Magica is scaled.").getDouble(1.0);

		UseSpecialRenderers = get(CATEGORY_GENERAL, KEY_UseSpecialRenderers, true,
		...
		DisplayManaInInventory = get(CATEGORY_GENERAL, KEY_DisplayManaInInventory, def,
				"This will toggle mana display on and off in your inventory.  Default 'O' key in game.")
						.getBoolean(def);

		FrictionCoefficient = (float) get(CATEGORY_GENERAL, KEY_FrictionCoefficient, 0.8,
		...
		Property retroWorldGenProp = get(CATEGORY_GENERAL, KEY_RetroactiveWorldGen, false,
		...
		secondarySkillTreeTierCap = get(CATEGORY_GENERAL, KEY_SecondarySkillTreeTierCap, 99,
		...
		SpellBookPosition = new AMVector2(get(CATEGORY_UI, KEY_SpellBookPositionX, 0.0).getDouble(0.0),
				get(CATEGORY_UI, KEY_SpellBookPositionY, 0.0).getDouble(0.0));
		manaShieldingPosition = new AMVector2(
				get(CATEGORY_UI, KEY_ManaShieldingPositionX, 0.7104166746139526).getDouble(0.7104166746139526),
				get(CATEGORY_UI, KEY_ManaShieldingPositionY, 0.9352226853370667).getDouble(0.9352226853370667));
		showHudMinimally = get(CATEGORY_UI, KEY_ShowHudMinimally, false,
		...
		witchwoodForestID = get(CATEGORY_GENERAL, KEY_WitchwoodForestBiomeID, -1,
				"Sets the biome ID that Witchwood Forest will use. Default: -1 (automatic))").getInt();
		witchwoodLeafParticles = get(CATEGORY_GENERAL, KEY_witchwoodLeavesFall, true,
				"Disable this if you experience low FPS in witchwood forests").getBoolean(true);
		enableWitchwoodForest = get(CATEGORY_GENERAL, KEY_EnableWitchwoodForest, true,
				"Disable this if you prefer the witchwood forest to not generate").getBoolean(true);
		witchwoodForestRarity = get(CATEGORY_GENERAL, KEY_WitchwoodForestRarity, 6,
				"Sets how rare witchwood forests are.  Lower is more rare.").getInt();

		allowCreativeTargets = get(CATEGORY_GENERAL, KEY_allowCreativeTargets, true,
				"Disable this to prevent spell effects on creative players").getBoolean(true);

		moonstoneMeteorsDestroyTerrain = get(CATEGORY_GENERAL, KEY_moonstoneMeteorsDestroyTerrain, true,
				"Should moonstone meteors destroy terrain when landing?  Keep in mind they will never land on anything other than grass.")
						.getBoolean(true);

		suggestSpellNames = get(CATEGORY_GENERAL, KEY_moonstoneMeteorsDestroyTerrain, true,
		...
		forgeSmeltsVillagers = get(CATEGORY_GENERAL, KEY_ForgeSmeltsVillagers, true,
		...
		meteorMinSpawnLevel = get(CATEGORY_GENERAL, KEY_MeteorMinSpawnLevel, 10,
				"You must reach this magic level before Moonstone meteors will fall near you.").getInt();

		hazardousGateways = get(CATEGORY_GENERAL, KEY_HazardousGateways, true,
				"Set this to false in order to disable gateways sending you partial distances if you don't have enough power.")
						.getBoolean(true);

		ArmorXPInfusionFactor = get(CATEGORY_GENERAL, KEY_ArmorXPInfusionFactor, 1.0,
				"Alter this to change the rate at which armor XP infuses.").getDouble();
		disarmAffectsPlayers = get(CATEGORY_GENERAL, KEY_DisarmAffectsPlayers, true,
		...
		digBreaksTileEntities = get(CATEGORY_GENERAL, KEY_DigBreaksTEs, true,
				"Can the dig component break blocks that have a tile entity?").getBoolean(true);

		savePowerOnWorldSave = get(CATEGORY_GENERAL, KEY_SavePowerOnWorldSave, true,
		...
		canDryadsDespawn = get(CATEGORY_MOBS, KEY_CanDryadsDespawn, true,
				"Set this to false if you don't want dryads to despawn.").getBoolean(true);

		enderAffinityAbilityCooldown = get(CATEGORY_GENERAL, KEY_EnderAffinityAbilityCooldown, 100,
				"Set this to the number of ticks between ender affinity teleports.").getInt();

		String digBlacklistString = get(CATEGORY_GENERAL, KEY_DigDisabledBlocks, "",
		...
		String worldgenBlackList = get(CATEGORY_GENERAL, KEY_WorldgenBlacklist, "-27,-28,-29",
		...
		String apBlockBL = get(CATEGORY_GENERAL, KEY_Appropriation_Block_Blacklist, "",
				"Comma-separated list of block IDs that appropriation cannot pick up.").getString();
		appropriationBlockBlacklist = apBlockBL.split(",");

		String apEntBL = get(CATEGORY_GENERAL, KEY_Appropriation_Mob_Blacklist, "",
	...
	public void setDisplayManaInInventory(boolean value) {
		boolean def = !Loader.isModLoaded("NotEnoughItems");
		Property prop = get(CATEGORY_GENERAL, KEY_DisplayManaInInventory, def,
				"This will toggle mana display on and off in your inventory.  Default 'O' key in game.");
		prop.set(value);
	...
	public void disableRetroactiveWorldgen() {
		Property prop = get(CATEGORY_GENERAL, KEY_RetroactiveWorldGen, false,
	...
	public void setGuiPositions(AMVector2 manaHud, AMVector2 burnoutHud, AMVector2 levelHud, AMVector2 affinityHud,
			AMVector2 posBuffsHud, AMVector2 negBuffsHud, AMVector2 armorHead, AMVector2 armorChest,
			AMVector2 armorLegs, AMVector2 armorBoots, AMVector2 xpBar, AMVector2 contingency, AMVector2 manaNumeric,
			AMVector2 burnoutNumeric, AMVector2 XPNumeric, AMVector2 spellBookPos, AMVector2 manaShieldingPos,
			boolean showBuffs, boolean showNumerics, boolean minimalHud, boolean showArmorUI, boolean showXPAlways,
			boolean showHudBars) {
