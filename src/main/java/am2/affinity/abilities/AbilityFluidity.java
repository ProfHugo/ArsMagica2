...
public class AbilityFluidity extends AbstractAffinityAbility {
	...
	private void applyReverseWaterMovement(EntityPlayer entity) {
		AxisAlignedBB par1AxisAlignedBB = entity.getEntityBoundingBox().expand(0.0D, -0.4000000059604645D, 0.0D)
				.contract(0.001D);
				...
				for (int i2 = i1; i2 < j1; ++i2) {
					...
					if (blockState != null && blockState.getMaterial() == Material.WATER) {
						double d0 = l1 + 1 - BlockLiquid
								.getLiquidHeightPercent(blockState.getBlock().getMetaFromState(blockState));

						if (l >= d0) {
							blockState.getBlock().modifyAcceleration(entity.worldObj, new BlockPos(k1, l1, i2), entity,
									vec3);
						}
					}
				}
