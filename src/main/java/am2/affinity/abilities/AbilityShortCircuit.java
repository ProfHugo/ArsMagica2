		...
		if (player.isWet() && !player.worldObj.isRemote) {
			if (player.getRNG().nextFloat() < 0.04f) {
				...
				if (player.worldObj.isRemote) {
					ArsMagica2.proxy.particleManager.BoltFromEntityToPoint(player.worldObj, player,
							player.posX - 2 + player.getRNG().nextDouble() * 4,
							player.posY + player.getEyeHeight() - 2 + player.getRNG().nextDouble() * 4,
							player.posZ - 2 + player.getRNG().nextDouble() * 4);
				}
			}
			// else{
			// if (player.getRNG().nextDouble() < 0.4f)
			// player.worldObj.playSoundAtEntity(player,
			// "arsmagica2:misc.event.mana_shield_block", 1.0f,
			// player.worldObj.rand.nextFloat() + 0.5f);
			// }
		}
	}
}
