...
public class AbilityLightAsAFeather extends AbstractAffinityAbility {
	...
	public void applyPreSpellCast(EntityPlayer ent, Pre event) {
		if (ent.worldObj.isRaining() && !ent.worldObj.isRemote
				&& ent.getEntityWorld().getBiome(ent.getPosition()).canRain() && !ent.worldObj.isRemote
				&& ent.worldObj.rand.nextInt(100) < 10) {
			if (!ent.isSneaking() && !ent.isPotionActive(PotionEffectsDefs.gravityWell)
					&& !ent.isInsideOfMaterial(Material.WATER) && ent.isWet()) {
