...
public class AffinityAbilityModifiers {
	...
	public static final AttributeModifier natureAffinityRoots = new AttributeModifier(natureAffinityModifierID,
			"Nature's Roots", -0.1, OPERATION_ADD);
	...
	public static final AttributeModifier iceAffinityColdBlooded = new AttributeModifier(iceAffinityodifierID,
			"Cold Blooded", -0.1, OPERATION_ADD);
	...
	public static final AttributeModifier lightningAffinitySpeed = new AttributeModifier(lightningAffinityModifierID,
			"Lightning Reflexes", 1.20, OPERATION_ADD);

	public static final UUID waterWeaknessID = UUID.fromString("3b51a94c-7844-732b-8b69-a1f5cd50a60f");
	public static final AttributeModifier waterWeakness = new AttributeModifier(waterWeaknessID, "Water Weakness",
			-0.25, OPERATION_ADD);
	public static final UUID fireWeaknessID = UUID.fromString("3b51a94c-7844-732b-8b69-a1f5cd50a60e");
	public static final AttributeModifier fireWeakness = new AttributeModifier(fireWeaknessID, "Fire Weakness", -0.25,
			OPERATION_ADD);
	public static final UUID sunlightWeaknessID = UUID.fromString("3b51a94c-7844-732b-8b69-a1f5cd50a60d");
	public static final AttributeModifier sunlightWeakness = new AttributeModifier(sunlightWeaknessID,
			"Sunlight Weakness", -0.25, OPERATION_ADD);

	public void applySpeedModifiersBasedOnDepth(EntityPlayer ent, float natureDepth, float iceDepth,
			float lightningDepth) {
	...
	public void applyHealthModifiers(EntityPlayer ent, float enderDepth, float waterDepth, float fireDepth,
			float lightningDepth) {
		...
		applyOrRemoveModifier(attribute, waterWeakness,
				((fireDepth >= 0.5f && fireDepth <= 0.9f) || (enderDepth >= 0.5f && enderDepth <= 0.9f)
						|| (lightningDepth >= 0.5f && lightningDepth <= 0.9f)) && ent.isWet());
		applyOrRemoveModifier(attribute, fireWeakness, (waterDepth >= 0.5f && waterDepth <= 0.9f)
				&& (ent.isBurning() || ent.worldObj.provider.getDimension() == -1));
		int worldTime = (int) ent.worldObj.getWorldTime() % 24000;
		applyOrRemoveModifier(attribute, sunlightWeakness, (enderDepth > 0.65 && enderDepth <= 0.95f)
				&& ent.worldObj.canBlockSeeSky(ent.getPosition()) && (worldTime > 23000 || worldTime < 12500));
	}
	...
	public boolean isOnIce(EntityPlayer ent) {
		AxisAlignedBB par1AxisAlignedBB = ent.getEntityBoundingBox().expand(0.0D, -0.4000000059604645D, 0.0D)
				.contract(0.001D);
