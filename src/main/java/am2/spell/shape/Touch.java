...
public class Touch extends SpellShape {
	...
	public SpellCastResult beginStackStage(ItemSpellBase item, ItemStack stack, EntityLivingBase caster,
		...
		if (mop == null) {
			...
			if (mop.typeOfHit == RayTraceResult.Type.ENTITY) {
				...
				if (e instanceof EntityDragonPart && ((EntityDragonPart) e).entityDragonObj instanceof EntityLivingBase)
					e = (EntityLivingBase) ((EntityDragonPart) e).entityDragonObj;
				SpellCastResult result = SpellUtils.applyStageToEntity(stack, caster, world,
						(target == null) ? e : target, giveXP);
				if (result != SpellCastResult.SUCCESS) {
					return result;
				}
				return SpellUtils.applyStackStage(stack, caster, target, mop.hitVec.xCoord, mop.hitVec.yCoord,
						mop.hitVec.zCoord, null, world, true, giveXP, 0);
			} else {
				SpellCastResult result = SpellUtils.applyStageToGround(stack, caster, world, mop.getBlockPos(),
						mop.sideHit, mop.hitVec.xCoord, mop.hitVec.yCoord, mop.hitVec.zCoord, giveXP);
				if (result != SpellCastResult.SUCCESS) {
					return result;
				}
				return SpellUtils.applyStackStage(stack, caster, target, mop.getBlockPos().getX(),
						mop.getBlockPos().getY(), mop.getBlockPos().getZ(), mop.sideHit, world, true, giveXP, 0);
			}
		}
	}
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.itemOre, 1, ItemOre.META_VINTEUM), Items.FEATHER, Items.FISH,
				Items.CLAY_BALL };
	}
