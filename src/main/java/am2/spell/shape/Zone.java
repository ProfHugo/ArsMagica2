...
public class Zone extends SpellShape {
	...
	public SpellCastResult beginStackStage(ItemSpellBase item, ItemStack stack, EntityLivingBase caster,
		...
		if (temp > 2D)
			temp++;
		int radius = (int) temp;
		// int radius = SpellUtils.getModifiedInt_Add(1, stack, caster, target,
		// world, SpellModifiers.RADIUS);
	...
	public Object[] getRecipe() {
		return new Object[] { BlockDefs.tarmaRoot, new ItemStack(ItemDefs.itemOre, 1, ItemOre.META_MOONSTONE),
				new ItemStack(ItemDefs.itemOre, 1, ItemOre.META_SUNSTONE), Items.DIAMOND };
	}
