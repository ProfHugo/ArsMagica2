...
public class Rune extends SpellShape {
	...
	public SpellCastResult beginStackStage(ItemSpellBase item, ItemStack stack, EntityLivingBase caster,
			EntityLivingBase target, World world, double x, double y, double z, EnumFacing side, boolean consumeMBR,
			int useCount) {
		...
		if (!world.isRemote) {
			// world.setTileEntity(mop.getBlockPos().up(),
			// BlockDefs.spellRune.createNewTileEntity(world, 0));
			...
			BlockDefs.spellRune.setNumTriggers(world, mop.getBlockPos().up(),
					world.getBlockState(mop.getBlockPos().up()), procs);
		}
	...
	public Object[] getRecipe() {
		// Blue Rune, Red Rune, White Rune, Black Rune, Orange Rune, Purple
		// Rune, Yellow Rune
