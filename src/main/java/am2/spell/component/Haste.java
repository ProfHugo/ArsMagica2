...
public class Haste extends SpellComponent implements IRitualInteraction {
		...
		if (target instanceof EntityLivingBase) {
			...
			if (!world.isRemote)
				((EntityLivingBase) target).addPotionEffect(
						new BuffEffectHaste(duration, SpellUtils.countModifiers(SpellModifiers.BUFF_POWER, stack)));
			return true;
		}
		return false;
	}
		...
		for (int i = 0; i < 25; ++i) {
			...
			if (particle != null) {
				...
				particle.AddParticleController(
						new ParticleOrbitPoint(particle, x, y, z, 2, false).setIgnoreYCoordinate(true)
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.YELLOW.getDyeDamage()), Items.REDSTONE,
	...
	public ItemStack[] getReagents() {
		return new ItemStack[] { new ItemStack(Items.GLOWSTONE_DUST) };
	}
