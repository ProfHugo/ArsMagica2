...
public class Shield extends SpellComponent implements IRitualInteraction {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		if (target instanceof EntityLivingBase) {
			...
			if (!world.isRemote)
				((EntityLivingBase) target).addPotionEffect(new BuffEffectMagicShield(duration,
		...
		for (int i = 0; i < 25; ++i) {
			...
			if (particle != null) {
				particle.AddParticleController(
						new ParticleOrbitEntity(particle, target, 0.2f, 2, false).setOrbitY(-1).SetTargetDistance(1));
	...
	public ItemStack[] getReagents() {
		return new ItemStack[] { new ItemStack(Items.IRON_INGOT) };
	}
