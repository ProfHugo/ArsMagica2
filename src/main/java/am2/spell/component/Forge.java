...
public class Forge extends SpellComponent {
	...
	public boolean applyEffectBlock(ItemStack stack, World world, BlockPos pos, EnumFacing blockFace, double impactX,
		...
		if (!CanApplyFurnaceToBlockAtCoords(caster, world, pos))
			return false;
		...
		if (!world.isRemote) {
			if (ItemIsBlock(smelted.getItem())) {
				world.setBlockState(pos,
						((ItemBlock) smelted.getItem()).block.getStateFromMeta(smelted.getItemDamage()));
			} else {
				EntityItem item = new EntityItem(world, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5F,
						smelted.copy());
				float f3 = 0.05F;
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.RED.getDyeDamage()), Blocks.FURNACE };
	}
