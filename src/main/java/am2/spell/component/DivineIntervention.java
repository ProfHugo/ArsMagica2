...
public class DivineIntervention extends SpellComponent {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		...
		if (((EntityLivingBase) target).isPotionActive(PotionEffectsDefs.astralDistortion)) {
			if (target instanceof EntityPlayer)
				((EntityPlayer) target).addChatMessage(
						new TextComponentString("The distortion around you prevents you from teleporting"));
			return true;
		}
		...
		if (target.dimension == 1) {
			...
			BlockPos coords = target instanceof EntityPlayer ? ((EntityPlayer) target).getBedLocation(target.dimension)
					: null;
			if (coords == null || (coords == BlockPos.ORIGIN)) {
				coords = world.getSpawnPoint();
			}
			while (world.getBlockState(coords).getBlock() != Blocks.AIR
					&& world.getBlockState(coords.up()).getBlock() != Blocks.AIR) {
				coords = coords.up();
			}
			...
			if (particle != null) {
				...
				if (rand.nextBoolean())
					...
					particle.AddParticleController(new ParticleOrbitPoint(particle, x, y, z, 1, false)
							.SetOrbitSpeed(0.1f).SetTargetDistance(rand.nextDouble() + 0.5));
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.PURPLE.getDyeDamage()), Items.BED,
				Items.ENDER_PEARL };
	}
