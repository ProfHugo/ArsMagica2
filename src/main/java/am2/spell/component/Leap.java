...
public class Leap extends SpellComponent implements IRitualInteraction {
		...
		if (target instanceof EntityLivingBase) {
			...
			if (!world.isRemote)
				((EntityLivingBase) target).addPotionEffect(
						new BuffEffectLeap(duration, SpellUtils.countModifiers(SpellModifiers.BUFF_POWER, stack)));
			return true;
		}
		return false;
	}
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.WHITE.getDyeDamage()), Blocks.HOPPER };
	}
	...
	public ItemStack[] getReagents() {
		return new ItemStack[] { new ItemStack(Items.FERMENTED_SPIDER_EYE) };
	}
