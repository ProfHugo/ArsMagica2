...
public class Invisiblity extends SpellComponent implements IRitualInteraction {
		...
		if (target instanceof EntityLivingBase) {
			...
			if (!world.isRemote)
				((EntityLivingBase) target).addPotionEffect(new PotionEffect(
						Potion.getPotionFromResourceLocation(new ResourceLocation("invisibility").toString()), duration,
		...
		for (int i = 0; i < 25; ++i) {
			...
			if (particle != null) {
				...
				particle.AddParticleController(
						new ParticleFadeOut(particle, 3, false).setFadeSpeed(0.05f).setKillParticleOnFinish(true));
	...
	public ItemStack[] getReagents() {
		return new ItemStack[] { new ItemStack(Items.ENDER_EYE) };
	}
