...
public class Attract extends SpellComponent {
	...
	private boolean doTK_Extrapolated(ItemStack stack, World world, double impactX, double impactY, double impactZ,
		...
		if (caster instanceof EntityPlayer) {
			...
			RayTraceResult mop = ((SpellBase) ItemDefs.spell).getMovingObjectPosition(caster, world, range, false,
					false);
			if (mop == null) {
		...
		EntityLivingBase target = getClosestEntityToPointWithin(caster, world, new Vec3i(impactX, impactY, impactZ),
				16);
	...
	private EntityLivingBase getClosestEntityToPointWithin(EntityLivingBase caster, World world, Vec3i point,
			double radius) {
		AxisAlignedBB bb = new AxisAlignedBB(point.getX() - radius, point.getY() - radius, point.getZ() - radius,
				point.getX() + radius, point.getY() + radius, point.getZ() + radius);
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.BLACK.getDyeDamage()), Items.IRON_INGOT };
	}
