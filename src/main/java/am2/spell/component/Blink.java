...
public class Blink extends SpellComponent {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		...
		double motionX = -MathHelper.sin((target.rotationYaw / 180F) * 3.141593F)
				* MathHelper.cos((target.rotationPitch / 180F) * 3.141593F) * distance;
		double motionZ = MathHelper.cos((target.rotationYaw / 180F) * 3.141593F)
				* MathHelper.cos((target.rotationPitch / 180F) * 3.141593F) * distance;
		...
		while (!coordsValid && distance > 0) {
			...
			TileEntityAstralBarrier blocker = DimensionUtilities.GetBlockingAstralBarrier(world,
					new BlockPos(newX, newY, newZ), keystoneKeys);
			while (blocker != null) {
				...
				motionX = -MathHelper.sin((target.rotationYaw / 180F) * 3.141593F)
						* MathHelper.cos((target.rotationPitch / 180F) * 3.141593F) * distance;
				motionZ = MathHelper.cos((target.rotationYaw / 180F) * 3.141593F)
						* MathHelper.cos((target.rotationPitch / 180F) * 3.141593F) * distance;
				...
				blocker = DimensionUtilities.GetBlockingAstralBarrier(world, new BlockPos(newX, newY, newZ),
						keystoneKeys);
			}
			if (distance < 0) {
				coordsValid = false;
		...
		if (!world.isRemote) {
			if (!coordsValid && target instanceof EntityPlayer) {
				((EntityPlayer) target)
						.addChatMessage(new TextComponentString("Can't find a place to blink forward to."));
				return false;
			}
		}
	...
	private boolean CheckCoords(World world, int x, int y, int z) {
		...
		if (secondBlock != null) {
			secondBlockBB = secondBlock.getBlock().getCollisionBoundingBox(secondBlock, world,
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.PURPLE.getDyeDamage()), Items.ENDER_PEARL };
	}
