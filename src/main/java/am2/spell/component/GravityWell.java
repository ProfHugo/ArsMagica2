...
public class GravityWell extends SpellComponent implements IRitualInteraction {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		if (target instanceof EntityLivingBase) {
			...
			if (!world.isRemote)
				((EntityLivingBase) target).addPotionEffect(new BuffEffectGravityWell(duration,
		...
		for (int i = 0; i < 25; ++i) {
			...
			if (particle != null) {
				...
				particle.AddParticleController(new ParticleLeaveParticleTrail(particle, "pulse", false, 5, 2, false)
						...
						.setParticleRGB_F(0.7f, 0.2f, 0.9f).setTicksBetweenSpawns(2));
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.GREEN.getDyeDamage()), Items.STRING,
				Blocks.STONE };
	}
