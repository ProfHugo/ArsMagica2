...
public class Rend extends SpellComponent {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		...
		float baseDamage = 2;
		float damage = 0;
		double dpa = SpellUtils.getModifiedDouble_Add(baseDamage, stack, caster, target, world, SpellModifiers.DAMAGE)
				/ 4;
		int arrowCount = (int) ((EntityLivingBase) target).getArrowCountInEntity();
		...
		damage += arrowCount * dpa;
		damage = damage + damage * lethalityCount / 40 * targetArmor;
		((EntityLivingBase) target).setArrowCountInEntity(0);
		return SpellUtils.attackTargetSpecial(stack, target, DamageSources.causePhysicalDamage(caster),
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.RED.getDyeDamage()), Items.ARROW,
