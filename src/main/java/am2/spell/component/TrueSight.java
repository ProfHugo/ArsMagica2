...
public class TrueSight extends SpellComponent implements IRitualInteraction {
		...
		if (target instanceof EntityLivingBase) {
			...
			if (!world.isRemote)
				((EntityLivingBase) target).addPotionEffect(
						new BuffEffectTrueSight(duration, SpellUtils.countModifiers(SpellModifiers.BUFF_POWER, stack)));
			return true;
		}
		return false;
	}
		...
		for (int i = 0; i < 25; ++i) {
			...
			if (particle != null) {
				...
				particle.AddParticleController(
						new ParticleOrbitEntity(particle, target, 0.1f, 2, false).setOrbitY(0.5).SetTargetDistance(1f));
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.BLUE.getDyeDamage()),
				new ItemStack(ItemDefs.itemOre, 1, ItemOre.META_CHIMERITE), Blocks.GLASS_PANE };
	}
	...
	public ItemStack[] getReagents() {
		return new ItemStack[] { new ItemStack(ItemDefs.itemOre, 1, ItemOre.META_VINTEUM) };
	}
