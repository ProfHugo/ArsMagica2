...
public class Repel extends SpellComponent {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		...
		if (target == caster) {
			...
			List<Entity> ents = world.getEntitiesWithinAABB(Entity.class,
					source.getEntityBoundingBox().expand(2, 2, 2));
	...
	private void performRepel(World world, EntityLivingBase caster, Entity target) {
		...
		Vec3d delta = new Vec3d(targetPos.xCoord - casterPos.xCoord, targetPos.yCoord - casterPos.yCoord,
				targetPos.zCoord - casterPos.zCoord);
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
		...
		for (int i = 0; i < ArsMagica2.config.getGFXLevel() * 2; i++) {
			...
			if (effect != null) {
				...
				// effect.AddParticleController(new
				// ParticleMoveOnHeading(effect, angle, 0, 0.1 +
				// rand.nextDouble() * 0.5, 1, false));
				effect.AddParticleController(new ParticleFleePoint(effect,
						new AMVector3(caster).add(new AMVector3(0, caster.getEyeHeight(), 0)).toVec3D(), 0.075f, 3f, 1,
						true));
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.WHITE.getDyeDamage()), Items.WATER_BUCKET };
	}
