...
public class Transplace extends SpellComponent implements IRitualInteraction {
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
		...
		for (int i = 0; i < 15; ++i) {
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(world, "sparkle2", caster.posX,
					caster.posY + caster.getEyeHeight(), caster.posZ);
			if (particle != null) {
				particle.addRandomOffset(1, 1, 1);
				particle.AddParticleController(
						new ParticleArcToPoint(particle, 1, target.posX, target.posY + target.getEyeHeight(),
								target.posZ, false).SetSpeed(0.05f).generateControlPoints());
		...
		for (int i = 0; i < 15; ++i) {
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(world, "sparkle2", target.posX,
					target.posY + target.getEyeHeight(), target.posZ);
			if (particle != null) {
				particle.addRandomOffset(1, 1, 1);
				particle.AddParticleController(
						new ParticleArcToPoint(particle, 1, caster.posX, caster.posY + caster.getEyeHeight(),
								caster.posZ, false).SetSpeed(0.05f).generateControlPoints());
				particle.setMaxAge(40);
				particle.setParticleScale(0.2f);
				particle.setRGBColorF(0, 0, 1);
				if (colorModifier > -1) {
					particle.setRGBColorF((0xFF - ((colorModifier >> 16) & 0xFF)) / 255.0f,
							(0xFF - ((colorModifier >> 8) & 0xFF)) / 255.0f, (0xFF - (colorModifier & 0xFF)) / 255.0f);
				}
			}
		}
	}
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.RED.getDyeDamage()), Items.COMPASS,
				new ItemStack(ItemDefs.rune, 1, EnumDyeColor.BLUE.getDyeDamage()), Items.ENDER_PEARL };
	}
	...
	public ItemStack[] getReagents() {
		return new ItemStack[] { new ItemStack(ItemDefs.itemOre, 1, ItemOre.META_PURIFIED_VINTEUM),
				new ItemStack(ItemDefs.mageArmor), new ItemStack(ItemDefs.mageBoots), new ItemStack(ItemDefs.mageHood),
				new ItemStack(ItemDefs.mageLeggings), new ItemStack(ItemDefs.playerFocus) };
	}
