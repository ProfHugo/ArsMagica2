...
public class Grow extends SpellComponent {
	...
	public boolean applyEffectBlock(ItemStack stack, World world, BlockPos pos, EnumFacing blockFace, double impactX,
		...
		BonemealEvent event = new BonemealEvent(DummyEntityPlayer.fromEntityLiving(caster), world, pos,
		...
		// EoD: Spawn AM2 flowers with 3% chance. This has to be the first one
		// in the list to override all others
		if (world.rand.nextInt(100) < 3 && block.isNormalCube()) {
			...
			// We did not find a flower or we have been executed on the wrong
			// block. Either way, we continue
		}
		...
		// If the spell is executed in water, check if we have space for a
		// wakebloom above and create one 3% of the time.
		...
		// EoD: If there is already tallgrass present, let's grow it further 20%
		// of the time.
		...
		// EoD: If there is already deadbush present, let's revitalize it 20% of
		// the time.
		...
		// EoD: Apply vanilla bonemeal effect to growables 30% of the time. This
		// is the generic grow section.
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
		...
		for (int i = 0; i < 25; ++i) {
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(world, "plant", x + 0.5, y + 1,
					z + 0.5);
			if (particle != null) {
				...
				particle.AddParticleController(
						new ParticleOrbitPoint(particle, x + 0.5, y + 0.5, z + 0.5, 2, false).setIgnoreYCoordinate(true)
								.SetOrbitSpeed(0.1f).SetTargetDistance(0.3f + rand.nextDouble() * 0.3));
				particle.AddParticleController(
						new ParticleFadeOut(particle, 1, false).setFadeSpeed(0.05f).setKillParticleOnFinish(true));
