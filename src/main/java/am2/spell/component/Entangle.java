...
public class Entangle extends SpellComponent implements IRitualInteraction {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		if (target instanceof EntityLivingBase) {
			int duration = 140 + SpellUtils.getModifiedInt_Mul(PotionEffectsDefs.default_buff_duration / 10, stack,
					caster, target, world, SpellModifiers.DURATION);
			...
			if (!world.isRemote)
				((EntityLivingBase) target).addPotionEffect(
						new BuffEffectEntangled(duration, SpellUtils.countModifiers(SpellModifiers.BUFF_POWER, stack)));
			return true;
		}
		return false;
	}
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.GREEN.getDyeDamage()), Blocks.VINE,
				Items.SLIME_BALL };
	}
