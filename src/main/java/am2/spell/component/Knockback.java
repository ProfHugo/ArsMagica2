...
public class Knockback extends SpellComponent {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		...
		if (target instanceof EntityLivingBase) {
			double speed = 1.5;
			speed = SpellUtils.getModifiedDouble_Add(speed, stack, caster, target, world,
			...
			if (curEntity instanceof EntityPlayer) {
				AMNetHandler.INSTANCE.sendVelocityAddPacket(world, curEntity, speed * Math.cos(radians), vertSpeed,
		...
		for (int i = 0; i < 25; ++i) {
			...
			if (particle != null) {
				...
				particle.AddParticleController(
						new ParticleMoveOnHeading(particle, angle, 0, 0.1 + rand.nextDouble() * 0.5, 1, false));
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.YELLOW.getDyeDamage()), Blocks.PISTON };
	}
