...
public class Flight extends SpellComponent implements IRitualInteraction {
		...
		if (target instanceof EntityLivingBase) {
			...
			if (!world.isRemote)
				((EntityLivingBase) target).addPotionEffect(
						new BuffEffectFlight(duration, SpellUtils.countModifiers(SpellModifiers.BUFF_POWER, stack)));
			return true;
		}
		return false;
	}
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
		...
		for (int i = 0; i < 15; ++i) {
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(world,
					rand.nextBoolean() ? "wind" : "ember", x, y, z);
			if (particle != null) {
				particle.addRandomOffset(1, 0.5, 1);
				particle.AddParticleController(
						new ParticleOrbitEntity(particle, target, 0.2f + rand.nextFloat() * 0.2f, 1, false));
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.WHITE.getDyeDamage()), Items.NETHER_STAR,
				Items.GHAST_TEAR };
	}
	...
	public ItemStack[] getReagents() {
		return new ItemStack[] { new ItemStack(Items.FEATHER), AffinityShiftUtils.getEssenceForAffinity(Affinity.AIR),
