...
public class Dispel extends SpellComponent {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		...
		// TODO:
		/*
		 * if (ExtendedProperties.For((EntityLivingBase)target).getNumSummons()
		 * > 0){ if (!world.isRemote){ Iterator it =
		 * world.loadedEntityList.iterator(); while (it.hasNext()){ Entity ent =
		 * (Entity)it.next(); if (ent instanceof EntitySummonedCreature &&
		 * ((EntitySummonedCreature)ent).GetOwningEntity() == target){
		 * ent.attackEntityFrom(DamageSource.outOfWorld, 5000); break; } } } }
		 */
		return true;
	}
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.PURPLE.getDyeDamage()),
				new ItemStack(ItemDefs.itemOre, 1, ItemOre.META_ARCANEASH),
				new ItemStack(ItemDefs.itemOre, 1, ItemOre.META_BLUE_TOPAZ), Items.MILK_BUCKET };
	}
