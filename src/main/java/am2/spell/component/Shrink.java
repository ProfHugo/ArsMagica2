...
public class Shrink extends SpellComponent implements IRitualInteraction {
	...
	public Object[] getRecipe() {
		return new Object[] { Blocks.BROWN_MUSHROOM, Blocks.STONE_BUTTON, Items.BONE, Items.GOLD_NUGGET,
				Items.SPIDER_EYE, ItemDefs.manaCake };
	}
		...
		if (target instanceof EntityLivingBase && target.isNonBoss()) {
			...
			if (!world.isRemote)
				((EntityLivingBase) target).addPotionEffect(
						new BuffEffectShrink(duration, SpellUtils.countModifiers(SpellModifiers.BUFF_POWER, stack)));
			return true;
		}
		return false;
	}
	...
	public ItemStack[] getReagents() {
		return new ItemStack[] { new ItemStack(Items.BLAZE_ROD) };
	}
