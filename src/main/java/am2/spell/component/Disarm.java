...
public class Disarm extends SpellComponent {
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		...
		if (target instanceof EntityDarkMage && !world.isRemote) {
			...
			if (dropstack.getMaxDamage() > 0)
				dropstack.setItemDamage(
						(int) Math.floor(dropstack.getMaxDamage() * (0.8f + (world.rand.nextFloat() * 0.19f))));
		...
		if (target instanceof EntityPlayer && (!ArsMagica2.config.getDisarmAffectsPlayers()
				|| (!world.isRemote && !FMLCommonHandler.instance().getMinecraftServerInstance().isPVPEnabled())))
			return false;

		if (target instanceof EntityPlayer && ((EntityPlayer) target).getHeldItemOffhand() != null
				&& !target.worldObj.isRemote && (rnd.nextInt(9) + 1 <= damage)
				&& EnchantmentHelper.getEnchantmentLevel(AMEnchantments.soulbound,
						((EntityPlayer) target).getHeldItemOffhand()) <= 0) {
			// Drop mainhand item? 1 line. You want to drop the offhand? You'd
			// better like workarounds.
			EntityItem item = new EntityItem(world);
		...
		if (target instanceof EntityPlayer && ((EntityPlayer) target).getHeldItemMainhand() != null
				&& !target.worldObj.isRemote) {
			if (EnchantmentHelper.getEnchantmentLevel(AMEnchantments.soulbound,
					((EntityPlayer) target).getHeldItemMainhand()) > 0)
				return true;
			...
			if (blockID != null) {
				...
				ItemStack dropstack = new ItemStack(blockID.getBlock(), 1,
						blockID.getBlock().getMetaFromState(blockID));
				EntityItem item = new EntityItem(world);
				item.setEntityItemStack(dropstack);
				item.setPosition(target.posX, target.posY, target.posZ);
				world.spawnEntityInWorld(item);
			...
			if (EnchantmentHelper.getEnchantmentLevel(AMEnchantments.soulbound,
					((EntityMob) target).getActiveItemStack()) > 0)
				return true;
			if (!world.isRemote) {
				EntityItem item = new EntityItem(world);
				ItemStack dropstack = ((EntityMob) target).getHeldItemMainhand().copy();
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
		...
		for (int i = 0; i < 25; ++i) {
			...
			if (particle != null) {
				...
				particle.AddParticleController(new ParticleMoveOnHeading(particle,
						MathHelper.wrapDegrees((target instanceof EntityLivingBase
								? ((EntityLivingBase) target).rotationYawHead : target.rotationYaw) + 90),
						MathHelper.wrapDegrees(target.rotationPitch), 0.1 + rand.nextDouble() * 0.5, 1, false));
	...
	public Object[] getRecipe() {
		return new Object[] { new ItemStack(ItemDefs.rune, 1, EnumDyeColor.ORANGE.getDyeDamage()), Items.IRON_SWORD };
	}
