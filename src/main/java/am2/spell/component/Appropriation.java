...
public class Appropriation extends SpellComponent {
	...
	public Object[] getRecipe() {
		return new Object[] { Items.ENDER_PEARL, new ItemStack(BlockDefs.blocks, 1, EnumBlockType.CHIMERITE.ordinal()),
				Blocks.CHEST };
	}
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
		...
		if (!world.isRemote) {
			if (originalSpellStack.getTagCompound().hasKey(storageKey)) {
				restore((EntityPlayer) caster, world, originalSpellStack, target.getPosition(), target.posX,
	...
	private ItemStack getOriginalSpellStack(EntityPlayer caster) {
		...
		if (originalSpellStack == null)
			...
			originalSpellStack = ((ItemSpellBook) originalSpellStack.getItem()).GetActiveItemStack(originalSpellStack); // it's
																														...
																														// active
																														// scroll
			// sanity check needed here because from cast to apply the spell
			// could have changed - just ensure appropriation is a part of this
			// spell somewhere so that any stored item can be retrieved
	...
	private void restore(EntityPlayer player, World world, ItemStack stack, BlockPos pos, double hitX, double hitY,
			double hitZ) {
		if (stack.getTagCompound().hasKey(storageKey)) {
			...
			if (storageCompound != null) {
				...
				if (type.equals("ent")) {
					...
					// String blockName =
					// storageCompound.getString("blockName");
					...
					if (block != null) {
						...
						if (!player.worldObj.isRemote)
							player.addChatComponentMessage(
									new TextComponentString(I18n.translateToLocal("am2.tooltip.approError")));
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
		...
		for (int i = 0; i < 5 + 5 * ArsMagica2.config.getGFXLevel(); ++i) {
			...
			if (particle != null) {
				...
				particle.AddParticleController(new ParticleOrbitPoint(particle, x, y, z, 1, false)
						.SetTargetDistance(world.rand.nextDouble() + 0.1f).SetOrbitSpeed(0.2f));
			}
		}
	}
			...
			if (originalSpellStack.getTagCompound().hasKey(storageKey)) {
				...
				if (world.isAirBlock(blockPos) || !world.getBlockState(blockPos).getMaterial().isSolid()) {
					...
					if (blockSnapshots.size() > 1) {
						...
						placeEvent = ForgeEventFactory.onPlayerBlockPlace(casterPlayer, blockSnapshots.get(0),
								blockFace);
					}
					...
					if (placeEvent != null && (placeEvent.isCanceled())) {
						...
						for (net.minecraftforge.common.util.BlockSnapshot blocksnapshot : blockSnapshots) {
							...
							if (newBlock != null && !(newBlock.getBlock().hasTileEntity(newBlock))) { // Containers
																										// get
																										// placed
																										// automatically
					...
					// restore((EntityPlayer)caster, world, originalSpellStack,
					// blockx, blocky, blockz, impactX, impactY, impactZ);
				}
			} else {
				...
				int event = ForgeHooks.onBlockBreakEvent(world, casterPlayer.interactionManager.getGameType(),
						casterPlayer, blockPos);
				if (event == -1) {
					return false;
				}
				...
				if (te != null) {
					...
					// remove tile entity first to prevent content dropping
					// which is already saved in the NBT
					try {
