...
public class SpellValidator {
	...
	private enum StageValidations {
		VALID, TERMINUS, PRINCIPUM, NOT_VALID
	}
	...
	public ValidationResult spellDefIsValid(ArrayList<ArrayList<AbstractSpellPart>> shapeGroups,
			ArrayList<ArrayList<AbstractSpellPart>> segmented) {
	...
	private ValidationResult internalValidation(ArrayList<ArrayList<AbstractSpellPart>> segmented) {
		for (int i = 0; i < segmented.size(); ++i) {
			...
			if (result == StageValidations.NOT_VALID) {
				return new ValidationResult(segmented.get(i).get(0),
						I18n.translateToLocal("am2.spell.validate.compMiss"));
			} else if (result == StageValidations.PRINCIPUM && i == segmented.size() - 1) {
				return new ValidationResult(segmented.get(i).get(0),
						String.format("%s %s", SpellRegistry.getSkillFromPart(segmented.get(i).get(0)),
								I18n.translateToLocal("am2.spell.validate.principum")));
			} else if (result == StageValidations.TERMINUS && i < segmented.size() - 1) {
				return new ValidationResult(segmented.get(i).get(0),
						String.format("%s %s", SpellRegistry.getSkillFromPart(segmented.get(i).get(0)),
								I18n.translateToLocal("am2.spell.validate.terminus")));
			}
		}
	...
	public static ArrayList<ArrayList<AbstractSpellPart>> splitToStages(ArrayList<AbstractSpellPart> currentRecipe) {
		...
		for (int i = 0; i < currentRecipe.size(); ++i) {
			...
			if (part instanceof SpellShape)
				idx++;
			if (segmented.size() - 1 < idx) // while loop not necessary as this
											// will keep up
