...
public class AMRibbon extends Particle {
	...
	@Override
	public void renderParticle(VertexBuffer par1Tessellator, Entity ent, float partialframe, float cosyaw,
		...
		double d = (this.prevPosX + (this.posX - this.prevPosX) * partialframe)
				- Minecraft.getMinecraft().getRenderManager().viewerPosX;
		double d1 = (this.prevPosY + (this.posY - this.prevPosY) * partialframe)
				- Minecraft.getMinecraft().getRenderManager().viewerPosY;
		double d2 = (this.prevPosZ + (this.posZ - this.prevPosZ) * partialframe)
	...
	void addRibbonCurve() {
		...
		Vec3d lastMidPt = new Vec3d((curPt.xCoord + lastPt.xCoord) / 2, (curPt.yCoord + lastPt.yCoord) / 2,
				(curPt.zCoord + lastPt.zCoord) / 2);

		Vec3d midPt = new Vec3d((curPt.xCoord + nextPt.xCoord) / 2, (curPt.yCoord + nextPt.yCoord) / 2,
				(curPt.zCoord + nextPt.zCoord) / 2);

		/*
		 * float width = 0.00003F * (getRelativeViewVector(midPt).length()) *
		 * 1.5f; if (width > 0.2f) width = 0.2f; if (width < 0.1f) width = 0.1f;
		 */
	...
	// return new Vec3d((float)renderentity.posX - pos.xCoord,
	// (float)renderentity.posY - pos.yCoord, (float)renderentity.posZ -
	// pos.zCoord);
	// }
