...
public class Quad3D {
	...
	Quad3D(Vec3d p0, Vec3d p1, Vec3d p2, Vec3d p3, TextureAtlasSprite icon) {
		...
		avg = new Vec3d((p0.xCoord + p1.xCoord + p2.xCoord + p3.xCoord) / 4,
				(p0.yCoord + p1.yCoord + p2.yCoord + p3.yCoord) / 4D,
				(p0.zCoord + p1.zCoord + p2.zCoord + p3.zCoord) / 4D);
	...
	void draw() {
		...
		Vec3d vecOffset = new Vec3d(rand.nextDouble() * mul - halfMul, rand.nextDouble() * mul - halfMul,
				rand.nextDouble() * mul - halfMul);
		p0.add(vecOffset);
		p1.add(vecOffset);
		p2.add(vecOffset);
		p3.add(vecOffset);
