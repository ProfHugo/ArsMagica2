...
public class ParticleArcToPoint extends ParticleController {
	...
	public ParticleArcToPoint(AMParticle particleEffect, int priority, double startX, double startY, double startZ,
			double endX, double endY, double endZ, boolean exclusive) {
	...
	public ParticleArcToPoint(AMParticle particleEffect, int priority, double endX, double endY, double endZ,
			boolean exclusive) {
		this(particleEffect, priority, particleEffect.getPosX(), particleEffect.getPosY(), particleEffect.getPosZ(),
				endX, endY, endZ, exclusive);
	}

	public ParticleArcToPoint generateControlPoints() {
		firstControl = new Vec3d(start.xCoord + ((target.xCoord - start.xCoord) / 3),
				start.yCoord + ((target.yCoord - start.yCoord) / 3),
				start.zCoord + ((target.zCoord - start.zCoord) / 3));

		secondControl = new Vec3d(start.xCoord + ((target.xCoord - start.xCoord) / 3 * 2),
	...
	public ParticleController clone() {
		return new ParticleArcToPoint(particle, priority, target.xCoord, target.yCoord, target.zCoord, exclusive)
				.SetSpeed(speed).specifyControlPoints(firstControl, secondControl);
	}
