...
public class ParticleLeaveParticleTrail extends ParticleController {
	...
	public ParticleLeaveParticleTrail(AMParticle particleEffect, String particleName, boolean ignoreMaxAge, int maxAge,
	...
	public void doUpdate() {
		...
		if (updateTicks == ticksBetweenSpawns) {
			updateTicks = 0;
			AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(particle.getWorldObj(),
					particleName, particle.getPosX(), particle.getPosY(), particle.getPosZ());
			if (effect != null) {
				...
				for (ParticleController pmc : this.controllers) {
					effect.AddParticleController(pmc.clone().setKillParticleOnFinish(pmc.getKillParticleOnFinish())
	...
	public ParticleController clone() {
		ParticleLeaveParticleTrail clone = new ParticleLeaveParticleTrail(particle, particleName, ignoreMaxAge, maxAge,
				priority, exclusive).setParticleRGB_F(r, g, b).setTicksBetweenSpawns(this.ticksBetweenSpawns)
