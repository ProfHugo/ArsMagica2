...
public class AMLineArc extends Particle {
	...
	public AMLineArc(World world, double x, double y, double z, double targetX, double targetY, double targetZ,
			String IIconName) {
		super(world, x, y, z);
	...
	public AMLineArc(World world, double x, double y, double z, Entity targetEntity, String IIconName) {
		this(world, x, y, z, targetEntity.posX,
				targetEntity.posY + targetEntity.getEyeHeight() - (targetEntity.height * 0.2f), targetEntity.posZ,
				IIconName);
	...
	public AMLineArc(World world, Entity sourceEntity, Entity targetEntity, String IIconName) {
		this(world, sourceEntity.posX, sourceEntity.posY + sourceEntity.getEyeHeight() - (sourceEntity.height * 0.2f),
				sourceEntity.posZ, targetEntity.posX,
				targetEntity.posY + targetEntity.getEyeHeight() - (targetEntity.height * 0.2f), targetEntity.posZ,
				IIconName);
		this.targetEntity = targetEntity;
	...
	public void onUpdate() {
		...
		if (targetEntity != null) {
			...
			targetPoint = new Vec3d(targetEntity.posX,
					targetEntity.posY + targetEntity.getEyeHeight() - (targetEntity.height * 0.2f), targetEntity.posZ);
		...
		if (sourceEntity != null) {
			...
			sourcePoint = new Vec3d(sourceEntity.posX,
					sourceEntity.posY + sourceEntity.getEyeHeight() - (sourceEntity.height * 0.2f), sourceEntity.posZ);
		} else if (hadSource) {
			this.setExpired();
			return;
		}
	...
	@Override
	public void renderParticle(VertexBuffer par1Tessellator, Entity ent, float partialTicks, float rotationX,
			float rotationXZ, float rotationZ, float rotationYZ, float rotationXY) {
		if (targetEntity != null && sourceEntity != null) {
			drawArcingLine(sourceEntity.prevPosX + (sourceEntity.posX - sourceEntity.prevPosX) * partialTicks,
					(sourceEntity.prevPosY + (sourceEntity.posY - sourceEntity.prevPosY) * partialTicks)
							+ (sourceEntity.getEyeHeight() - (sourceEntity.height * 0.2f)),
					...
					(targetEntity.prevPosY + (targetEntity.posY - targetEntity.prevPosY) * partialTicks)
							+ (targetEntity.getEyeHeight() - (targetEntity.height * 0.2f)),
					targetEntity.prevPosZ + (targetEntity.posZ - targetEntity.prevPosZ) * partialTicks, partialTicks,
					speed, deviation);
		} else if (targetEntity != null) {
			drawArcingLine(prevPosX + (posX - prevPosX) * partialTicks, prevPosY + (posY - prevPosY) * partialTicks,
			...
			drawArcingLine(prevPosX + (posX - prevPosX) * partialTicks, prevPosY + (posY - prevPosY) * partialTicks,
					prevPosZ + (posZ - prevPosZ) * partialTicks, currentTargetPoint.xCoord, currentTargetPoint.yCoord,
					currentTargetPoint.zCoord, partialTicks, speed, deviation);
		}
	}

	public void drawArcingLine(double srcX, double srcY, double srcZ, double dstX, double dstY, double dstZ,
			float partialTicks, float speed, double distance) {
		...
		for (int i = 0; i <= length * distance; i++) {
			...
			// ZXY
			double dx = deltaX + MathHelper.sin(
					(float) ((srcX % 16.0D + dist * (1.0F - lengthFactor) * fxQuality / 2.0F - time % 32767.0F / 5.0F)
							/ 4.0D))
					* 0.5F * f3;
			double dy = deltaY + MathHelper.sin(
					(float) ((srcY % 16.0D + dist * (1.0F - lengthFactor) * fxQuality / 2.0F - time % 32767.0F / 5.0F)
							/ 3.0D))
					* 0.5F * f3;
			double dz = deltaZ + MathHelper.sin(
					(float) ((srcZ % 16.0D + dist * (1.0F - lengthFactor) * fxQuality / 2.0F - time % 32767.0F / 5.0F)
							/ 2.0D))
					* 0.5F * f3;
			...
			tessellator.getBuffer().pos(dx * lengthFactor - curWidth, dy * lengthFactor, dz * lengthFactor).tex(u, 1.0f)
					.endVertex();
			tessellator.getBuffer().pos(dx * lengthFactor + curWidth, dy * lengthFactor, dz * lengthFactor).tex(u, 0.0f)
					.endVertex();
