...
public class ParticleManagerServer {
	...
	public Object spawn(World world, String name, double x, double y, double z, double targetX, double targetY,
			double targetZ) {
		if (!world.isRemote) {
			...
			writer.add(targetY);
			writer.add(targetZ);
			AMNetHandler.INSTANCE.sendPacketToAllClientsNear(world.provider.getDimension(), x, y, z, 32,
					AMPacketIDs.PARTICLE_SPAWN_SPECIAL, writer.generate());
		}
		return null;
	}
	...
	public Object spawn(World world, String name, Entity source, Entity target) {
		if (!world.isRemote) {
			...
			writer.add(target.getEntityId());
			AMNetHandler.INSTANCE.sendPacketToAllClientsNear(world.provider.getDimension(), source.posX, source.posY,
					source.posZ, 32, AMPacketIDs.PARTICLE_SPAWN_SPECIAL, writer.generate());
		}
		return null;
	}
	...
	public void BoltFromEntityToEntity(World world, Entity caster, Entity source, Entity target, int damage, int type,
			int color) {
		if (!world.isRemote) {
			// send a packet to all clients telling them to spawn this lightning
			// bolt!
			AMDataWriter writer = new AMDataWriter();
			...
			writer.add(type);
			writer.add(color);
			AMNetHandler.INSTANCE.sendPacketToAllClientsNear(world.provider.getDimension(), caster.posX, caster.posY,
					caster.posZ, 32, AMPacketIDs.PARTICLE_SPAWN_SPECIAL, writer.generate());
		}
	}
	...
	public void BoltFromPointToPoint(World world, double startX, double startY, double startZ, double endX, double endY,
		...
		if (!world.isRemote) {
			...
			writer.add(type);
			writer.add(color);
			AMNetHandler.INSTANCE.sendPacketToAllClientsNear(world.provider.getDimension(), startX, startY, startZ, 32,
					AMPacketIDs.PARTICLE_SPAWN_SPECIAL, writer.generate());
		}
	}
	...
	public ParticleController createDefaultParticleController(int type, Object effect, Vec3d location, float modifier,
			int meta) {
		return null;
	}
}
