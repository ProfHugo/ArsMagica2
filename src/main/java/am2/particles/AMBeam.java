...
public class AMBeam extends Particle implements IBeamParticle {
	...
	public void setBeamLocationAndTarget(double posX, double posY, double posZ, double targetX, double targetY,
			double targetZ) {
		this.updateX = posX;
		this.updateY = posY;
		this.updateZ = posZ;
	...
	@Override
	public void renderParticle(VertexBuffer tessellator, Entity ent, float par2, float par3, float par4, float par5,
		...
		float rot = this.worldObj.provider.getWorldTime() % (360 / this.rotateSpeed) * this.rotateSpeed
				+ this.rotateSpeed * par2;
		...
		for (int t = 0; t < i; t++) {
			...
			double mV = beamIcon.getMinV(); // this.length * size * scaleFactor
											// + mU;

			GL11.glRotatef(inc, 0.0F, 1.0F, 0.0F);
			GlStateManager.resetColor();
			// if (t % 2 == 0){
			// GL11.glColor4f(this.particleRed, this.particleGreen,
			...
			int j = b >> 16 & 65535;
			int k = b & 65535;
			tessellator.pos(offset3, l, 0.0D).tex(br, mV)
					.color(this.particleRed, this.particleGreen, this.particleBlue, op).lightmap(j, k).endVertex();
			tessellator.pos(offset1, 0.0D, 0.0D).tex(br, mU)
					.color(this.particleRed, this.particleGreen, this.particleBlue, op).lightmap(j, k).endVertex();
			tessellator.pos(offset2, 0.0D, 0.0D).tex(tl, mU)
					.color(this.particleRed, this.particleGreen, this.particleBlue, op).lightmap(j, k).endVertex();
			tessellator.pos(offset4, l, 0.0D).tex(tl, mV)
					.color(this.particleRed, this.particleGreen, this.particleBlue, op).lightmap(j, k).endVertex();
			Tessellator.getInstance().draw();
			tessellator.begin(7, DefaultVertexFormats.PARTICLE_POSITION_TEX_COLOR_LMAP);
		}
