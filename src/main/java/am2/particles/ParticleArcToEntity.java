...
public class ParticleArcToEntity extends ParticleController {
	...
	public ParticleArcToEntity(AMParticle particleEffect, int priority, double startX, double startY, double startZ,
			Entity target, boolean exclusive) {
	...
	public ParticleArcToEntity(AMParticle particleEffect, int priority, Entity target, boolean exclusive) {
		this(particleEffect, priority, particleEffect.getPosX(), particleEffect.getPosY(), particleEffect.getPosZ(),
				target, exclusive);
	}

	public ParticleArcToEntity generateControlPoints() {
		firstControl = new Vec3d(start.xCoord + ((target.posX - start.xCoord) / 3),
				start.yCoord + ((target.posY - start.yCoord) / 3), start.zCoord + ((target.posZ - start.zCoord) / 3));

		secondControl = new Vec3d(start.xCoord + ((target.posX - start.xCoord) / 3 * 2),
	...
	private void addParticleAtPoint(Vec3d point) {
		AMParticle p = (AMParticle) ArsMagica2.proxy.particleManager.spawn(particle.getWorldObj(), "smoke",
	...
	public void doUpdate() {
		...
		Vec3d bez = MathUtilities.bezier(start, firstControl, secondControl,
				new Vec3d(target.posX, target.posY, target.posZ).add(new Vec3d(0.0, target.getEyeHeight(), 0.0)),
				percent);
