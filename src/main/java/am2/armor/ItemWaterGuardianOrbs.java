...
public class ItemWaterGuardianOrbs extends AMArmor {
	...
	public ItemWaterGuardianOrbs(ArmorMaterial inheritFrom, ArsMagicaArmorMaterial enumarmormaterial, int par3,
	...
	public void onArmorTick(World world, EntityPlayer plr, ItemStack stack) {
		...
		if (!plr.getActivePotionEffects().contains(PotionTypes.WATER_BREATHING))
			plr.addPotionEffect(new PotionEffect(
					Potion.getPotionFromResourceLocation(new ResourceLocation("water_breathing").toString())));
		...
		for (int k1 = minX; k1 < maxX; ++k1)
			for (int l1 = minY; l1 < maxY; ++l1)
				for (int i2 = minZ; i2 < maxZ; ++i2) {
					...
					Boolean result = block.isEntityInsideMaterial(world, blockpos$pooledmutableblockpos, iblockstate,
							entityIn, (double) maxY, Material.WATER, false);
