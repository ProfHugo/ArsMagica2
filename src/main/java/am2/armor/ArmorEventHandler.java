...
public class ArmorEventHandler {
	...
	public void onEntityHurt(LivingHurtEvent event) {
		...
		if (event.getEntityLiving() instanceof EntityPlayer)
			doXPInfusion((EntityPlayer) event.getEntityLiving(), 0.01f,
					Math.max(0.05f, Math.min(event.getAmount(), 5)));
	}
	...
	public void onEntityDeath(LivingDeathEvent event) {
		if (event.getSource().getEntity() instanceof EntityPlayer)
			doXPInfusion((EntityPlayer) event.getSource().getEntity(), 1,
					Math.min(20, event.getEntityLiving().getMaxHealth()));
							...
							if (inf.getCooldown() > 0) {
								if (props.getCooldown(inf.getRegistryName().toString()) < inf.getCooldown()) {
									...
									if (player instanceof EntityPlayerMP)
										ArsMagica2.proxy.blackoutArmorPiece((EntityPlayerMP) player, slot,
												inf.getCooldown());
								}
							}
						}
	...
	private void doXPInfusion(EntityPlayer player, float xpMin, float xpMax) {
		float amt = (float) ((player.worldObj.rand.nextFloat() * xpMin + (xpMax - xpMin))
				* ArsMagica2.config.getArmorXPInfusionFactor());
	...
	public void onItemTooltip(ItemTooltipEvent event) {
		...
		if (stack != null && stack.getItem() instanceof ItemArmor) {
			...
			if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
				event.getToolTip()
						.add(I18n.translateToLocalFormatted("am2.tooltip.armorxp", String.format("%.2f", xp)));
				...
				for (String s : effects) {
					event.getToolTip()
							.add("-" + I18n.translateToLocal("am2.tooltip." + s.replaceAll("arsmagica2:", "")));
				}
