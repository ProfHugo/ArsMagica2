...
public class ArmorHelper {
	...
	// private static final HashMap<String, Integer> armorRenderPrefixes = new
	...
	private static void RepairEquippedArsMagicaArmorItem(EntityPlayer player, int repairAmount,
	...
	private static boolean PlayerHasArsInfusableInSlot(EntityPlayer player, EntityEquipmentSlot armorSlot) {
		return (player.getItemStackFromSlot(armorSlot) != null
				&& (player.getItemStackFromSlot(armorSlot).getItem() instanceof AMArmor));
	}
	...
	// private static ItemStack GetAMProtectiveArmorInSlot(EntityPlayer player,
	// int armorSlot){
		...
		if (armorStack != null && imbuement != null && armorStack.getItem() instanceof ItemArmor) {
			...
			for (EntityEquipmentSlot i : imbuement.getValidSlots()) {
				if (i == ((ItemArmor) armorStack.getItem()).armorType) {
					...
					NBTTagCompound armorProps = (NBTTagCompound) armorStack.getTagCompound()
	...
	public static void deductXPFromArmor(float amt, ItemStack armor) {
		if (armor != null && armor.getItem() instanceof ItemArmor) {
			...
			if (armorProps == null)
				armorProps = new NBTTagCompound();
			armorProps.setDouble(AMArmor.NBT_KEY_TOTALXP,
					Math.max(armorProps.getDouble(AMArmor.NBT_KEY_TOTALXP) - amt, 0));
			armorProps.setInteger(AMArmor.NBT_KEY_ARMORLEVEL,
					EntityUtils.getLevelFromXP((float) armorProps.getDouble(AMArmor.NBT_KEY_TOTALXP)));
