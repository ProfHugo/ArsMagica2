...
public class RitualShapeHelper {
	...
	public boolean matchesRitual(IRitualInteraction ritual, World world, BlockPos pos) {
		List<EntityItem> items = world.getEntitiesWithinAABB(EntityItem.class,
				new AxisAlignedBB(pos, pos.add(1, 1, 1)).expand(ritual.getReagentSearchRadius(),
						ritual.getReagentSearchRadius(), ritual.getReagentSearchRadius()));
		...
		for (ItemStack stack : ritual.getReagents()) {
			...
			for (EntityItem item : items) {
				...
				if (is.getItem().equals(stack.getItem()) && (stack.getMetadata() == OreDictionary.WILDCARD_VALUE
						|| is.getMetadata() == stack.getMetadata()) && is.stackSize >= stack.stackSize)
					matches = true;
			}
			if (!matches)
				return false;
	...
	public ItemStack[] checkForRitual(IRitualInteraction ritual, World world, BlockPos pos) {
		ArrayList<EntityItem> items = (ArrayList<EntityItem>) world.getEntitiesWithinAABB(EntityItem.class,
				new AxisAlignedBB(pos, pos.add(1, 1, 1)).expand(ritual.getReagentSearchRadius(),
						ritual.getReagentSearchRadius(), ritual.getReagentSearchRadius()));
	...
	public void consumeReagents(IRitualInteraction ritual, World world, BlockPos pos) {
		...
		for (ItemStack stack : ritual.getReagents()) {
			for (EntityItem item : items) {
				ItemStack is = item.getEntityItem();
				if (is.getItem().equals(stack.getItem()) && is.getMetadata() == stack.getMetadata()
	...
	public void consumeAllReagents(IRitualInteraction interaction, World world, BlockPos pos) {
		...
		List<EntityItem> items = world.getEntitiesWithinAABB(EntityItem.class,
				new AxisAlignedBB(pos, pos.add(1, 1, 1)).expand(r, r, r));
		for (EntityItem item : items) {
	...
	private void corruptionRitual() {
		...
		TypedMultiblockGroup defaultRotation = new TypedMultiblockGroup("defaultRotation",
				Lists.newArrayList(corruptionMap), true);
	...
	private void purificationRitual() {
		MultiblockGroup chalks = new MultiblockGroup("chalk",
				Lists.newArrayList(BlockDefs.wizardChalk.getDefaultState()), true);
		MultiblockGroup candles = new MultiblockGroup("candle",
				Lists.newArrayList(BlockDefs.wardingCandle.getDefaultState()), true);
	...
	private void hourglassRitual() {
		MultiblockGroup chalks = new MultiblockGroup("chalk",
				Lists.newArrayList(BlockDefs.wizardChalk.getDefaultState()), true);
		MultiblockGroup chalks_rotated = new MultiblockGroup("chalk_rot",
				Lists.newArrayList(BlockDefs.wizardChalk.getDefaultState()), true);
