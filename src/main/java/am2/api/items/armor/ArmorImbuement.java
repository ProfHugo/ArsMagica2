...
public abstract class ArmorImbuement extends IForgeRegistryEntry.Impl<ArmorImbuement> {
	...
	/**
	 * Applies the effect. This will be called when any of the application types
	 * is matched in an event.
	 *
	 * @param player
	 ...
	 * @param stack
	 *            The itemstack that was matched
	 * @param matchedType
	 *            The application type that was matched.
	 * @param params
	 *            Depends on the application type. <br/>
	 *            In the case of ON_TICK, it will be a 1-length array with the
	 *            first element being the {@link LivingEvent} event <br/>
	 *            In the case of ON_JUMP, it will be a 1-length array with the
	 *            first element being the {@link LivingJumpEvent} event<br/>
	 *            In the case of ON_HIT, it will be a 1-length array with the
	 *            first element being the {@link LivingHurtEvent} event<br/>
	 *            In the case of ON_MINING_SPEED, it will be a 1-length array
	 *            with the first element being the {@link BreakSpeed} event
	 *            <br/>
	 */
	public abstract boolean applyEffect(EntityPlayer player, World world, ItemStack stack,
			ImbuementApplicationTypes matchedType, Object... params);

	public boolean canApply(EntityPlayer player) {
		if (canApplyOnCooldown())
			return true;
		if (AffinityData.For(player).getCooldown(this.getRegistryName().toString()) == 0)
			return true;
		return false;
	}
