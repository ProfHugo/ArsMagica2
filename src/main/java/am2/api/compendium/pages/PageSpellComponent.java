...
public class PageSpellComponent extends CompendiumPage<AbstractSpellPart> {
	...
	protected void renderPage(int posX, int posY, int mouseX, int mouseY) {
		...
		if (mouseX > cx && mouseX < cx + 16) {
			if (mouseY > cy && mouseY < cy + 16) {
				stackTip = new ItemStack(ItemDefs.spell_component, 1,
						ArsMagicaAPI.getSkillRegistry().getId(element.getRegistryName()));
				tipX = mouseX;
				tipY = mouseY;
			}
		}
		{
			mc.renderEngine
					.bindTexture(new ResourceLocation("arsmagica2", "textures/gui/ArcaneCompendiumGuiExtras.png"));
			zLevel++;
	...
	private void renderModifiers(int posX, int posY, int mouseX, int mouseY) {
		...
		if (!modifiers.isEmpty()) {
			String shapeName = I18n
					.translateToLocal(element instanceof SpellComponent ? "am2.gui.modifies" : "am2.gui.modifiedBy");
			mc.fontRendererObj.drawString(shapeName, posX + 72 - (mc.fontRendererObj.getStringWidth(shapeName) / 2),
					posY, 0);
		...
		for (SpellModifier mod : modifiers) {
			...
			if (mouseX > posX + startX && mouseX < posX + startX + 16) {
				if (mouseY > posY + yOffset && mouseY < posY + yOffset + 16) {
					stackTip = new ItemStack(ItemDefs.spell_component, 1,
							ArsMagicaAPI.getSkillRegistry().getId(mod.getRegistryName()));
					tipX = mouseX;
					tipY = mouseY;
				}
			}
	...
	private void renderCraftingComponent(int index, int sx, int sy, int mousex, int mousey) {
		...
		if (alternates.size() > 0) {
			stack = alternates.get(
					new Random(new Random(AMGuiHelper.instance.getSlowTicker()).nextLong()).nextInt(alternates.size()));
		}
