...
public class PageRitual extends CompendiumPage<IRitualInteraction.Wrapper> {
	...
	protected void renderPage(int posX, int posY, int mouseX, int mouseY) {
		...
		if (element.getRitualInteraction() instanceof AbstractSpellPart)
			ritualName = TextFormatting.UNDERLINE.toString() + TextFormatting.BOLD.toString()
					+ I18n.translateToLocal("ritual."
							+ ((AbstractSpellPart) element.getRitualInteraction()).getRegistryName().toString()
							+ ".name");
		else
			ritualName = TextFormatting.UNDERLINE.toString() + TextFormatting.BOLD.toString() + I18n.translateToLocal(
					"ritual." + element.getRitualInteraction().getClass().getSimpleName().toLowerCase() + ".name");
		mc.fontRendererObj.drawString(ritualName, posX + 72 - (mc.fontRendererObj.getStringWidth(ritualName) / 2), posY,
				0);
		String categoryName = TextFormatting.UNDERLINE.toString() + I18n.translateToLocal("am2.gui.ritualshape");
		mc.fontRendererObj.drawString(categoryName, posX + 72 - (mc.fontRendererObj.getStringWidth(categoryName) / 2),
				posY + 20, 0);
		// String shapeName = I18n.translateToLocal("ritualshape." +
		// element.getRitualInteraction().getRitualShape().getId() + ".name");
		// mc.fontRendererObj.drawString(shapeName, posX + 72 -
		// (mc.fontRendererObj.getStringWidth(shapeName) / 2), posY + 30, 0);

		String reagentsName = TextFormatting.UNDERLINE.toString() + I18n.translateToLocal("am2.gui.ritualreagents");
		mc.fontRendererObj.drawString(reagentsName, posX + 72 - (mc.fontRendererObj.getStringWidth(reagentsName) / 2),
				posY + 50, 0);
		...
		for (int l = 0; l < lines; l++) {
			...
			for (int i = 0; i < items; i++) {
				...
				if (mouseX > posX + xOffset && mouseX < posX + xOffset + 16 && mouseY > posY + yOffset + 60
						&& mouseY < posY + yOffset + 76)
					stackTip = stack;
				xOffset += 32;
			}
			yOffset += 32;
		...
		if (element.getRitualInteraction().getResult() != null) {
			...
			mc.fontRendererObj.drawString(resultName, posX + 72 - (mc.fontRendererObj.getStringWidth(resultName) / 2),
					posY + 165, 0);
	...
	public void actionPerformed(GuiButton button) throws IOException {
		if (button == shapeButton) {
			...
			Minecraft.getMinecraft()
					.displayGuiScreen(new GuiArcaneCompendium(new CompendiumEntry(null, "shape." + def.getId())
							.addObject("compendium.shape." + def.getId() + ".page1").addObject(def)));
		}
	}
	...
	public GuiButton[] getButtons(int id, int posX, int posY) {
		String shapeName = I18n
				.translateToLocal("ritualshape." + element.getRitualInteraction().getRitualShape().getId() + ".name");
		...
		((GuiButtonVariableDims) shapeButton)
				.setPosition(posX + 72 - (mc.fontRendererObj.getStringWidth(shapeName) / 2), posY + 55);
