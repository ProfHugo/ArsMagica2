...
public class PageEntity extends CompendiumPage<Entity> {
	...
	protected void renderPage(int posX, int posY, int mouseX, int mouseY) {
		...
		try {
			...
			GlStateManager.translate((float) (cx - 2), (float) (cy + 20),
					-3.0F + Minecraft.getMinecraft().getRenderItem().zLevel);
			...
			try {
				...
				Entity ent = element.getClass().getConstructor(World.class)
		...
		mc.fontRendererObj.drawString(renderString, posX + 72 - (mc.fontRendererObj.getStringWidth(renderString) / 2),
				posY + 200, 0x000000);
	}
