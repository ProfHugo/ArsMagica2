...
/**
 * This class is used in the flicker lure, it manages the different flicker
 * generation weights
 * 
 * @author Edwin
 *
...
public class FlickerGenerationPool {
	...
	public void addWeightedAffinity(Affinity aff, int weight) {
		if (map.containsKey(aff))
			LogHelper.warn("Override : %s is already registered, report to the mod author",
					aff.getRegistryName().toString());
		map.put(aff, weight);
	}
