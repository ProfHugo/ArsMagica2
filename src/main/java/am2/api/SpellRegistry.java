...
public class SpellRegistry {
	 ...
	 *            : Icon
	 * @param tier
	 *            : Skill Point required to unlock
	 * @param part
	 *            : Actual Component, use new {@link SpellComponent} ()
	 * @param tree
	 *            : Skill Tree
	 * @param posX
	 ...
	 *            : Position in the tree
	 * @param parents
	 *            : Skills that need to be unlocked before this one (occulus
	 *            only)
	 */
	public static void registerSpellComponent(String id, ResourceLocation icon, SkillPoint tier, SpellComponent part,
			SkillTree tree, int posX, int posY, String... parents) {
		id = id.toLowerCase();
		GameRegistry.register(part, new ResourceLocation(ArsMagicaAPI.getCurrentModId(), id));
		GameRegistry.register(new Skill(icon, tier, posX, posY, tree, parents),
				new ResourceLocation(ArsMagicaAPI.getCurrentModId(), id));
	}
	 ...
	 *            : Skill Point required to unlock
	 * @param part
	 *            : Actual Modifier, use new {@link SpellModifier} ()
	 * @param tree
	 *            : Skill Tree
	 * @param posX
	 *            : Position in the tree
	 ...
	 *            only)
	 */
	public static void registerSpellModifier(String id, ResourceLocation icon, SkillPoint tier, SpellModifier part,
			SkillTree tree, int posX, int posY, String... parents) {
		id = id.toLowerCase();
		GameRegistry.register(part, new ResourceLocation(ArsMagicaAPI.getCurrentModId(), id));
		GameRegistry.register(new Skill(icon, tier, posX, posY, tree, parents),
	 ...
	 *            : Skill Point required to unlock
	 * @param part
	 *            : Actual Shape, use new {@link SpellShape} ()
	 * @param tree
	 *            : Skill Tree
	 * @param posX
	 *            : Position in the tree
	 ...
	 *            only)
	 */
	public static void registerSpellShape(String id, ResourceLocation icon, SkillPoint tier, SpellShape part,
			SkillTree tree, int posX, int posY, String... parents) {
		id = id.toLowerCase();
		GameRegistry.register(part, new ResourceLocation(ArsMagicaAPI.getCurrentModId(), id));
		GameRegistry.register(new Skill(icon, tier, posX, posY, tree, parents),
		...
		for (AbstractSpellPart data : ArsMagicaAPI.getSpellRegistry().getValues()) {
			if (data != null && data.getRecipe() != null) {
				...
				for (int i = 0; i < convRecipe.size(); i++) {
					...
					match &= convRecipe.get(i).getTagCompound() == null ? true
							: (currentAddedItems.get(i).getTagCompound() == null ? false
									: NBTUtils.contains(convRecipe.get(i).getTagCompound(),
											currentAddedItems.get(i).getTagCompound()));
					if (!match)
						break;
				}
				if (!match)
