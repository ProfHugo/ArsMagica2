...
public class EntityHandler {
	...
	public void onPlayerMagicLevelChange(PlayerMagicLevelChangeEvent event) {
		if (event.getEntityPlayer() != null) {
			for (SkillPoint point : SkillPointRegistry.getSkillPointMap().values()) {
				if (point.getMinEarnLevel() > event.getLevel())
					continue;
				if ((event.getLevel() - point.getMinEarnLevel()) % point.getLevelsForPoint() == 0)
					SkillData.For(event.getEntityPlayer()).setSkillPoint(point,
							SkillData.For(event.getEntityPlayer()).getSkillPoint(point) + 1);
			}
		}
	}
	...
	public void onClonePlayer(PlayerEvent.Clone event) {
		transferCapability(AffinityData.INSTANCE, AffinityData.For(event.getOriginal()),
				AffinityData.For(event.getEntityPlayer()));
		transferCapability(EntityExtension.INSTANCE, EntityExtension.For(event.getOriginal()),
				EntityExtension.For(event.getEntityPlayer()));
		transferCapability(SkillData.INSTANCE, SkillData.For(event.getOriginal()),
				SkillData.For(event.getEntityPlayer()));
		transferCapability(RiftStorage.INSTANCE, RiftStorage.For(event.getOriginal()),
				RiftStorage.For(event.getEntityPlayer()));
		transferCapability(ArcaneCompendium.INSTANCE, ArcaneCompendium.For(event.getOriginal()),
				ArcaneCompendium.For(event.getEntityPlayer()));
	...
	private <T> void transferCapability(Capability<T> capability, T original, T target) {
		capability.getStorage().readNBT(capability, target, null,
				capability.getStorage().writeNBT(capability, original, null));
	}
	...
	public void entityTick(LivingUpdateEvent event) {
		...
		if (!event.getEntity().worldObj.isRemote && DataSyncExtension.For(event.getEntityLiving()).shouldSync())
			AMNetHandler.INSTANCE.sendPacketToAllClientsNear(event.getEntity().dimension, event.getEntity().posX,
					event.getEntity().posY, event.getEntity().posZ, 64, AMPacketIDs.SYNC_CLIENT,
					DataSyncExtension.For(event.getEntityLiving()).createUpdatePacket());

		if (event.getEntityLiving() instanceof EntityPlayer)
			playerTick((EntityPlayer) event.getEntityLiving());
		...
		if (event.getEntity().ticksExisted % 20 == 0) {
			...
			for (int i = 0; i < rs.size(); i++) {
				...
				if (is != null && is.getTagCompound() != null) {
					SpellCastResult result = SpellUtils.applyStackStage(is.copy(), ent, ent, ent.posX, ent.posY,
							ent.posZ, null, ent.worldObj, true, true, 0);
			...
			if (foundID != -1) {
				...
				if (ent instanceof EntityPlayer) {
					...
					for (int i = 0; i < inv.getSizeInventory(); i++) {
						...
						if (is2 != null && is2.getItem() instanceof SpellBase && is2.getTagCompound() != null
								&& is.getTagCompound().getString("ToggleShapeID")
										.equals(is2.getTagCompound().getString("ToggleShapeID"))) {
		...
		if (event.getEntityLiving().isPotionActive(PotionEffectsDefs.spellReflect)) {
			int d0 = 3;
			AxisAlignedBB bb = new AxisAlignedBB(event.getEntityLiving().posX - 0.5, event.getEntityLiving().posY - 0.5,
					event.getEntityLiving().posZ - 0.5, event.getEntityLiving().posX + 0.5,
					event.getEntityLiving().posY + 0.5, event.getEntityLiving().posZ + 0.5).expand(d0, d0, d0);
			List entityList = event.getEntityLiving().getEntityWorld().getEntitiesWithinAABB(Entity.class, bb);

			for (Object thing : entityList) {
				if (!(thing instanceof EntitySpellProjectile))
					continue;
				EntitySpellProjectile projectile = (EntitySpellProjectile) thing;
				if (projectile.getShooter() == event.getEntityLiving())
					continue;
				...
				double angle = (rX * projectile.motionX + rZ * projectile.motionZ) / (Math.sqrt(rX * rX + rZ * rZ)
						* Math.sqrt(projectile.motionX * projectile.motionX + projectile.motionZ * projectile.motionZ));
		...
		if (event.getEntityLiving().isPotionActive(PotionEffectsDefs.wateryGrave)
		...
		if (event.getEntityLiving().isBurning() && type == ContingencyType.FIRE) {
			SpellUtils.applyStackStage(ext.getContingencyStack(), event.getEntityLiving(), null, event.getEntity().posX,
					event.getEntity().posY, event.getEntity().posZ, null, event.getEntityLiving().worldObj, false, true,
					0);
			if (ext.getContingencyType() == ContingencyType.FIRE)
				ext.setContingency(ContingencyType.NULL, null);
		} else if (event.getEntityLiving().getHealth() * 4 < event.getEntityLiving().getMaxHealth()
				&& type == ContingencyType.HEALTH) {
			SpellUtils.applyStackStage(ext.getContingencyStack(), event.getEntityLiving(), null, event.getEntity().posX,
					event.getEntity().posY, event.getEntity().posZ, null, event.getEntityLiving().worldObj, false, true,
					0);
	...
	public void playerTick(EntityPlayer player) {
		...
		if (player.worldObj.isRemote)
			AMNetHandler.INSTANCE.sendPacketToServer(AMPacketIDs.PLAYER_FLIP,
					new AMDataWriter().add(ext.getIsFlipped()).generate());
		if (ext.getIsFlipped()) {
			if ((player).motionY < 2 && !player.capabilities.isFlying)
				(player).motionY += 0.15f;
			double posY = player.posY + player.height;
			World world = player.worldObj;
			RayTraceResult mop = world.rayTraceBlocks(new Vec3d(player.posX, posY, player.posZ),
					new Vec3d(player.posX, posY + 1, player.posZ), true);
			if (mop != null) {
				if (!player.onGround) {
					world.getBlockState(mop.getBlockPos()).getBlock().onFallenUpon(world, mop.getBlockPos(), player,
							Math.abs(player.fallDistance));
		...
		if (ArmorHelper.isInfusionPreset(player.getItemStackFromSlot(EntityEquipmentSlot.LEGS),
				GenericImbuement.stepAssist)) {
		...
		if (ArmorHelper.isInfusionPreset(player.getItemStackFromSlot(EntityEquipmentSlot.FEET),
				GenericImbuement.runSpeed)) {
	...
	public void entityDeath(LivingDeathEvent e) {
		...
		if (e.getSource() != null && e.getEntityLiving() != null && e.getEntityLiving() instanceof EntityLiving
				&& e.getSource().getEntity() instanceof EntityPlayer) {
			EntityPlayer player = (EntityPlayer) e.getSource().getEntity();
			for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
				ItemStack stack = player.inventory.getStackInSlot(i);
				if (stack == null || stack.getItem() != ItemDefs.crystalPhylactery)
					continue;
		...
		if (type == ContingencyType.DEATH) {
			SpellUtils.applyStackStage(ext.getContingencyStack(), e.getEntityLiving(), target, e.getEntity().posX,
					e.getEntity().posY, e.getEntity().posZ, null, e.getEntityLiving().worldObj, false, true, 0);
	...
	public void attackEntity(LivingAttackEvent e) {
		if (e.getEntityLiving() instanceof EntityPlayer) {
			...
			if (e.getAmount() > 0.0F && stack != null && stack.getItem() == ItemDefs.BoundShield
					&& EntityUtils.canBlockDamageSource(player, e.getSource())) {
				if (EntityExtension.For(player).hasEnoughtMana(e.getAmount() * 10)) {
					...
					EntityLivingBase target = e.getSource().getEntity() instanceof EntityLivingBase
							? (EntityLivingBase) e.getSource().getEntity() : null;
					...
					SpellUtils.applyStackStage(copiedStack, player, target, posX, posY, posZ, null, player.worldObj,
							true, true, 0);
				} else {
					stack.getItem().onDroppedByPlayer(stack, player);
				}
	...
	public void onPlayerRender(RenderPlayerEvent.Pre event) {
		...
		if (EntityExtension.For(event.getEntityPlayer()).getFlipRotation() > 0) {
			...
			float offset = event.getEntityPlayer().height
					* (EntityExtension.For(event.getEntityPlayer()).getFlipRotation() / 180.0f);
	...
	public void onLivingDrops(LivingDropsEvent event) {
		...
		if (!event.getEntityLiving().worldObj.isRemote && event.getEntityLiving() instanceof EntityPig
				&& event.getEntityLiving().getRNG().nextDouble() < 0.3f) {
	...
	public void onPlayerRender(RenderPlayerEvent.Post event) {
		...
		// CloakUtils.renderCloakModel(event.entityPlayer, mainModel,
	...
	public void onItemPickup(ItemPickupEvent event) {
		...
		if (!event.player.worldObj.isRemote && EntityExtension.For(event.player).getCurrentLevel() <= 0
				&& event.pickedUp.getEntityItem().getItem() == ItemDefs.arcaneCompendium) {
	...
	public void playerJumpEvent(LivingJumpEvent event) {
		if (event.getEntityLiving() instanceof EntityPlayer) {
			...
			if (boots != null && boots.getItem() == ItemDefs.enderBoots && event.getEntityLiving().isSneaking()) {
				EntityExtension.For(event.getEntityLiving())
						.setInverted(!EntityExtension.For(event.getEntityLiving()).isInverted());
			}
	...
	// if (event.getSource() == DamageSource.magic || event.getSource() ==
	// DamageSource.dragonBreath){
	// float newDamage = EnchantMagicResist.ApplyEnchantment(entity,
	// event.getAmount());
