	...
	public void onEntityLiving(LivingEvent event) {
		...
		if (exProps.getShrinkPct() > 0f) {
			if (exProps.shrinkAmount == 0f || // shrink hasn't yet been applied
					exProps.getOriginalSize().x * 0.5 != player.width || // width
																			// has
																			// changed
																			// through
																			// other
																			// means
					exProps.getOriginalSize().y * 0.5 != player.height) { // height
																			// has
																			// changed
																			// through
																			// other
		...
		if (player.worldObj.isRemote && exProps.getPrevShrinkPct() != exProps.getShrinkPct()) {
			...
			// player.yOffset = (float)exProps.getOriginalSize().y * 0.9f * (1f
			// - 0.5f * exProps.getShrinkPct());
		}
	}
}