			...
			for (int i = 0; i <= max; i++) {
				for (Entry<KeyValuePair<IBlockState, IBlockState>, Integer> entry : map.entrySet()) {
					if (entry.getValue() != null && entry.getValue().intValue() == i) {
						...
						if (item != null) {
							text += item.getItemStackDisplayName(new ItemStack(entry.getKey().key.getBlock(), 1,
									entry.getKey().key.getBlock().getMetaFromState(entry.getKey().key)));
							text += " ( +" + i + " Effects) !l ";
						}
					}
				}
			...
			for (int i = 0; i <= capsmax; i++) {
				for (Entry<IBlockState, Integer> entry : capsmap.entrySet()) {
					if (entry.getValue() != null && entry.getValue().intValue() == i) {
						...
						if (item != null) {
							text += item.getItemStackDisplayName(new ItemStack(entry.getKey().getBlock(), 1,
									entry.getKey().getBlock().getMetaFromState(entry.getKey())));
							text += " ( +" + i + " Effects) !l ";
						}
					}
				}
