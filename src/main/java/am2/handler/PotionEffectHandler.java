...
public class PotionEffectHandler {
	...
	public void entityDamageEvent(LivingHurtEvent event) {
		...
		if (event.getSource().damageType.equals(DamageSource.outOfWorld.damageType))
			return;
		...
		if (event.getSource().damageType.equals(DamageSource.magic.damageType)
				|| event.getSource().damageType.equals(DamageSource.dragonBreath.damageType)) {
	...
	public void livingUpdate(LivingUpdateEvent e) {
		...
		if (e.getEntityLiving().isPotionActive(PotionEffectsDefs.slowfall)) {
			e.getEntityLiving().setPosition(e.getEntityLiving().posX,
					e.getEntityLiving().posY + (e.getEntityLiving().fallDistance / 1.1), e.getEntityLiving().posZ);
		...
		if (event.getEntityLiving().isPotionActive(PotionEffectsDefs.leap)) {
			...
			if (event.getEntityLiving().getRidingEntity() != null) {
				if (event.getEntityLiving().getRidingEntity() instanceof EntityMinecart) {
					event.getEntityLiving().getRidingEntity().setPosition(
							event.getEntityLiving().getRidingEntity().posX,
							event.getEntityLiving().getRidingEntity().posY + 1.5,
							event.getEntityLiving().getRidingEntity().posZ);
				}
			...
			yVelocity = 0.4
					+ (event.getEntityLiving().getActivePotionEffect(PotionEffectsDefs.leap).getAmplifier() * 0.3);
			xVelocity = velocityTarget.motionX
					* (Math.pow(2,
							event.getEntityLiving().getActivePotionEffect(PotionEffectsDefs.leap).getAmplifier()))
					* Math.abs(vec.xCoord);
			zVelocity = velocityTarget.motionZ
					* (Math.pow(2,
							event.getEntityLiving().getActivePotionEffect(PotionEffectsDefs.leap).getAmplifier()))
					* Math.abs(vec.zCoord);
			...
			if (event.getEntityLiving().getRidingEntity() != null
					&& (event.getEntityLiving().getRidingEntity() instanceof EntityMinecart
							|| event.getEntityLiving().getRidingEntity() instanceof EntityBoat)
					|| event.getEntityLiving().isPotionActive(PotionEffectsDefs.haste)) {
	...
	public void livingFall(LivingFallEvent e) {
		...
		if (e.getEntityLiving().isPotionActive(PotionEffectsDefs.leap)) {
			if (e.getDistance() < (e.getEntityLiving().getActivePotionEffect(PotionEffectsDefs.leap).getAmplifier() + 1)
					* 10) {
				e.setCanceled(true);
			} else {
				e.setDistance(e.getDistance()
						- (e.getEntityLiving().getActivePotionEffect(PotionEffectsDefs.leap).getAmplifier() + 1) * 10);
			}
		}
	}
	...
	public void spellCast(SpellCastEvent.Pre e) {
		if (e.entityLiving.isPotionActive(PotionEffectsDefs.clarity)) {
			...
			if (effect.getAmplifier() <= 0)
				return;
			e.entityLiving.addPotionEffect(
	...
	public void teleportEvent(EnderTeleportEvent e) {
		...
		TileEntityAstralBarrier blockingBarrier = DimensionUtilities.GetBlockingAstralBarrier(
				e.getEntityLiving().worldObj, new BlockPos(e.getTargetX(), e.getTargetY(), e.getTargetZ()),
				keystoneKeys);
	...
	public void playerRender(RenderPlayerEvent.Pre e) {
		if (e.getEntityLiving().isPotionActive(PotionEffectsDefs.trueSight)) {
			...
			for (int rune : runes) {
				...
				Minecraft.getMinecraft().getItemRenderer().renderItem(e.getEntityPlayer(),
						new ItemStack(ItemDefs.rune, 1, rune), TransformType.GUI);
				GL11.glPopMatrix();
