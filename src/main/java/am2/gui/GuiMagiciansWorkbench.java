...
public class GuiMagiciansWorkbench extends GuiContainer {
	...
	private static final ResourceLocation background = new ResourceLocation("arsmagica2",
			"textures/gui/magicians_workbench.png");
	...
	protected void actionPerformed(GuiButton par1GuiButton) throws IOException {
		...
		if (par1GuiButton == clearGridOne) {
			for (int i = 0; i < 9; ++i) {
				...
				this.mc.playerController.windowClick(this.inventorySlots.windowId, slot.slotNumber, 0, ClickType.THROW,
						this.mc.thePlayer);
			}
	...
	protected void drawGuiContainerBackgroundLayer(float f, int mouseX, int mouseY) {
		...
		drawTexturedModalRect(l + 166, i1, 202, 0, 54, 162); // internal
																// inventory
		drawTexturedModalRect(l + 12, i1 + 162, 0, 137, 176, 89); // player
																	// inventory
		drawTexturedModalRect(l + 191, i1 + 172, 178, 0, 24, 72); // keystone
																	// lockable
																	// slots
		...
		if (this.workbenchInventory.getUpgradeStatus(TileEntityMagiciansWorkbench.UPG_CRAFT))
			drawTexturedModalRect(l + 92, i1 + 28, 202, 162, 54, 54); // grid
																		// 3x3
		else
			drawTexturedModalRect(l + 101, i1 + 37, 202, 162, 36, 36); // grid
																		// 2x2
