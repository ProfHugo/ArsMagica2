...
public class GuiSummoner extends GuiContainer {
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		int color = cost >= 0
				? cost <= PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld).getHighestPower(summonerInventory)
						? 0x007700 : 0x770000
				: 0x333333;
		...
		String readyString = summonerInventory.canSummon() ? I18n.translateToLocal("am2.gui.summonReady")
				: I18n.translateToLocal("am2.gui.summonNotReady");
		color = summonerInventory.canSummon() ? 0x007700 : 0x770000;

		fontRendererObj.drawString(readyString, xSize / 2 - (fontRendererObj.getStringWidth(readyString) / 2),
				ySize - 107, color);
	}
