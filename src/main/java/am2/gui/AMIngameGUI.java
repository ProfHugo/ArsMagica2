...
public class AMIngameGUI extends Gui {
	...
	// private final PotionEffectDurationComparator durationComparator = new
	// PotionEffectDurationComparator();

	// private static final ResourceLocation inv_top = new
	// ResourceLocation("arsmagica2", "textures/gui/Inventory_Top.png");
	private static final ResourceLocation mc_gui = new ResourceLocation("textures/gui/icons.png");
	private static final ResourceLocation spellbook_ui = new ResourceLocation("arsmagica2",
			"textures/gui/spellbook_ui.png");
	// private static final ResourceLocation inventory = new
	// ResourceLocation("textures/gui/container/inventory.png");
	...
	public void renderGameOverlay(RenderGameOverlayEvent.Post e) {
		...
		if (mc.currentScreen instanceof GuiHudCustomization || mc.inGameHasFocus) {
			...
			boolean drawAMHud = !ArsMagica2.config.showHudMinimally()
					|| (ci != null && (ci.getItem() == ItemDefs.spellBook || ci.getItem() == ItemDefs.spell
							|| ci.getItem() == ItemDefs.arcaneSpellbook || ci.getItem() instanceof IBoundItem));
			...
			GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA,
					GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.DST_ALPHA,
	...
	private void RenderManaBar(int i, int j, FontRenderer fontRendererObj) {
		...
		if (ArsMagica2.config.showHudBars()) {
			...
			// TODO Spell Groups
			if (curItem != null && (curItem.getItem() == ItemDefs.spell || curItem.getItem() == ItemDefs.spellBook
					|| curItem.getItem() == ItemDefs.arcaneSpellbook)) {
				ItemStack spellStack = curItem.getItem() == ItemDefs.spell ? curItem
				...
				if (spellStack != null) {
					...
					for (AbstractSpellPart p : parts) {
						TextureAtlasSprite icon = SpellIconManager.INSTANCE
								.getSprite(SpellRegistry.getSkillFromPart(p).getID());
						if (icon != null) {
			...
			DrawPartialIconAtXY(AMGuiIcons.manaLevel, progressScaled, 1, mana_hud.iX + 16, mana_hud.iY + 1f,
					(int) (barWidth * 0.99F), 40, false);
			...
			DrawPartialIconAtXY(AMGuiIcons.fatigueLevel, progressScaled, 1, Burnout_hud.iX, Burnout_hud.iY + 3f,
					BurnoutBarWidth, 40, false);
			...
			String magicLevel = (new StringBuilder()).append("")
					.append(EntityExtension.For(mc.thePlayer).getCurrentLevel()).toString();
		...
		if (ArsMagica2.config.getShowNumerics()) {
			...
			if (curItem != null && (curItem.getItem() == ItemDefs.spell || curItem.getItem() == ItemDefs.spellBook
				...
				ItemStack spellStack = curItem.getItem() == ItemDefs.spell ? curItem
						: ((ItemSpellBook) curItem.getItem()).GetActiveItemStack(curItem);
				if (spellStack != null) {
					float manaCost = SpellUtils.getManaCost(spellStack, Minecraft.getMinecraft().thePlayer)
							* (1F + (float) ((float) EntityExtension.For(Minecraft.getMinecraft().thePlayer)
									.getCurrentBurnout()
									/ (float) EntityExtension.For(Minecraft.getMinecraft().thePlayer).getMaxBurnout()));
					spellcost = (EntityExtension.For(Minecraft.getMinecraft().thePlayer).hasEnoughtMana(manaCost)
							? ChatFormatting.AQUA.toString() : ChatFormatting.DARK_RED.toString()) + " ("
							+ (int) (manaCost) + ")";
					spellcost += ChatFormatting.RESET.toString();
				}
			}

			String manaStr = I18n.translateToLocal("am2.gui.mana") + ": " + (int) (mana + bonusMana) + "/"
					+ (int) maxMana + spellcost;
			String burnoutStr = I18n.translateToLocal("am2.gui.burnout") + ": " + (int) props.getCurrentBurnout() + "/"
					+ (int) props.getMaxBurnout();
			...
			fontRendererObj.drawString(manaStr, manaNumericPos.iX, manaNumericPos.iY,
					hasBonusMana ? 0xeae31c : hasOverloadMana ? 0xFF2020 : 0x2080FF);
			fontRendererObj.drawString(burnoutStr,
					burnoutNumericPos.iX + 25 - fontRendererObj.getStringWidth(burnoutStr), burnoutNumericPos.iY,
					0xFF2020);
		}
		// Minecraft.getMinecraft().renderEngine.bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
	}
	...
	// return stack;
	// else if (stack.getItem() == ItemDefs.spellBook || stack.getItem() ==
	// ItemDefs.arcaneSpellbook)
	...
	private void RenderArmorStatus(int i, int j, Minecraft mc, FontRenderer fontRenderer) {
		...
		for (int slot = 0; slot < 4; ++slot) {
			if (ArmorHelper.PlayerHasArmorInSlot(mc.thePlayer, EntityEquipmentSlot.values()[5 - slot])) {
				AMVector2 position = getArmorSlotPosition(slot, scaledResolution.getScaledWidth(),
				...
				if (armor.isItemDamaged() && armor.getMaxDamage() > 0) {
					...
					AMGuiHelper.line2d(position.iX, position.iY + 10, position.iX + 10, position.iY + 10,
							this.zLevel + 100, lineweight, 0);

					int color = (int) (255.0f * (1 - pct)) << 16 | (int) (255.0f * pct) << 8;

					AMGuiHelper.line2d(position.iX, position.iY + 10, position.iX + (10 * pct), position.iY + 10,
							this.zLevel + 101, lineweight, color);
				}
				// cooldown
				if (blackoutMaxTimer > 0) {
					float pct = (float) (blackoutMaxTimer - blackoutTimer) / (float) blackoutMaxTimer;
					AMGuiHelper.line2d(position.iX, position.iY + 11, position.iX + 10, position.iY + 11,
							this.zLevel + 100, lineweight, 0);
					AMGuiHelper.line2d(position.iX, position.iY + 11, position.iX + (10 * pct), position.iY + 11,
							this.zLevel + 101, lineweight, 0xFF0000);
				} else {
					AMGuiHelper.line2d(position.iX, position.iY + 11, position.iX + 10, position.iY + 11,
							this.zLevel + 101, lineweight, 0x0000FF);
				}
				...
				// AMGuiHelper.DrawIconAtXY(icon, position.iX, position.iY,
				// this.zLevel, 10, 10, true);
				// }else{
				GlStateManager.pushMatrix();
				AMGuiHelper.DrawItemAtXY(mc.thePlayer.inventory.armorInventory[3 - slot], position.iX, position.iY,
						this.zLevel, 0.63f);
				GlStateManager.popMatrix();
				// }
			}
		...
		AMVector2 shieldPos = getShiftedVector(ArsMagica2.config.getManaShieldingPosition(),
	...
	public void RenderAffinity(int i, int j) {
		...
		for (Affinity affinity : ad.getHighestAffinities()) {
			if (affinity == null || affinity == Affinity.NONE)
				continue;
			GlStateManager.color(1.0f, 1.0f, 1.0f);
			AMGuiHelper.DrawIconAtXY(mc.getRenderItem().getItemModelMesher().getParticleIcon(ItemDefs.essence,
					AffinityShiftUtils.getEssenceForAffinity(affinity).getItemDamage()), x, y, j, 12, 12, true);

			if (ArsMagica2.config.getShowNumerics()) {
				GlStateManager.enableBlend();
				String display = String.format("%.2f%%",
						AffinityData.For(mc.thePlayer).getAffinityDepth(affinity) * 100f);
				if (x < i / 2)
					Minecraft.getMinecraft().fontRendererObj.drawString(display, x + 14, y + 2, affinity.getColor());
				else
					Minecraft.getMinecraft().fontRendererObj.drawString(display,
							x - 2 - Minecraft.getMinecraft().fontRendererObj.getStringWidth(display), y + 2,
							affinity.getColor());
			}
			y += 15;
		}
	...
	// int barWidth = i / 8;
	//
	// AMVector2 posBuffStart =
	// getShiftedVector(ArsMagica2.config.getPositiveBuffsPosition(), i, j);
	// AMVector2 negBuffStart =
	// getShiftedVector(ArsMagica2.config.getNegativeBuffsPosition(), i, j);
	//
	...
	// }
	// this.drawTexturedModalRect_Classic(negative_buff_x, negative_buff_y, 0 +
	// l % 8 * 18, 198 + l / 8 * 18, 10, 10, 18, 18);
	...
	// GL11.glColor4f(1.0f, 1.0f, 1.0f, AMGuiHelper.instance.slowFlashAlpha);
	// }
	// this.drawTexturedModalRect_Classic(positive_buff_x, positive_buff_y, 0 +
	// l % 8 * 18, 198 + l / 8 * 18, 10, 10, 18, 18);
	// GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	// positive_buff_x += 12;
	...
	public void RenderMagicXP(int i, int j) {
		...
		if (props.getCurrentLevel() > 0) {
			...
			GlStateManager.color(0.5f, 0.5f, 1.0f,
					ArsMagica2.config.showXPAlways() ? 1.0f : AMGuiHelper.instance.getMagicXPBarAlpha());

			// base XP bar
			drawTexturedModalRect_Classic(position.iX, position.iY, 0, 64, dimensions.iX, dimensions.iY, dimensions.iX,
					dimensions.iY);
			...
			if (props.getCurrentXP() > 0) {
				...
				drawTexturedModalRect_Classic(position.iX, position.iY, 0, 69, width, dimensions.iY, width,
						dimensions.iY);
			}

			if (ArsMagica2.config.getShowNumerics()
					&& (ArsMagica2.config.showXPAlways() || AMGuiHelper.instance.getMagicXPBarAlpha() > 0)) {
				String xpStr = I18n.translateToLocal("am2.gui.xp") + ": " + +(int) (props.getCurrentXP() * 100) + "/"
						+ (int) (props.getMaxXP() * 100);
	...
	public void drawTexturedModalRect_Classic(int par1, int par2, int par3, int par4, int par5, int par6) {
		...
		var9.getBuffer().pos(par1 + 0, par2 + par6, this.zLevel).tex((par3 + 0) * var7, (par4 + par6) * var8)
				.endVertex();
		var9.getBuffer().pos(par1 + par5, par2 + par6, this.zLevel).tex((par3 + par5) * var7, (par4 + par6) * var8)
				.endVertex();
		var9.getBuffer().pos(par1 + par5, par2 + 0, this.zLevel).tex((par3 + par5) * var7, (par4 + 0) * var8)
				.endVertex();
	 ...
	 *
	 * @param dst_x
	 *            The x coordinate on the screen to draw to
	 * @param dst_y
	 *            The y coordinate on the screen to draw to
	 * @param src_x
	 *            The x coordinate on the texture to pull from
	 * @param src_y
	 *            The y coordinate on the texture to pull from
	 * @param dst_width
	 *            The width on screen to draw
	 * @param dst_height
	 *            The height on screen to draw
	 * @param src_width
	 *            The width of the texture section
	 * @param src_height
	 *            The height of the texture section
	 */
	...
	public void drawTexturedModalRect(int par1, int par2, int par3, int par4, int par5, int par6) {
		...
		var8.getBuffer().pos(par1 + par5, par2 + par6, this.zLevel).tex(var10, var12).normal(0.0F, 1.0F, 0.0F)
				.endVertex();
	...
	private void DrawIconAtXY(TextureAtlasSprite IIcon, String base, float x, float y, int w, int h,
	...
	private void DrawPartialIconAtXY(TextureAtlasSprite IIcon, float pct_x, float pct_y, float x, float y, float w,
			float h, boolean semitransparent) {
		Minecraft.getMinecraft().renderEngine.bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
		if (IIcon == null)
			return;
		...
		tessellator.getBuffer().pos(x + (w * pct_x), y + (h * pct_y), this.zLevel).tex(IIcon.getMaxU(), IIcon.getMaxV())
				.endVertex();
			...
			if (((ItemBlock) e.getItemStack().getItem()).getBlock() instanceof BlockManaBattery)
				if (e.getItemStack().getTagCompound() != null) {
					...
					PowerTypes powerType = PowerTypes
							.getByID(e.getItemStack().getTagCompound().getInteger("mana_battery_powertype"));
					e.getToolTip().add(String.format("\u00A7r\u00A79Contains \u00A75%.2f %s%s \u00A79etherium", charge,
							powerType.getChatColor(), powerType.name()));
				}
		}
	}
}
