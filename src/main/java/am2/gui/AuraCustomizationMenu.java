...
public class AuraCustomizationMenu extends GuiScreen {
	...
	@Override
	public void initGui() {
		btnParticleType = new GuiButtonVariableDims(10, 50, 40,
				AMParticle.particleTypes[ArsMagica2.config.getAuraIndex()]);
		btnParticleBehaviour = new GuiButtonVariableDims(11, 50, 60,
				ParticleController.AuraControllerOptions[ArsMagica2.config.getAuraBehaviour()]);
		btnParticleColorMode = new GuiButtonVariableDims(12, 50, 80,
				ArsMagica2.config.getAuraColorDefault() ? I18n.translateToLocal("am2.gui.default")
						: ArsMagica2.config.getAuraColorRandom() ? I18n.translateToLocal("am2.gui.random")
								: I18n.translateToLocal("am2.gui.custom"));

		btnParticleType.setDimensions(80, 20);
		btnParticleBehaviour.setDimensions(80, 20);
		btnParticleColorMode.setDimensions(80, 20);

		sliParticleScale = new GuiSlideControl(14, width - 110, 40, 100, I18n.translateToLocal("am2.gui.scale"),
				ArsMagica2.config.getAuraScale() * 10, 1f, 200f);
		sliParticleAlpha = new GuiSlideControl(15, width - 110, 60, 100, I18n.translateToLocal("am2.gui.alpha"),
				ArsMagica2.config.getAuraAlpha() * 100, 1f, 100f);
		sliParticleRed = new GuiSlideControl(16, width - 110, 80, 100, I18n.translateToLocal("am2.gui.red"),
				(ArsMagica2.config.getAuraColor() >> 16) & 0xFF, 0f, 255f);
		sliParticleRed.setInteger(true);
		sliParticleGreen = new GuiSlideControl(17, width - 110, 100, 100, I18n.translateToLocal("am2.gui.green"),
				(ArsMagica2.config.getAuraColor() >> 8) & 0xFF, 0f, 255f);
		sliParticleGreen.setInteger(true);
		sliParticleBlue = new GuiSlideControl(18, width - 110, 120, 100, I18n.translateToLocal("am2.gui.blue"),
				ArsMagica2.config.getAuraColor() & 0xFF, 0f, 255f);
		sliParticleBlue.setInteger(true);

		sliParticleQuantity = new GuiSlideControl(20, width - 110, 140, 100, I18n.translateToLocal("am2.gui.qty"),
				ArsMagica2.config.getAuraQuantity(), 1, 5);
		sliParticleQuantity.setInteger(true);

		sliParticleDelay = new GuiSlideControl(21, width - 110, 160, 100, I18n.translateToLocal("am2.gui.delay"),
				ArsMagica2.config.getAuraDelay(), 1, 100);
		sliParticleDelay.setInteger(true);

		sliParticleSpeed = new GuiSlideControl(22, width - 110, 180, 100, I18n.translateToLocal("am2.gui.speed"),
				ArsMagica2.config.getAuraSpeed(), 0.05f, 10.0f);
		...
		this.buttonList
				.add(new GuiButton(200, this.width / 2 - 100, this.height - 28, I18n.translateToLocal("am2.gui.done")));
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		...
		case 13: // random color
			...
			btnParticleColorMode.displayString = ArsMagica2.config.getAuraColorDefault() ? "Default"
					: ArsMagica2.config.getAuraColorRandom() ? I18n.translateToLocal("am2.gui.random")
							: I18n.translateToLocal("am2.gui.custom");
			break;
		case 14: // scale
		...
		if (clickedBtn != null && button == 1) {
			if (clickedBtn.id == 10) {
				...
				while (AMParticle.particleTypes[index].startsWith("lightning_bolt")
						&& ArsMagica2.proxy.playerTracker.getAAL(Minecraft.getMinecraft().thePlayer) < 3) {
					index--;
					if (index < 0)
						index = AMParticle.particleTypes.length - 1;
				}
