...
public class GuiSlideControl extends GuiButton {
	...
	public GuiSlideControl(int par1, int par2, int par3, int width, String par5Str, float initialValue, float sliderMin,
			float sliderMax) {
	...
	public GuiSlideControl(int par1, int par2, int par3, int width, int height, String par5Str, float initialValue,
	...
	public void drawButton(Minecraft par1Minecraft, int par2, int par3) {
		if (this.visible) {
			...
			// this.dragging = par2 >= this.xPosition && par3 >= this.yPosition
			// && par2 < this.xPosition + this.width && par3 < this.yPosition +
			// this.height;
			...
			this.drawCenteredString(fontrenderer, this.displayString, this.xPosition + this.width / 2,
					this.yPosition + (this.height - 6) / 2, l);
		}
	}
		...
		if (this.visible) {
			...
			if (this.isVertical) {
				this.drawTexturedModalRect(xPos,
						this.yPosition + (int) (this.sliderValue / scale * (this.height - this.buttonH)), buttonUL,
						buttonVL, buttonW, buttonH);
				this.drawTexturedModalRect(xPos + buttonW,
						this.yPosition + (int) (this.sliderValue / scale * (this.height - this.buttonH)), buttonUR,
						buttonVR, buttonW, buttonH);
			} else {
				this.drawTexturedModalRect(this.xPosition + (int) (this.sliderValue * (this.width - buttonW * 2)), yPos,
						buttonUL, buttonVL, buttonW, buttonH);
				this.drawTexturedModalRect(
						this.xPosition + (int) (this.sliderValue * (this.width - buttonW * 2)) + buttonW, yPos,
						buttonUR, buttonVR, buttonW, buttonH);
			}
			GL11.glPopMatrix();
		}
	}
