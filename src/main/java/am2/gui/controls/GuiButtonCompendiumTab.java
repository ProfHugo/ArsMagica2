...
public class GuiButtonCompendiumTab extends GuiButton {
	...
	// private static final int renderTextureID =
	// AMGuiHelper.createRenderTexture();
	// private static final int depthBufferTextureID =
	// AMGuiHelper.instance.createFBO(renderTextureID, 100, 20, true);
	...
	public GuiButtonCompendiumTab(int id, int xPos, int yPos, CompendiumCategory category) {
		...
		this.width = category.getTexture() == null
				? Minecraft.getMinecraft().fontRendererObj.getStringWidth(category.getCategoryName()) : 20;
	...
	public void drawButton(Minecraft par1Minecraft, int par2, int par3) {
		if (this.visible) {
			boolean isMousedOver = par2 >= this.xPosition && par3 >= this.yPosition
					&& par2 < this.xPosition + this.width && par3 < this.yPosition + this.height;
			GL11.glColor4f(0.6f, 0.6f, 0.6f, 1.0f);
			par1Minecraft.renderEngine
					.bindTexture(new ResourceLocation("arsmagica2", "textures/gui/ArcaneCompendiumIndexGui.png"));
			...
			this.drawTexturedModalRect_Classic(this.xPosition, this.yPosition, u, v, this.width * 4, this.height, 138,
					16);
			if (hasNew) {
				GL11.glColor4f(1, 1, 1, 0.7f);
				AMGuiHelper.DrawIconAtXY(AMParticleIcons.instance.getIconByName("lights"),
						this.xPosition + (this.width / 3) - 1, this.yPosition + 1, this.zLevel, 16, 16, true);
			}
			if (displayIcon != null) {
				par1Minecraft.renderEngine.bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
				AMGuiHelper.DrawIconAtXY(displayIcon, this.xPosition + (this.width / 3), this.yPosition + 2,
						this.zLevel, 14, 14, true);
			} else {
				GL11.glColor4f(0, 0, 0, 1);

				Minecraft.getMinecraft().fontRendererObj.drawString(this.displayString,
						this.xPosition + (this.width / 4), this.yPosition + 2, 0x000000);
			}
