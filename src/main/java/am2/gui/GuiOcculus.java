...
public class GuiOcculus extends GuiScreen {
	...
	public void initGui() {
		...
		// for (int i = 0; i < 16; i++) {
		// test.add(new SkillTree("Tree" + i, new
		// ResourceLocation("textures/blocks/dirt.png"), new
		// ResourceLocation("textures/blocks/stone.png")));
		// }
		for (SkillTree entry : testTab) {
			if (tabId % 16 < 8)
				buttonList.add(new GuiButtonSkillTree(tabId, posX + 7 + ((tabId % 16) * 24), posY - 22, entry,
						(int) Math.floor((float) tabId / 16F), false));
			else
				buttonList.add(new GuiButtonSkillTree(tabId, posX + 7 + (((tabId % 16) - 8) * 24), posY + 210, entry,
						(int) Math.floor((float) tabId / 16F), true));
		...
		if (mouseButton == 0) {
			if (hoverItem != null && !SkillData.For(player).hasSkill(hoverItem.getID())) {
				...
				if (data.canLearn(hoverItem.getID())) {
					AMNetHandler.INSTANCE.sendPacketToServer(AMPacketIDs.UNLOCK_OCCULUS_ENTRY,
							new AMDataWriter().add(hoverItem.getID()).generate());
				}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		Minecraft.getMinecraft().renderEngine
				.bindTexture(new ResourceLocation("arsmagica2", "textures/occulus/overlay.png"));
		// Overlay
		...
		float calcXOffest = ((float) offsetX / 568) * (1 - renderRatio);
		{
			...
			for (SkillPoint point : SkillPointRegistry.getSkillPointMap().values()) {
				if (!point.canRender())
					continue;
				maxSize = Math.max(maxSize, fontRendererObj
						.getStringWidth(point.getName() + " : " + SkillData.For(player).getSkillPoint(point)));
			}
			zLevel = 0F;
			Minecraft.getMinecraft().renderEngine
					.bindTexture(new ResourceLocation("arsmagica2", "textures/occulus/skill_points.png"));
			drawSkillPointBackground(posX, posY, maxSize + 10, 210);
			int pointOffsetX = 5;
			for (SkillPoint point : SkillPointRegistry.getSkillPointMap().values()) {
				if (!point.canRender())
					continue;
				fontRendererObj.drawString(point.getName() + " : " + SkillData.For(player).getSkillPoint(point),
						posX + 215, posY + pointOffsetX, point.getColor());
				pointOffsetX += 10;
			}
		...
		if (currentTree != SkillDefs.TREE_AFFINITY) {
			RenderUtils.drawBox(posX + 7, posY + 7, 196, 196, zLevel, calcXOffest, calcYOffest,
					renderRatio + calcXOffest, renderRatio + calcYOffest);
			...
			for (Skill s : skills) {
				...
				for (String p : s.getParents()) {
					...
					if (parent == null || !skills.contains(parent))
						continue;
					...
					int color = (!SkillData.For(player).hasSkill(s.getID()) ? s.getPoint().getColor() & 0x999999
							: 0x00ff00);
					if (!hasPrereq)
						color = 0x000000;
			...
			for (Skill s : skills) {
				...
				int tick = (player.ticksExisted % 80) >= 40 ? (player.ticksExisted % 40) - 20
						: -(player.ticksExisted % 40) + 20;
				float multiplier = 0.75F + tick / 80F;
				TextureAtlasSprite sprite = SpellIconManager.INSTANCE.getSprite(s.getID());
				if (offsetX + renderSize < posX + 7 || offsetX > posX + 203 || offsetY + renderSize < posY + 7
						|| offsetY > posY + 203 || sprite == null) {
					continue;
				}
				...
				if (!hasPrereq)
					...
					GlStateManager.color(Math.max(RenderUtils.getRed(s.getPoint().getColor()), 0.6F) * multiplier,
							Math.max(RenderUtils.getGreen(s.getPoint().getColor()), 0.6F) * multiplier,
							Math.max(RenderUtils.getBlue(s.getPoint().getColor()), 0.6F) * multiplier);

				if (ArsMagica2.disabledSkills.isSkillDisabled(s.getID()))
					GlStateManager.color(0.3f, 0.3f, 0.3f);
				RenderUtils.drawBox(offsetX + xStartMod, offsetY + yStartMod, renderSize - xStartMod - xEndMod,
						renderSize - yStartMod - yEndMod, 0, sprite.getMinU() + (xStartMod / renderSize * spriteXSize),
				...
				if (ArsMagica2.disabledSkills.isSkillDisabled(s.getID())) {
					...
					RenderUtils.drawBox(offsetX + xStartMod + 8, offsetY + yStartMod + 8, 16 - xStartMod - xEndMod,
							16 - yStartMod - yEndMod, 0, sprite.getMinU() + (xStartMod / 16 * spriteXSize),
			...
			if (mouseX > posX && mouseX < posX + 210 && mouseY > posY && mouseY < posY + 210) {
				...
				for (Skill s : skills) {
					...
					int offsetX = calcXOffset(posX, s);
					int offsetY = calcYOffset(posY, s);
					if (offsetX > mouseX || offsetX < mouseX - renderSize || offsetY > mouseY
							|| offsetY < mouseY - renderSize)
						continue;
					...
					if (ArsMagica2.disabledSkills.isSkillDisabled(s.getID()))
						list.add(
								TextFormatting.DARK_RED.toString() + I18n.translateToLocal("am2.gui.occulus.disabled"));
					else if (hasPrereq)
						list.add(TextFormatting.DARK_GRAY.toString() + s.getOcculusDesc());
					else
						list.add(TextFormatting.DARK_RED.toString()
								+ I18n.translateToLocal("am2.gui.occulus.missingrequirements"));
			...
			// AffinityData.For(player).getAffinityDepth(SkillDefs.NONE) * 100;
			...
			for (Affinity aff : ArsMagicaAPI.getAffinityRegistry().getValues()) {
				...
				double affEndX = Math.cos(Math.toRadians(portion * currentID)) * 10F
						+ Math.cos(Math.toRadians(portion * currentID)) * depth * 60F;
				double affEndY = Math.sin(Math.toRadians(portion * currentID)) * 10F
						+ (Math.sin(Math.toRadians(portion * currentID))) * depth * 60F;
				...
				int displace = (int) ((Math.max(affStartX1, affStartX2) - Math.min(affStartX1, affStartX2)
						+ Math.max(affStartY1, affStartY2) - Math.min(affStartY1, affStartY2)) / 2);
				if (depth > 0.01F) {
					RenderUtils.fractalLine2dd(affStartX1 + cX, affStartY1 + cY, affEndX + cX, affEndY + cY, zLevel,
							aff.getColor(), displace, 0.8F);
					RenderUtils.fractalLine2dd(affStartX2 + cX, affStartY2 + cY, affEndX + cX, affEndY + cY, zLevel,
							aff.getColor(), displace, 0.8F);

					RenderUtils.fractalLine2dd(affStartX1 + cX, affStartY1 + cY, affEndX + cX, affEndY + cY, zLevel,
							aff.getColor(), displace, 1.1F);
					RenderUtils.fractalLine2dd(affStartX2 + cX, affStartY2 + cY, affEndX + cX, affEndY + cY, zLevel,
							aff.getColor(), displace, 1.1F);
				} else {
					RenderUtils.line2d((float) affStartX1 + cX, (float) affStartY1 + cY, (float) affEndX + cX,
							(float) affEndY + cY, zLevel, aff.getColor());
					RenderUtils.line2d((float) affStartX2 + cX, (float) affStartY2 + cY, (float) affEndX + cX,
							(float) affEndY + cY, zLevel, aff.getColor());
				}

				Minecraft.getMinecraft().fontRendererObj.drawString("" + (float) Math.round(depth * 10000) / 100F,
						(int) ((affDrawTextX * 0.9) + cX), (int) ((affDrawTextY * 0.9) + cY), aff.getColor());
				// Minecraft.getMinecraft().fontRendererObj.drawString("" +
				// (float)Math.round(depth * 10000) / 100F, , aff.getColor());
				...
				this.itemRender.renderItemAndEffectIntoGUI(
						new ItemStack(ItemDefs.essence, 1, ArsMagicaAPI.getAffinityRegistry().getId(aff)), drawX,
						drawY);
				if (mouseX > drawX && mouseX < drawX + 16 && mouseY > drawY && mouseY < drawY + 16) {
					...
					ArrayList<AbstractAffinityAbility> abilites = Lists
							.newArrayList(ArsMagicaAPI.getAffinityAbilityRegistry().getValues());
					...
					for (AbstractAffinityAbility ability : abilites) {
						if (ability.getAffinity() == aff) {
							...
							if (isShiftDown) {
								advancedTooltip = " (Min. : " + Math.round(ability.getMinimumDepth() * 100) + "%"
										+ (ability.hasMax()
												? (", Max. : " + Math.round(ability.getMaximumDepth() * 100) + "%")
												: "")
										+ ")";
							}
			...
			if (!drawString.isEmpty()) {
				if (!isShiftDown)
					drawString
							.add(TextFormatting.GRAY.toString() + I18n.translateToLocal("am2.tooltip.shiftForDetails"));
		...
		//
		// int tier0 =
		// SkillData.For(player).getSkillPoint(SkillPoint.SKILL_POINT_1);
		// int tier1 =
		// SkillData.For(player).getSkillPoint(SkillPoint.SKILL_POINT_2);
		// int tier2 =
		// SkillData.For(player).getSkillPoint(SkillPoint.SKILL_POINT_3);
		// int tier3 =
		// SkillData.For(player).getSkillPoint(SkillPoint.SKILL_POINT_4);
		// int tier4 =
		// SkillData.For(player).getSkillPoint(SkillPoint.SKILL_POINT_5);
		// int tier5 =
		// SkillData.For(player).getSkillPoint(SkillPoint.SKILL_POINT_6);
		// GlStateManager.disableDepth();
		// fontRendererObj.drawString("" + tier0, posX + 191, posY - 19,
		// SkillPointRegistry.getPointForTier(0).getColor());
		// fontRendererObj.drawString("" + tier1, posX + 203, posY - 19,
		// SkillPointRegistry.getPointForTier(1).getColor());
		// fontRendererObj.drawString("" + tier2, posX + 197, posY - 9,
		// SkillPointRegistry.getPointForTier(2).getColor());
		// if (SkillPointRegistry.getPointForTier(3) != null)
		// fontRendererObj.drawString("" + tier3, posX + 191, posY + 210 + 2,
		// SkillPointRegistry.getPointForTier(3).getColor());
		// if (SkillPointRegistry.getPointForTier(4) != null)
		// fontRendererObj.drawString("" + tier4, posX + 203, posY + 210 + 2,
		// SkillPointRegistry.getPointForTier(4).getColor());
		// if (SkillPointRegistry.getPointForTier(5) != null)
		// fontRendererObj.drawString("" + tier5, posX + 197, posY + 210 + 12,
		// SkillPointRegistry.getPointForTier(5).getColor());
	...
	private void drawSkillPointBackground(int startX, int startY, int width, int height) {
		...
		// drawTexturedModalRect(startX + posX, startY + posY + height - 4, 0,
		// 252, 4, 4);
		...
		while (h > 0) {
			...
			if (h > 248)
				...
				y = h;
			// drawTexturedModalRect(startX + posX, startY + posY + 4 + h - y,
			// 0, 4, 4, y);
