...
public class ContainerMagiciansWorkbench extends AM2Container {
	...
	public ContainerMagiciansWorkbench(InventoryPlayer playerInventory, TileEntityMagiciansWorkbench tileEntity) {
		...
		secondCraftMatrix = tileEntity.getUpgradeStatus(TileEntityMagiciansWorkbench.UPG_CRAFT)
				? new InventoryCrafting(this, 3, 3) : new InventoryCrafting(this, 2, 2);
		...
		// first output
		addSlotToContainer(new SlotMagiciansWorkbenchCrafting(playerInventory.player, firstCraftMatrix,
				tileEntity.firstCraftResult, this, 0, 37, 89));
		...
		// second output
		addSlotToContainer(new SlotMagiciansWorkbenchCrafting(playerInventory.player, secondCraftMatrix,
				tileEntity.secondCraftResult, this, 0, 111, 89));
	...
	public void onCraftMatrixChanged(IInventory par1iInventory) {
		this.workbenchInventory.firstCraftResult.setInventorySlotContents(0,
				CraftingManager.getInstance().findMatchingRecipe(this.firstCraftMatrix, world));
		if (!initializing) {
			for (int i = 0; i < 9; ++i) {
		...
		this.workbenchInventory.secondCraftResult.setInventorySlotContents(0,
				CraftingManager.getInstance().findMatchingRecipe(this.secondCraftMatrix, world));
		if (!initializing) {
			for (int i = 0; i < 9; ++i) {
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int i) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			} else if (i >= INVENTORY_STORAGE_START && i < PLAYER_INVENTORY_START) // from
																					// player
																					// inventory
			{
	...
	public HashMap<ImmutablePair<Item, Integer>, Integer> getComponentCount(int recipeIndex) {
		...
		for (ItemStack stack : recipe.components) {
			if (stack == null)
				continue;
			ImmutablePair<Item, Integer> pair = new ImmutablePair<Item, Integer>(stack.getItem(),
					stack.getItemDamage());
	...
	private boolean hasComponent(ItemStack component, int qty) {
		int matchedQty = 0;
		for (int i = getWorkbench().getStorageStart() - 3; i < getWorkbench().getStorageStart() - 3
				+ getWorkbench().getStorageSize(); ++i) {
	...
	private void setRecipeItemsToGrid(int recipeIndex) {
		...
		int count = 0;
		getWorkbench();
		if ((recipe.is2x2 || getWorkbench().getUpgradeStatus(TileEntityMagiciansWorkbench.UPG_CRAFT))
				&& craftingGridIsEmpty(true)) {
			for (ItemStack stack : recipe.components) {
	...
	public boolean isRecipeAlreadyInGrid(int recipeIndex) {
		RememberedRecipe recipe = this.workbenchInventory.getRememberedRecipeItems().get(recipeIndex);
		if (getWorkbench().firstCraftResult.getStackInSlot(0) != null
				&& getWorkbench().firstCraftResult.getStackInSlot(0).isItemEqual(recipe.output))
			return true;
		if (getWorkbench().secondCraftResult.getStackInSlot(0) != null
				&& getWorkbench().secondCraftResult.getStackInSlot(0).isItemEqual(recipe.output))
			return true;
		return false;
	}
}
