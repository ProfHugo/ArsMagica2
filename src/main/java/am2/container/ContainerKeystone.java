...
public class ContainerKeystone extends Container {
	...
	public ItemStack[] GetFullRuneBagInventory() {
		...
		for (int i = InventoryKeyStone.inventorySize; i < InventoryKeyStone.inventorySize
				+ InventoryRuneBag.inventorySize; ++i) {
		...
		if (!world.isRemote) {
			...
			if (runeBagStack != null) {
				...
				entityplayer.inventory.setInventorySlotContents(
						InventoryUtilities.getInventorySlotIndexFor(entityplayer.inventory, ItemDefs.runeBag),
						runebagItemStack);
			}
		}
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int i) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			} else if (i >= PLAYER_INVENTORY_START && i < PLAYER_ACTION_BAR_START) // range
																					// 27
																					// -
																					// player
																					// inventory
			...
			} else if (i >= PLAYER_ACTION_BAR_START && i < PLAYER_ACTION_BAR_END) // range
																					// 9
																					// -
																					// player
																					// action
	...
	public boolean setInventoryToCombination(int comboIndex) {
		...
		if (player.worldObj.isRemote) {
			AMNetHandler.INSTANCE.sendPacketToServer(AMPacketIDs.SET_KEYSTONE_COMBO,
					new AMDataWriter().add(comboIndex).generate());
			return true;
		}
		...
		while (matchIndex < combo.metas.length && searchIndex < InventoryKeyStone.inventorySize
				+ (runeBag != null ? InventoryRuneBag.inventorySize : 0)) {
			...
			int inventoryIndex = searchIndex >= InventoryKeyStone.inventorySize
					? searchIndex - InventoryKeyStone.inventorySize : searchIndex;
