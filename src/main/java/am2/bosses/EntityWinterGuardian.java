...
public class EntityWinterGuardian extends AM2Boss {
	...
	protected void initSpecificAI() {
		this.tasks.addTask(1, new EntityAICastSpell<EntityWinterGuardian>(this, NPCSpells.instance.dispel, 16, 23, 50,
				BossActions.CASTING, new ISpellCastCallback<EntityWinterGuardian>() {
					@Override
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(2900D);
	}
		...
		if (worldObj.getBiome(getPosition()).getEnableSnow() && worldObj.getWorldInfo().isRaining()) {
			if (worldObj.isRemote) {
				AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "ember",
						posX + (rand.nextFloat() * 6 - 3), posY + 2 + (rand.nextFloat() * 2 - 1),
			...
			if (this.ticksExisted % 100 == 0) {
				...
				for (EntityLivingBase entity : entities) {
					...
					entity.addPotionEffect(
							new PotionEffect(Potion.getPotionFromResourceLocation("mining_fatigue"), 220, 3));
				}
			}
		}
	...
	private void spawnParticles() {
		for (int i = 0; i < ArsMagica2.config.getGFXLevel() * 4; ++i) {
			...
			AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj,
					rnd < 5 ? "snowflakes" : "ember", posX + (rand.nextFloat() * 0.4 - 0.2), posY + 2,
	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		for (int j = 0; j < i; j++) {
			this.entityDropItem(
					new ItemStack(ItemDefs.essence, 1, ArsMagicaAPI.getAffinityRegistry().getId(Affinity.ICE)), 0.0f);
		}
		i = rand.nextInt(10);
