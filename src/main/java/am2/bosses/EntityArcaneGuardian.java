...
public class EntityArcaneGuardian extends AM2Boss {
	...
	private static final DataParameter<Integer> DW_TARGET_ID = EntityDataManager.createKey(EntityArcaneGuardian.class,
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(1150D);
	}
		...
		if (isWithin(runeRotationY, targetRuneRotationY, 0.5f)) {
			if (this.getDistanceSqToEntity(source) < 9) {
				...
				if (source instanceof EntityPlayer) {
					AMNetHandler.INSTANCE.sendVelocityAddPacket(source.worldObj, (EntityLivingBase) source,
							speed * Math.cos(radians), vertSpeed, speed * Math.sin(radians));
				}
	...
	protected void initSpecificAI() {
		...
		this.tasks.addTask(1, new EntityAICastSpell<EntityArcaneGuardian>(this, NPCSpells.instance.healSelf, 16, 23, 60,
				BossActions.CASTING, new ISpellCastCallback<EntityArcaneGuardian>() {
					@Override
					public boolean shouldCast(EntityArcaneGuardian host, ItemStack spell) {
						return host.getHealth() < host.getMaxHealth();
					}
				}));
		this.tasks.addTask(2, new EntityAICastSpell<EntityArcaneGuardian>(this, NPCSpells.instance.blink, 16, 23, 20,
				BossActions.CASTING));
		this.tasks.addTask(3, new EntityAICastSpell<EntityArcaneGuardian>(this, NPCSpells.instance.arcaneBolt, 12, 23,
				5, BossActions.CASTING));
	}
