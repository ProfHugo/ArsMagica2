...
public abstract class AM2Boss extends EntityMob implements IEntityMultiPart, IArsMagicaBoss {
	...
	public AM2Boss(World par1World) {
		super(par1World);
		if (par1World != null)
			bossInfo = (BossInfoServer) (new BossInfoServer(this.getDisplayName(), getBarColor(),
	...
	// Bosses should be able to follow players through doors and hallways, so
	// setSize is overridden to instead add a
	// damageable entity based bounding box of the specified size, unless a boss
	// already uses parts.
	@Override
	...
	/**
	 * This contains the default AI tasks. To add new ones, override
	 * {@link #initSpecificAI()}
	 */
	protected void initAI() {
		...
		this.targetTasks.addTask(3,
				new EntityAINearestAttackableTarget<EntityLightMage>(this, EntityLightMage.class, 0, true, true, null));
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
		...
		if (par1DamageSource == DamageSource.inWall) {
			if (!worldObj.isRemote) {// dead code? (calling canSnowAt() without
										// using the result) could it be a buggy
										// upgrade to 1.7.10?
				for (int i = -1; i <= 1; ++i) {
					for (int j = 0; j < 3; ++j) {
						for (int k = -1; k <= 1; ++k) {
		...
		if (par1DamageSource.getSourceOfDamage() != null) {
			...
			if (par1DamageSource.getSourceOfDamage() instanceof EntityPlayer) {
				...
				if (player.capabilities.isCreativeMode && player.getHeldItemMainhand() != null
						&& player.getHeldItemMainhand().getItem() == ItemDefs.woodenLeg) {
