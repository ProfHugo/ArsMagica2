...
public class EntityLightningGuardian extends AM2Boss implements IAnimatedEntity {
	...
	protected void initSpecificAI() {
		...
		this.tasks.addTask(3, new EntityAICastSpell<EntityLightningGuardian>(this, NPCSpells.instance.lightningRune, 22,
				27, 200, BossActions.CASTING));
		this.tasks.addTask(3, new EntityAICastSpell<EntityLightningGuardian>(this, NPCSpells.instance.scrambleSynapses,
				45, 60, 300, BossActions.SMASH));
			...
			if (this.getCurrentAction() == BossActions.CHARGE) {
				if (ticksInCurrentAction > 50) {
					for (int i = 0; i < 2 * ArsMagica2.config.getGFXLevel(); ++i) {
						AMParticle smoke = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "smoke", posX,
								posY + 4, posZ);
						if (smoke != null) {
				...
				if (ticksInCurrentAction > 66) {
					ArsMagica2.proxy.particleManager.BoltFromPointToPoint(worldObj, posX + rand.nextDouble() - 0.5,
							posY + rand.nextDouble() - 0.5 + 2, posZ + rand.nextDouble() - 0.5,
							posX + rand.nextDouble() * dist - halfDist, posY + rand.nextDouble() * dist - halfDist,
				...
				if (ticksInCurrentAction > 25 && ticksInCurrentAction < 150) {
					for (int i = 0; i < 2 * ArsMagica2.config.getGFXLevel(); ++i) {
						...
						if (smoke != null) {
							...
							smoke.SetParticleAlpha(1f);
							smoke.setParticleScale(1f);
							smoke.setRGBColorI(ticksInCurrentAction < 85
									? 0xFFFFFF - 0x111111 * ((ticksInCurrentAction - 25) / 4) : 0x222222);
							smoke.setMaxAge(20);
							smoke.AddParticleController(new ParticleHoldPosition(smoke, 10, 1, false));
