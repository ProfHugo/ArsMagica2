...
public class BossSpawnHelper {
	...
	private void checkForWaterGuardianSpawn(World world, BlockPos pos) {
		...
		for (Type type : types) {
			if (type == Type.WATER || type == Type.SWAMP || type == Type.BEACH || type == Type.OCEAN
					|| type == Type.RIVER || type == Type.WET) {
		...
		List<EntityItem> itemsInRange = world.getEntitiesWithinAABB(EntityItem.class,
				new AxisAlignedBB(pos, pos).expandXyz(1D));
	...
	private void checkForAirGuardianSpawn(World world, BlockPos pos) {
		...
		if (itemsInRange.size() != 1)
			return;
		if (itemsInRange.get(0).getEntityItem().getItem() != ItemDefs.essence || itemsInRange.get(0).getEntityItem()
				.getItemDamage() != ArsMagicaAPI.getAffinityRegistry().getId(Affinity.AIR))
			return;
	...
	private void checkForArcaneGuardianSpawn(World world, BlockPos pos) {
		...
		if (itemsInRange.size() != 1)
			return;
		if (itemsInRange.get(0).getEntityItem().getItem() != ItemDefs.essence || itemsInRange.get(0).getEntityItem()
				.getItemDamage() != ArsMagicaAPI.getAffinityRegistry().getId(Affinity.ARCANE))
			return;
		...
		if (hasStructure && lectern != null && lectern.hasStack()
				&& lectern.getStack().getItem() == ItemDefs.arcaneCompendium) {
			itemsInRange.get(0).setDead();
	...
	private void checkForEarthGuardianSpawn(World world, BlockPos pos) {
		...
		for (EntityItem item : itemsInRange) {
			...
			if (item.getEntityItem().getItem() == Items.EMERALD)
				hasEmerald = true;
			else if (item.getEntityItem().getItem() == ItemDefs.itemOre
					&& item.getEntityItem().getItemDamage() == ItemOre.META_BLUE_TOPAZ)
				hasTopaz = true;
			else if (item.getEntityItem().getItem() == ItemDefs.itemOre
					&& item.getEntityItem().getItemDamage() == ItemOre.META_CHIMERITE)
				hasChimerite = true;
		}

		hasStructure = world.getBlockState(pos.down()) == Blocks.STONEBRICK.getDefaultState()
	...
	private void checkForFireGuardianSpawn(EntityItem item, World world, BlockPos pos) {
		if (item.getEntityItem().getItem() != ItemDefs.essence
				|| item.getEntityItem().getItemDamage() != ArsMagicaAPI.getAffinityRegistry().getId(Affinity.WATER))
			return;
		boolean hasStructure = false;
	...
	public void checkForEnderGuardianSpawn(World world, BlockPos pos) {
		...
		for (EntityItem item : itemsInRange) {
			...
			if (item.getEntityItem().getItem() == Items.ENDER_EYE)
				hasEyeofEnder = true;
			else if (item.getEntityItem().getItem() == ItemDefs.essence
					&& item.getEntityItem().getItemDamage() == ArsMagicaAPI.getAffinityRegistry().getId(Affinity.ENDER))
