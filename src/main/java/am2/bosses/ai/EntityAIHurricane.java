...
public class EntityAIHurricane extends EntityAIBase {
	...
	public boolean continueExecuting() {
		...
		if (AITarget == null || AITarget.isDead
			...
			if (!host.worldObj.isRemote) {
				...
				for (int x = -1; x <= 1; ++x) {
					for (int z = -1; z <= 1; ++z) {
						...
						while (!host.worldObj.canBlockSeeSky(pos)
								&& host.worldObj.getBlockState(pos).getBlock() != Blocks.BEDROCK) {
							if (Math.abs(y - host.posY) > 10)
								break;
							host.worldObj.destroyBlock(new BlockPos((int) host.posX + x, y++, (int) host.posZ + z),
									true);
						}
						y = (int) host.posY + 2;
					}
			...
			for (Iterator<EntityLivingBase> enti = nearbyEntities.iterator(); enti.hasNext();) {
				...
				if (ent == host)
					continue;
				AMVector3 movement = MathUtilities.GetMovementVectorBetweenPoints(new AMVector3(host),
						new AMVector3(ent));
				float factor = 2.15f;
	...
	public void updateTask() {
		...
		List<EntityLivingBase> nearbyEntities = host.worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
				host.getEntityBoundingBox().expand(6, 3, 6));
		for (EntityLivingBase ent : nearbyEntities) {
			...
			AMVector3 movement = MathUtilities.GetMovementVectorBetweenPoints(
					new AMVector3(host).add(new AMVector3(0, host.getEyeHeight(), 0)), new AMVector3(ent));
			float factor = 0.18f;
			...
			if (ent instanceof EntityPlayer) {
				AMNetHandler.INSTANCE.sendVelocityAddPacket(host.worldObj, ent, -(oX - ent.motionX),
						-(oY - ent.motionY), -(oZ - ent.motionZ));
			}
			ent.fallDistance = 0f;
