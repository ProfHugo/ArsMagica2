...
public class EntityAIEnderwave extends AIAnimation {
	...
	public boolean shouldAnimate() {
		...
		if (living.getAttackTarget() == null
				|| new AMVector3(living).distanceSqTo(new AMVector3(living.getAttackTarget())) > 100)
			return false;
		...
		if (guardian.getAttackTarget() != null) {
			...
			if (guardian.getTicksInCurrentAction() == 7) {
				...
				guardian.worldObj.playSound(guardian.posX, guardian.posY, guardian.posZ,
						((IArsMagicaBoss) guardian).getAttackSound(), SoundCategory.HOSTILE, 1.0f,
						(float) (0.5 + guardian.getRNG().nextDouble() * 0.5f), false);
				SpellUtils.applyStackStage(NPCSpells.instance.enderGuardian_enderWave, guardian, guardian,
						guardian.posX, guardian.posY + 0.5f, guardian.posZ, null, guardian.worldObj, false, false, 0);
			} else {
				guardian.faceEntity(guardian.getAttackTarget(), 180, 180);
			}
		}
