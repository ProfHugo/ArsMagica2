...
public class EntityAIEnderRush extends AIAnimation {
	...
	public void updateTask() {
		...
		if (guardian.getTicksInCurrentAction() >= 18 && guardian.getTicksInCurrentAction() <= 30
				&& guardian.getAttackTarget() != null) {
			...
			if (a.distanceSqTo(b) > 4) {
				...
				if (guardian.getAttackTarget().attackEntityFrom(DamageSource.causeMobDamage(guardian), 15)
						&& guardian.getAttackTarget().getHealth() <= 0)
					guardian.heal(200);
			}
		}
	}
