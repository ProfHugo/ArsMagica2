...
public class EntityAILightningRod extends AIAnimation {
	...
	public boolean shouldAnimate() {
		...
		if (living.getHealth() > living.getMaxHealth() * 0.75f || living.getAttackTarget() == null
		...
		if (ticks <= 25) {
			...
			if (ticks > 85 && ticks <= 150) {
				if (!guardian.worldObj.isRemote && ticks % 20 == 0)
					guardian.worldObj.playSound(guardian.posX, guardian.posY, guardian.posZ,
							AMSounds.LIGHTNING_GUARDIAN_LIGHTNING_ROD_1, SoundCategory.HOSTILE, 1.0f,
			...
			if (ticks > 25 && ticks <= 85) {
				...
				if (!guardian.worldObj.isRemote && ticks == 30)
					guardian.worldObj.playSound(guardian.posX, guardian.posY, guardian.posZ,
							AMSounds.LIGHTNING_GUARDIAN_LIGHTNING_ROD_START, SoundCategory.HOSTILE, 1.0f,
							guardian.getRNG().nextFloat() * 0.5f + 0.5f, false);
				...
				ArsMagica2.proxy.particleManager.BoltFromEntityToEntity(guardian.worldObj, guardian, guardian, target,
						0);
				if (!guardian.worldObj.isRemote && ticks % 20 == 0)
					guardian.worldObj.playSound(guardian.posX, guardian.posY, guardian.posZ,
							AMSounds.LIGHTNING_GUARDIAN_IDLE, SoundCategory.HOSTILE, 1.0f,
							guardian.getRNG().nextFloat() * 0.5f + 0.5f, false);
				...
				if (!hasBolted) {
					hasBolted = true;
					EntityLightningBolt bolt = new EntityLightningBolt(guardian.worldObj, target.posX, target.posY,
							target.posZ, false);
