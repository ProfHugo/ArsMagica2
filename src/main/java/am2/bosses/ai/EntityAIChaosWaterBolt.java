...
public class EntityAIChaosWaterBolt extends EntityAIBase {
	...
	private static ItemStack createDummyStack() {
		ItemStack stack = SpellUtils.createSpellStack(new ArrayList<>(),
				Lists.newArrayList(Projectile(), WateryGrave(), MagicDamage(), Knockback()), new NBTTagCompound());
		return stack;
	}
	...
	public boolean shouldExecute() {
		if (host.getCurrentAction() == BossActions.IDLE && host.isActionValid(BossActions.CASTING))
			return true;
		return false;
	}
	...
	public void updateTask() {
		...
		if (!host.worldObj.isRemote && host.getCurrentAction() == BossActions.CASTING) {
			...
			SpellUtils.applyStackStage(castStack, host, host, host.posX, host.posY, host.posZ, null, host.worldObj,
					false, false, 0);
		}
	}
}
