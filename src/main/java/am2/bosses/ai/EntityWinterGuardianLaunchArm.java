...
public class EntityWinterGuardianLaunchArm extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		List<EntityLivingBase> entities = host.worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
				host.getEntityBoundingBox().expand(20, 20, 20));
		if (entities.size() > 0) {
	...
	public boolean continueExecuting() {
		if (target == null || target.isDead || host.getDistanceSqToEntity(target) < 49D
				|| host.getDistanceSqToEntity(target) > 225D || (host.getCurrentAction() == BossActions.LAUNCHING
						&& host.getTicksInCurrentAction() > host.getCurrentAction().getMaxActionTime())) {
		...
		if (host.getDistanceSqToEntity(target) > 144 && host.getCurrentAction() == BossActions.IDLE) {
			...
			if (host.getTicksInCurrentAction() == 14) {
				...
				if (!host.worldObj.isRemote) {
					...
					if (!host.worldObj.isRemote)
						host.worldObj.playSound(host.posX, host.posY, host.posZ, AMSounds.WINTER_GUARDIAN_LAUNCH_ARM,
