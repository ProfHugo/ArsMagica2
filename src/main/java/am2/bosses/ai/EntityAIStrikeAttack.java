...
public class EntityAIStrikeAttack extends EntityAIBase {
	...
	public EntityAIStrikeAttack(IArsMagicaBoss host, float moveSpeed, float damage,
			DamageSources.DamageSourceTypes damageType) {
	...
	public boolean shouldExecute() {
		if (cooldownTicks-- > 0 || ((IArsMagicaBoss) host).getCurrentAction() != BossActions.IDLE
				|| !((IArsMagicaBoss) host).isActionValid(BossActions.STRIKE))
			return false;
	...
	public boolean continueExecuting() {
		EntityLivingBase AITarget = host.getAttackTarget();
		if (AITarget == null || AITarget.isDead
				|| (((IArsMagicaBoss) host).getCurrentAction() == BossActions.STRIKE && ((IArsMagicaBoss) host)
