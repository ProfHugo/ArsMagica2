...
public class ObeliskFuelHelper implements IObeliskFuelHelper {
	...
	public int getFuelBurnTime(ItemStack stack) {
		...
		for (ItemStack possibleFuel : validFuels.keySet()) {
			...
			boolean metaCheck = possibleFuel.getItemDamage() == Short.MAX_VALUE
					|| possibleFuel.getItemDamage() == stack.getItemDamage();
			boolean tagCheck = possibleFuel.getTagCompound() == null ? true
					: (stack.getTagCompound() == null ? false
							: NBTUtils.contains(possibleFuel.getTagCompound(), stack.getTagCompound()));
