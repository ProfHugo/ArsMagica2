...
public class EntityDarkMage extends EntityMob {
	...
	private static ItemStack diminishedHeldItem = new ItemStack(ItemDefs.affinityTome, 1,
			ArsMagicaAPI.getAffinityRegistry().getId(Affinity.NONE));
	private static ItemStack normalHeldItem = new ItemStack(ItemDefs.affinityTome, 1,
			ArsMagicaAPI.getAffinityRegistry().getId(Affinity.NONE));
	private static ItemStack augmentedHeldItem = new ItemStack(ItemDefs.affinityTome, 1,
			ArsMagicaAPI.getAffinityRegistry().getId(Affinity.NONE));

	public static final DataParameter<Integer> MAGE_SKIN = EntityDataManager.createKey(EntityDarkMage.class,
			DataSerializers.VARINT);
	public static final DataParameter<Integer> MAGE_BOOK = EntityDataManager.createKey(EntityDarkMage.class,
			DataSerializers.VARINT);
	...
	private void initAI() {
		...
		this.targetTasks.addTask(2, new EntityAINearestAttackableTarget<EntityLightMage>(this, EntityLightMage.class, 0,
				true, false, null));
		...
		this.targetTasks.addTask(2, new EntityAINearestAttackableTarget<EntityLivingBase>(this, EntityLivingBase.class,
				0, true, false, DarkMageEntitySelector.instance));
	...
	private int getAverageNearbyPlayerMagicLevel() {
		...
		List<EntityPlayer> players = worldObj.getEntitiesWithinAABB(EntityPlayer.class,
				new AxisAlignedBB(this.posX - 250, 0, this.posZ - 250, this.posX + 250, 250, this.posZ + 250));
		if (players.size() == 0)
			return 0;
		int avgLvl = 0;
	...
	public boolean getCanSpawnHere() {
		...
		if (avgLevel == 0) {
			if (rand.nextInt(100) < 10) {
				this.tasks.addTask(3, new EntityAIRangedAttackSpell(this, MovementSpeed(), 80,
						NPCSpells.instance.darkMage_NormalAttack));
				this.dataManager.set(MAGE_BOOK, 1);
			}
		} else {
			int levelRand = rand.nextInt(avgLevel * 2);
			if (levelRand > 60) {
				this.tasks.addTask(2, new EntityAIRangedAttackSpell(this, MovementSpeed(), 160,
	...
	public String getTexture() {
		return String.format("arsmagica2:textures/mobs/dark_mages/dark_mage_%d.png",
