...
public class EntityFlicker extends EntityAmbientCreature {
	...
	private static final DataParameter<Integer> WATCHER_FLICKERTYPE = EntityDataManager.createKey(EntityFlicker.class,
			DataSerializers.VARINT);
	private static final DataParameter<Boolean> WATCHER_AMBIENTFLICK = EntityDataManager.createKey(EntityFlicker.class,
	...
	public void onUpdate() {
		...
		if (this.ticksExisted > 100 && playerClose && !this.dataManager.get(WATCHER_AMBIENTFLICK)) {
			if (this.getActivePotionEffects().size() == 0
					|| (this.getActivePotionEffects().size() == 1 && worldObj.rand.nextDouble() < 0.1f))
		...
		if (worldObj.isRemote) {
			...
			if (getRNG().nextInt(10) < ArsMagica2.config.getGFXLevel()) {
				AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj,
						AMParticleDefs.getParticleForAffinity(getFlickerAffinity()), posX, posY, posZ);
				if (effect != null) {
	...
	protected void updateAITasks() {
		...
		if (targetPosition == null) // this represents the pause state in
									// between picking new waypoints
			return;
		...
		float speed = 0.2f;
		this.addVelocity(-normalizedMovementVector.x * speed, -normalizedMovementVector.y * speed,
				-normalizedMovementVector.z * speed);
	}
	...
	private void flick() {
		if (this.worldObj.isRemote) {
			for (int i = 0; i < 10 * ArsMagica2.config.getGFXLevel(); ++i) {
				AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "radiant", posX,
						posY, posZ);
				if (particle != null) {
					particle.AddParticleController(new ParticleMoveOnHeading(particle, worldObj.rand.nextDouble() * 360,
							worldObj.rand.nextDouble() * 360, worldObj.rand.nextDouble() * 0.3f + 0.01f, 1, false));
					particle.setRGBColorI(getFlickerAffinity().getColor());
					particle.AddParticleController(new ParticleFadeOut(particle, 1, false)
							.setFadeSpeed((float) (worldObj.rand.nextDouble() * 0.1 + 0.05))
	...
	private void pickNewTargetPosition() {
			...
			for (int y = 0; y <= 5; y++) {
				for (int z = 0; z <= 5; z++) {
					if (worldObj.getBlockState(getPosition().add(x, y, z)).getBlock() == BlockDefs.flickerLure) {
						...
						targetPosition = new AMVector3(pos.getX() + worldObj.rand.nextInt(5) - 2,
								groundLevel + worldObj.rand.nextInt(3), pos.getY() + worldObj.rand.nextInt(5) - 2);
						return;
					}
				}
			}
		...
		if (aff == Affinity.WATER) {
			for (int i = 0; i < 5; ++i) {
				targetPosition = new AMVector3(this.posX - 5 + worldObj.rand.nextInt(10),
						this.posY - 5 + worldObj.rand.nextInt(10), this.posZ - 5 + worldObj.rand.nextInt(10));
			...
			groundLevel = getTopBlockNearMe();
			targetPosition = new AMVector3(this.posX - 5 + worldObj.rand.nextInt(10),
					groundLevel + 10 + worldObj.rand.nextInt(15), this.posZ - 5 + worldObj.rand.nextInt(10));
		} else if (aff == Affinity.EARTH) {
			groundLevel = getTopBlockNearMe();
			targetPosition = new AMVector3(this.posX - 5 + worldObj.rand.nextInt(10),
					groundLevel + worldObj.rand.nextInt(1) + 1, this.posZ - 5 + worldObj.rand.nextInt(10));
		} else {
			groundLevel = getTopBlockNearMe();
			targetPosition = new AMVector3(this.posX - 5 + worldObj.rand.nextInt(10),
					groundLevel + 3 + worldObj.rand.nextInt(5), this.posZ - 5 + worldObj.rand.nextInt(10));
		}
	}
	...
	public boolean getCanSpawnHere() {
		if (ArsMagica2.proxy.getTotalFlickerCount() > 12 * worldObj.playerEntities.size()
				|| worldObj.rand.nextDouble() > 0.2f) {
			return false;
		}
		...
		if (biome != null) {
			...
			// populate the list
			// DO NOT USE THIS LIST FOR AIR/EARTH/LIFE - they are handled by
			// special cases.
			switch (tagType) {
			case DEAD:
			case END:
