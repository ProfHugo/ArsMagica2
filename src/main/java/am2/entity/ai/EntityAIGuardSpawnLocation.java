...
public class EntityAIGuardSpawnLocation extends EntityAIBase {
	...
	public void updateTask() {
		theGuard.getLookHelper().setLookPosition(spawnLocation.x, spawnLocation.y, spawnLocation.z, 10F,
				theGuard.getVerticalFaceSpeed());
		...
		for (int l = 0; l <= 4; l++) {
			for (int i1 = 0; i1 <= 4; i1++) {
				...
				if ((l < 1 || i1 < 1 || l > 3 || i1 > 3) && this.theWorld.isSideSolid(pos, EnumFacing.UP)
						&& !otherBlock.isBlockNormalCube()) {
					this.theGuard.setLocationAndAngles((double) ((float) (pos.getX() + l) + 0.5F), (double) pos.getY(),
							(double) ((float) (pos.getZ() + i1) + 0.5F), this.theGuard.rotationYaw,
