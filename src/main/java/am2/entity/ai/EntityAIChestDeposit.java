...
public class EntityAIChestDeposit extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if (InventoryUtilities.isInventoryEmpty(host.getBroomInventory()))
			return false;
		return !host.isInventoryEmpty() && host.isInventoryFull()
				|| EntityExtension.For(host).getInanimateTarget() == null;
	}
	...
	public void updateTask() {
		...
		if (new AMVector3(host).distanceSqTo(iLoc) > 9) {
			...
			if (depositCounter > 10) {
				...
				InventoryUtilities.deductFromInventory(host.getBroomInventory(), mergeStack,
						originalSize - mergeStack.stackSize);
			}

			if (depositCounter > 10 && (InventoryUtilities.isInventoryEmpty(host.getBroomInventory())
					|| !InventoryUtilities.canMergeHappen(host.getBroomInventory(), inventory))) {
