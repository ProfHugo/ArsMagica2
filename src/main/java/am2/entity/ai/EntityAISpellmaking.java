	...
	public void updateTask() {
		...
		if (searchItem.getItem() == ItemDefs.etherium) {
			...
			if (action_state == 0 && host.getNavigator().noPath()) {
				host.getNavigator().tryMoveToXYZ(altar.getPos().getX() + bc.getX(), altar.getPos().getY() + bc.getY(),
						altar.getPos().getZ() + bc.getZ(), MOVE_SPEED);
			} else if (action_state == 0 && hostLocation.distanceSqTo(new AMVector3(altar.getPos().getX() + bc.getX(),
					altar.getPos().getY() + bc.getY(), altar.getPos().getZ() + bc.getZ())) < DISTANCE_THRESHOLD) {
				action_state = 1;
				altar.flipSwitch();
				wait_counter = 0;
			...
			if (action_state == 0 && host.getNavigator().noPath()) { // no item
																		// and
																		// too
																		// far
																		// away
