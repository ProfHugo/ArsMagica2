...
public class EntityBroom extends EntityCreature {
	...
	public void onUpdate() {
		if (worldObj.isRemote) {
			updateRotations();
			if (isMoving()) {
				AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "smoke", posX, posY,
						posZ);
				if (particle != null) {
					particle.addRandomOffset(0.5, 0.5, 0.5);
					particle.setRGBColorF(0.8f, 0.6f, 0.4f);
					particle.AddParticleController(
							new ParticleFadeOut(particle, 1, false).setFadeSpeed(0.03f).setKillParticleOnFinish(true));
				}
			}
		}
		super.onUpdate();
	...
	private void updateRotations() {
		if (isMoving()) {
			...
			if (((int) (moveCounter)) % 6 == 0) {
				// TODO: worldObj.playSoundAtEntity(this,
				// .soundGrassFootstep.stepSoundName, 10.6f,
	...
	public NBTTagCompound writeToNBT(NBTTagCompound nbttagcompound) {
		...
		if (chestLocation != null)
			nbttagcompound.setIntArray("chestLoc",
					new int[] { (int) chestLocation.x, (int) chestLocation.y, (int) chestLocation.z });
	...
	public EnumActionResult applyPlayerInteraction(EntityPlayer player, Vec3d vec, ItemStack stack, EnumHand hand) {
		if (player.getHeldItemMainhand() != null
				&& player.getHeldItemMainhand().getItem() == ItemDefs.spellStaffMagitech) {
			if (this.worldObj.isRemote) {
				for (int i = 0; i < ArsMagica2.config.getGFXLevel() * 2; ++i) {
					AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "smoke", posX,
							posY, posZ);
					if (particle != null) {
