...
public class EntityThrownSickle extends EntityLiving {
	...
	private static final DataParameter<Integer> THROWING_ENTITY = EntityDataManager.createKey(EntityThrownSickle.class,
			DataSerializers.VARINT);
	private static final DataParameter<Integer> PROJECTILE_SPEED = EntityDataManager.createKey(EntityThrownSickle.class,
			DataSerializers.VARINT);
		...
		if (movingobjectposition.entityHit != null && movingobjectposition.entityHit instanceof EntityLivingBase) {
			...
			if (getThrowingEntity() != null
				...
				if (getThrowingEntity() instanceof EntityPlayer) {
					if (movingobjectposition.entityHit instanceof EntityPlayer && (getThrowingEntity().worldObj.isRemote
							|| !FMLCommonHandler.instance().getMinecraftServerInstance().isPVPEnabled()))
						return;
					movingobjectposition.entityHit
							.attackEntityFrom(DamageSources.causeCactusDamage(getThrowingEntity(), true), 10);
				} else {
					movingobjectposition.entityHit
							.attackEntityFrom(DamageSources.causeCactusDamage(getThrowingEntity(), true), 12);
				}
			}
		} else if (movingobjectposition.typeOfHit == RayTraceResult.Type.BLOCK && this.getThrowingEntity() != null
			...
			for (int i = -radius; i <= radius; ++i) {
				for (int j = -radius; j <= radius; ++j) {
					for (int k = -radius; k <= radius; ++k) {
						...
						if (nextBlock == null)
							continue;
						if (nextBlock.getBlock() instanceof BlockLeaves || nextBlock.getBlock() instanceof BlockBush
								|| nextBlock.getBlock() instanceof BlockCrops) {
							if (ForgeEventFactory.doPlayerHarvestCheck(
									DummyEntityPlayer.fromEntityLiving(getThrowingEntity()), nextBlock, true))
								if (!worldObj.isRemote)
