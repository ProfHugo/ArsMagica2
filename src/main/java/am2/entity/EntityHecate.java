...
public class EntityHecate extends EntityZombie {
	...
	private void initAI() {
		...
		this.targetTasks.addTask(2,
				new EntityAINearestAttackableTarget<EntityGolem>(this, EntityGolem.class, 0, false, false, null));
		this.targetTasks.addTask(3,
				new EntityAINearestAttackableTarget<EntityPlayer>(this, EntityPlayer.class, 0, true, false, null));
		this.targetTasks.addTask(3,
				new EntityAINearestAttackableTarget<EntityVillager>(this, EntityVillager.class, 0, false, false, null));
	}
	...
	public void onUpdate() {
		...
		if (this.worldObj != null) {
			...
			if (this.worldObj.getDifficulty() == EnumDifficulty.HARD && this.getAttackTarget() != null
	...
	public void onLivingUpdate() {
		if (this.worldObj.isDaytime() && !this.worldObj.isRemote && !this.isDead) {
			...
			if (f > 0.5F && this.rand.nextFloat() * 30.0F < (f - 0.4F) * 2.0F
					&& this.worldObj.canBlockSeeSky(getPosition())) {
	...
	private void spawnInvisibilityParticles() {
		/*
		 * for (int i = 0; i < 50; ++i){ ArsMagicaParticle effect =
		 * ParticleManager.spawn(this.worldObj, "hr_smoke", this.posX +
		 * rand.nextDouble(), this.posY + 1, this.posZ); if (effect != null){
		 * effect.setMaxAge(20); effect.setIgnoreMaxAge(false);
		 * effect.AddParticleController(new ParticleFleeEntity(effect, this,
		 * 0.1, 3, 1, false)); } }
		 */
		...
		if (rand.nextInt(3) == 0) {
			...
			if (effect != null) {
				if (this.currentForwardRotation < 0.24) {
					...
					effect.AddParticleController(new ParticleMoveOnHeading(effect, this.rotationYaw - 90,
							this.rotationPitch, 0.01f, 1, false));
				}
