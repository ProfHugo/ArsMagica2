...
public class EntityWinterGuardianArm extends EntityLiving {
	...
	private static final DataParameter<Integer> THROWING_ENTITY = EntityDataManager
			.createKey(EntityWinterGuardianArm.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> PROJECTILE_SPEED = EntityDataManager
			.createKey(EntityWinterGuardianArm.class, DataSerializers.VARINT);
		...
		if (getThrowingEntity() != null) {
			if (getThrowingEntity() instanceof EntityWinterGuardian) {
				...
				if (getThrowingEntity().getHealth() <= 0) {
					PlayerTracker.storeSoulboundItemForRespawn((EntityPlayer) getThrowingEntity(),
							ItemDefs.winterArmEnchanted.copy());
				} else {
					if (!((EntityPlayer) getThrowingEntity()).inventory
							.addItemStackToInventory(ItemDefs.winterArmEnchanted.copy())) {
	...
	public void onUpdate() {
		...
		if (isInWater()) {
			...
			for (int i = 0; i < 2; ++i) {
				AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "ember",
						posX + rand.nextFloat() * 0.2 - 0.1, posY + 1.2, posZ + rand.nextFloat() * 0.2 - 0.1);
				if (particle != null) {
					...
					particle.AddParticleController(new ParticleMoveOnHeading(particle,
							Math.toDegrees(this.rotationPitch), Math.toDegrees(this.rotationYaw), 0.2f, 1, false));
				}
			}
		}
		setPosition(posX, posY, posZ);
		...
		if (movingobjectposition.entityHit != null && movingobjectposition.entityHit instanceof EntityLivingBase) {
			...
			if (getThrowingEntity() != null && this.entityHit == null) {
				...
				if (movingobjectposition.entityHit instanceof EntityLivingBase) {
					EntityExtension.For((EntityLivingBase) movingobjectposition.entityHit).deductMana(
							EntityExtension.For((EntityLivingBase) movingobjectposition.entityHit).getMaxMana() * 0.1f);
					((EntityLivingBase) movingobjectposition.entityHit)
							.addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("weakness"), 60, 2));
					((EntityLivingBase) movingobjectposition.entityHit).addPotionEffect(
							new PotionEffect(Potion.getPotionFromResourceLocation("mining_fatigue"), 60, 2));
					((EntityLivingBase) movingobjectposition.entityHit)
