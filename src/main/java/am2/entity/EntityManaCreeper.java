...
public class EntityManaCreeper extends EntityCreeper {
	...
	public float getCreeperFlashIntensity(float par1) {
		return (this.lastActiveTime + (this.timeSinceIgnited_Local - this.lastActiveTime) * par1)
				/ (this.fuseLength - 2);
	}
	...
	public void onUpdate() {
		if (this.isEntityAlive()) {
			...
			if (var1 > 0 && this.timeSinceIgnited_Local == 0) {
				this.worldObj.playSound(posX, posY, posZ, SoundEvents.ENTITY_CREEPER_PRIMED, SoundCategory.HOSTILE,
						1.0F, 0.5F, false);
			}
