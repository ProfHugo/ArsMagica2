...
public class EntityFireElemental extends EntityMob {
	...
	private static final DataParameter<Integer> COOK_TARGET_ID = EntityDataManager.createKey(EntityFireElemental.class,
	...
	public void onUpdate() {
		...
		if (cookTargetID != 0) {
			List<EntityItem> items = worldObj.getEntitiesWithinAABB(EntityItem.class,
					this.getEntityBoundingBox().expand(cookRadius, cookRadius, cookRadius));
			...
			if (inanimate != null && worldObj.isRemote) {
				AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "fire", posX,
						posY + getEyeHeight(), posZ);
				if (effect != null) {
					effect.setIgnoreMaxAge(true);
					effect.AddParticleController(new ParticleApproachPoint(effect,
							inanimate.posX + (rand.nextFloat() - 0.5), inanimate.posY + (rand.nextFloat() - 0.5),
							inanimate.posZ + (rand.nextFloat() - 0.5), 0.1f, 0.1f, 1, false)
