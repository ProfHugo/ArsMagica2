...
public class EntityManaVortex extends Entity {
	...
	public static final DataParameter<Integer> TICKS_TO_EXIST = EntityDataManager.createKey(EntityManaVortex.class,
			DataSerializers.VARINT);
	public static final DataParameter<Float> MANA_STOLEN = EntityDataManager.createKey(EntityManaVortex.class,
	...
	public void onUpdate() {
		...
		if (getTicksToExist() - this.ticksExisted <= 5 && !hasGoneBoom) {
			...
			if (!worldObj.isRemote) {
				List<EntityLivingBase> players = worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
						this.getEntityBoundingBox().expand(3 + Math.floor(this.ticksExisted / 50), 2,
				...
				for (Object o : playerArray) {
					...
					RayTraceResult mop = this.worldObj.rayTraceBlocks(new Vec3d(this.posX, this.posY, this.posZ),
							new Vec3d(e.posX, e.posY + e.getEyeHeight(), e.posZ), false);
					if (mop == null)
				...
				for (int i = 0; i < 360; i += ArsMagica2.config.FullGFX() ? 5 : ArsMagica2.config.LowGFX() ? 10 : 20) {
					AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "ember",
					...
					if (effect != null) {
						...
						effect.AddParticleController(new ParticleLeaveParticleTrail(effect, "ember", false, 5, 1, false)
								...
								.addControllerToParticleList(new ParticleFadeOut(effect, 1, false).setFadeSpeed(0.1f)
										.setKillParticleOnFinish(true))
								.setParticleRGB_F(0.24f, 0.24f, 0.8f).addRandomOffset(0.2f, 0.2f, 0.2f));
					}
				}
			}
		}
		...
		if (getTicksToExist() - this.ticksExisted > 30) {
			...
			for (Object o : playerArray) {
				...
				if (worldObj.isRemote) {
					...
					AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "ember", e.posX,
							e.posY + (e.getEyeHeight() / 2), e.posZ);
					if (effect != null) {
						effect.setRGBColorF(0.24f, 0.24f, 0.8f);
						effect.AddParticleController(new ParticleArcToEntity(effect, 1, this, false)
								.generateControlPoints().setKillParticleOnFinish(true));
						effect.setIgnoreMaxAge(true);
					}
				}
