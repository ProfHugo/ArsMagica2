...
public class EntitySpellEffect extends Entity {
	...
	private static final DataParameter<Optional<ItemStack>> WATCHER_STACK = EntityDataManager
			.createKey(EntitySpellEffect.class, DataSerializers.OPTIONAL_ITEM_STACK);
	private static final DataParameter<Float> WATCHER_RADIUS = EntityDataManager.createKey(EntitySpellEffect.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Float> WATCHER_GRAVITY = EntityDataManager.createKey(EntitySpellEffect.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Integer> WATCHER_TYPE = EntityDataManager.createKey(EntitySpellEffect.class,
			DataSerializers.VARINT); // 0 == zone, 1 == rain of fire, 2 ==
										// blizzard, 3 == wall, 4 == wave
	private static final DataParameter<Boolean> WATCHER_ROF_IGNITE = EntityDataManager
			.createKey(EntitySpellEffect.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Float> WATCHER_DAMAGEBONUS = EntityDataManager.createKey(EntitySpellEffect.class,
			DataSerializers.FLOAT);
	...
	private void zoneUpdate() {
			...
			if (!ArsMagica2.config.NoGFX()) {
				...
				if ((ArsMagica2.config.FullGFX() && this.ticksExisted % 2 == 0) || this.ticksExisted % 8 == 0) {
					for (int i = 0; i < 4; ++i) {
						...
						AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, AMParticleDefs
								.getParticleForAffinity(AffinityShiftUtils.getMainShiftForStack(spellStack)), x, posY,
								z);
						if (effect != null) {
							...
							if (ArsMagica2.config.LowGFX()) {
								effect.AddParticleController(new ParticleOrbitPoint(effect, posX, posY, posZ, 2, false)
										.setIgnoreYCoordinate(true).SetOrbitSpeed(0.05f).SetTargetDistance(dist)
		...
		if (ticksToEffect <= 0) {
			...
			List<Entity> possibleTargets = worldObj.getEntitiesWithinAABB(Entity.class,
					new AxisAlignedBB(posX - radius, posY - 3, posZ - radius, posX + radius, posY + 3, posZ + radius));
			for (Entity e : possibleTargets) {
				if (e instanceof EntityDragonPart && ((EntityDragonPart) e).entityDragonObj instanceof EntityLivingBase)
					e = (EntityLivingBase) ((EntityDragonPart) e).entityDragonObj;

				if (e instanceof EntityLivingBase)
					// SpellUtils.applyStageToEntity(spellStack, dummycaster,
					// worldObj, e, false);
					SpellUtils.applyStackStage(spellStack.copy(), dummycaster, (EntityLivingBase) e, e.posX, e.posY - 1,
							e.posZ, null, worldObj, false, false, this.ticksExisted);
			}
			if (this.dataManager.get(WATCHER_GRAVITY) < 0 && !firstApply)
				SpellUtils.applyStackStage(spellStack.copy(), dummycaster, null, posX, posY - 1, posZ, null, worldObj,
						false, false, this.ticksExisted);
			else
				SpellUtils.applyStackStage(spellStack.copy(), dummycaster, null, posX, posY, posZ, null, worldObj,
						false, false, this.ticksExisted);
			firstApply = false;
			for (float i = -radius; i <= radius; i++) {
				for (int j = -3; j <= 3; j++) {
					Vec3d[] blocks = getAllBlockLocationsBetween(new Vec3d(posX + i, posY + j, posZ - radius),
							new Vec3d(posX + i, posY + j, posZ + radius));
					for (Vec3d vec : blocks) {
						SpellUtils.applyStageToGround(spellStack.copy(), dummycaster, worldObj, new BlockPos(vec),
								EnumFacing.UP, vec.xCoord + 0.5, vec.yCoord + 0.5, vec.zCoord + 0.5, false);
					}
				}
			}
		}
	...
	private void rainOfFireUpdate() {
		...
		if (worldObj.isRemote) {
			...
			for (int i = 0; i < 10; ++i) {
				...
				AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "explosion_2", x, y,
						z);
				if (particle != null) {
			...
			// TODO: SoundHelper.instance.loopSound(worldObj, (float)posX,
			// (float)posY, (float)posZ, "arsmagica2:spell.loop.fire", 1.0f);
		} else {
			List<Entity> possibleTargets = worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
					new AxisAlignedBB(posX - radius, posY - 1, posZ - radius, posX + radius, posY + 3, posZ + radius));
			for (Entity e : possibleTargets) {
				if (e != dummycaster) {
					...
					if (SpellUtils.attackTargetSpecial(null, e, DamageSources.causeFireDamage(dummycaster), damage)
	...
	private void blizzardUpdate() {
		...
		if (worldObj.isRemote) {
			...
			for (int i = 0; i < 20; ++i) {
				...
				AMParticle particle = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "snowflakes", x, y,
						z);
				if (particle != null) {
					particle.setMaxAge(20);
			...
			// TODO: SoundHelper.instance.loopSound(worldObj, (float)posX,
			// (float)posY, (float)posZ, "arsmagica2:spell.loop.air", 1.0f);
		} else {
			List<Entity> possibleTargets = worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
					new AxisAlignedBB(posX - radius, posY - 1, posZ - radius, posX + radius, posY + 3, posZ + radius));
			for (Entity e : possibleTargets) {
				if (e != dummycaster) {
					...
					double lastVelY = e.motionY;
					double lastVelZ = e.motionZ;
					if (SpellUtils.attackTargetSpecial(null, e, DamageSources.causeFrostDamage(dummycaster), damage)
							&& !(e instanceof EntityPlayer))
			...
			if (rand.nextInt(10) < 2) {
				...
				if (worldObj.isAirBlock(pos) && !worldObj.isAirBlock(pos.down())
	...
	private void wallUpdate() {
		if (worldObj.isRemote) {
			...
			for (float i = 0; i < dist; i += 0.5f) {
				...
				AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj,
						AMParticleDefs.getParticleForAffinity(AffinityShiftUtils.getMainShiftForStack(spellStack)), x,
						posY, z);
				if (effect != null) {
					effect.setIgnoreMaxAge(false);
					effect.setMaxAge(20);
				...
				effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj,
						AMParticleDefs.getParticleForAffinity(AffinityShiftUtils.getMainShiftForStack(spellStack)), x,
						posY, z);
				if (effect != null) {
					effect.setIgnoreMaxAge(false);
			...
			if (ticksToEffect <= 0) {
				...
				float radius = this.dataManager.get(WATCHER_RADIUS);
				List<Entity> possibleTargets = worldObj.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(
						posX - radius, posY - 1, posZ - radius, posX + radius, posY + 3, posZ + radius));
				for (Entity e : possibleTargets) {
					...
					if (e instanceof EntityLivingBase && hDistance < 0.75f && vDistance < 2) {
						// commented out in favor of line below so as to apply
						// subsequent shapes as well
						// uncomment and comment out below line to revert to
						// direct target only, but mark wave/wall as terminus
						// SpellUtils.applyStageToEntity(spellStack,
						// dummycaster, worldObj, e, false);
						SpellUtils.applyStackStage(spellStack.copy(), dummycaster, (EntityLivingBase) e, this.posX,
								this.posY, this.posZ, null, worldObj, false, false, 0);
					}
				}
			}
		}
	...
	private void waveUpdate() {
		...
		for (int j = -1; j <= 1; j++) {
			...
			for (Vec3d vec : vecs) {
				SpellUtils.applyStageToGround(SpellUtils.popStackStage(spellStack.copy()), dummycaster, worldObj,
						new BlockPos(vec), EnumFacing.UP, vec.xCoord + 0.5, vec.yCoord + 0.5, vec.zCoord + 0.5, false);
			}
		}
