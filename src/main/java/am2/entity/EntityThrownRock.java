...
public class EntityThrownRock extends EntityLiving {
	...
	private static final DataParameter<Boolean> IS_MOONSTONE_METEOR = EntityDataManager
			.createKey(EntityThrownRock.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> IS_SHOOTING_STAR = EntityDataManager.createKey(EntityThrownRock.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Optional<ItemStack>> SPELL_STACK = EntityDataManager
			.createKey(EntityThrownRock.class, DataSerializers.OPTIONAL_ITEM_STACK);
	...
	public void onUpdate() {
		...
		if (worldObj.isRemote) {
			if (getIsMoonstoneMeteor()) {
				AMParticle fire = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "explosion_2", posX,
						posY, posZ);
				if (fire != null) {
					fire.setMaxAge(20);
					...
					fire.AddParticleController(new ParticleColorShift(fire, 1, false).SetShiftSpeed(0.1f)
							.SetColorTarget(0.01f, 0.01f, 0.01f).SetEndOnReachingTargetColor()
				...
				for (float i = 0; i < Math.abs(motionY); i += 0.1f) {
					AMParticle star = (AMParticle) ArsMagica2.proxy.particleManager.spawn(worldObj, "ember",
							posX + motionX * i, posY + motionY * i, posZ + motionZ * i);
					if (star != null) {
						star.setMaxAge(22);
		...
		if (movingobjectposition != null) {
			vec3d1 = new Vec3d(movingobjectposition.hitVec.xCoord, movingobjectposition.hitVec.yCoord,
	...
	protected void HitObject(RayTraceResult movingobjectposition) {
		...
		if (getIsShootingStar()) {
			AMNetHandler.INSTANCE.sendStarImpactToClients(posX,
					posY + ((movingobjectposition.typeOfHit == RayTraceResult.Type.ENTITY)
							? -movingobjectposition.entityHit.getEyeHeight() : 1.5f),
					posZ, worldObj, this.getSpellStack());
			List<EntityLivingBase> ents = worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
					getEntityBoundingBox().expand(12, 5, 12));
			this.posY++;
			...
			if (movingobjectposition.entityHit != null && movingobjectposition.entityHit instanceof EntityLivingBase) {
				...
				if (this.getIsMoonstoneMeteor()) {
					...
					this.worldObj.newExplosion(this, this.target.xCoord, this.target.yCoord, this.target.zCoord, 0.8f,
							false, ArsMagica2.config.moonstoneMeteorsDestroyTerrain());
					...
					// if (((EntityPlayer)player).getDistanceSqToEntity(this) <
					// 4096)
	...
	private void generateSurfaceOreAtOffset(World world, BlockPos pos, boolean force) {
		...
		if (rand.nextInt(4) < 2 || force)
			world.setBlockState(pos,
					BlockDefs.ores.getDefaultState().withProperty(BlockArsMagicaOre.ORE_TYPE, EnumOreType.MOONSTONE));
		else
