...
public class MinHeap {
	...
	private void MinHeapify(int position) {
		do {
			...
			int left = ((position << 1) + 1); // same as doing (position*2 + 1),
												// but operationally
			...
			// get the minimum position between the right and the minimum from
			// the previous check
			if (right < count && array[right].compareTo(array[minPosition]) < 0) {
				minPosition = right;
			}

			// switch the position of the minimum and specified indices in the
			// array if the minimum position is not the one passed in.
