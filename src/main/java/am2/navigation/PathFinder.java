...
public final class PathFinder {
	...
	private static BreadCrumb FindPathReversed(World world, Point3D start, Point3D end, Entity entity)
			throws Exception {
		MinHeap openList = new MinHeap(256);

		// bounds checking - are the points within a valid distance?
		if (Math.abs(end.x) - Math.abs(start.x) > 19 || Math.abs(end.y) - Math.abs(start.y) > 19
				|| Math.abs(end.z) - Math.abs(start.z) > 19) {
			return null;
		}
		...
		// shift the coordinates so that they are based on a zero-index (this is
		// because we use the point coordinates as an index into the path array)
		if (start.x < end.x) {
		...
		// loop as long as we have have items in the open list (points we
		// haven't checked yet)
		while (openList.Count() > 0) {
			// Find best item and switch it to the 'closedList' - best being the
			// item currently on the open list with the lowest cost
			...
			for (int i = 0; i < surrounding.length; i++) {
				...
				if (BlockIsNavigateable(world, tmp, current.position, entity)) {
					// Check if we've already examined a neighbour, if not
					...
					// If the node is not on the 'closedList' check it's new
					// score, keep the best
	...
	private static boolean BlockIsNavigateable(World world, Point3D point, Point3D Current, Entity entity) {
		...
		if (!isDiagonalMovement(point, Current)) {
			...
			for (int i = 0; i < height; i += 4) {
				...
				Block block = world.getBlockState(new BlockPos(unshifted.x, unshifted.y + i, unshifted.z)).getBlock();
				blocks[i] = block;
				block = world.getBlockState(new BlockPos(unshifted.x + deltaX, unshifted.y + i, unshifted.z))
						.getBlock();
				blocks[i + 1] = block;
				block = world.getBlockState(new BlockPos(unshifted.x, unshifted.y + i, unshifted.z + deltaZ))
						.getBlock();
				blocks[i + 2] = block;
				block = world.getBlockState(new BlockPos(unshifted.x, unshifted.y + i + deltaY, unshifted.z))
						.getBlock();
				blocks[i + 3] = block;
			}
			for (Block b : blocks) {
	...
	private static boolean isDiagonalMovement(Point3D start, Point3D end) {
		...
		return (hasDeltaX && hasDeltaY) || (hasDeltaX && hasDeltaZ) || (hasDeltaZ && hasDeltaY)
				|| (hasDeltaX && hasDeltaY && hasDeltaZ);
	}

	private static Block[] validBlockIDs = new Block[] { Blocks.AIR, Blocks.LADDER, Blocks.DEADBUSH, Blocks.RED_FLOWER,
			Blocks.YELLOW_FLOWER, Blocks.RAIL, Blocks.STONE_BUTTON, Blocks.WOODEN_BUTTON, Blocks.LEVER,
			Blocks.DETECTOR_RAIL, Blocks.TORCH, Blocks.VINE };

	private static Point3D[] surrounding = new Point3D[] {
			// Top slice (Y=1)
			new Point3D(-1, 1, 1), new Point3D(0, 1, 1), new Point3D(1, 1, 1), new Point3D(-1, 1, 0),
			new Point3D(0, 1, 0), new Point3D(1, 1, 0), new Point3D(-1, 1, -1), new Point3D(0, 1, -1),
			new Point3D(1, 1, -1),
			// Middle slice (Y=0)
			new Point3D(-1, 0, 1), new Point3D(0, 0, 1), new Point3D(1, 0, 1), new Point3D(-1, 0, 0),
			new Point3D(1, 0, 0), // (0,0,0) is self
