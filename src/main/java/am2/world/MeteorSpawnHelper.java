...
public class MeteorSpawnHelper {
	...
	public void spawnMeteor() {
		ticksSinceLastMeteor = FMLCommonHandler.instance().getMinecraftServerInstance().getEntityWorld().rand
				.nextInt(36000) + 12000;
		if (FMLCommonHandler.instance().getMinecraftServerInstance().worldServers.length < 1)
			return;
		...
		if (time > 14500 && time < 21500) { // night time range (just past dusk
			...
			if (EntityExtension.For(player).getCurrentLevel() < ArsMagica2.config.getMeteorMinSpawnLevel())
				return;
			...
			for (int i = 0; i < 10; ++i) {
				BlockPos offsetCoord = spawnCoord
						.add(new AMVector3(rand.nextInt(meteorOffsetRadius) - (meteorOffsetRadius / 2), 0,
			...
			meteor.setPosition(spawnCoord.x + rand.nextInt(meteorOffsetRadius) - (meteorOffsetRadius / 2),
					ws.getActualHeight(), spawnCoord.z + rand.nextInt(meteorOffsetRadius) - (meteorOffsetRadius / 2));
