...
public class WitchwoodTreeHuge extends WorldGenAbstractTree {
	...
	boolean validTreeLocation() {
		...
		IBlockState soil = this.worldObj
				.getBlockState(new BlockPos(this.basePos[0], this.basePos[1] - 1, this.basePos[2]));

		boolean isValidSoil = (soil != null && soil.getBlock().canSustainPlant(soil, worldObj,
				new BlockPos(basePos[0], basePos[1] - 1, basePos[2]), EnumFacing.UP, (BlockSapling) Blocks.SAPLING));
		if (!isValidSoil) {
			return false;
		} else {
