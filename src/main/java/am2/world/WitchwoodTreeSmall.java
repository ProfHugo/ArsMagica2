...
public class WitchwoodTreeSmall extends WorldGenAbstractTree {
	...
	// variable names have been changed for readability, and some un-used parts
	// (vine, cocoa generation) have been removed
		...
		if (pos.getY() >= 1 && pos.getY() + height + 1 <= 256) {
			...
			for (int j = pos.getY(); j <= pos.getY() + 1 + height; j++) {
				...
				for (int i = pos.getX() - leafMaxRadius; i <= pos.getX() + leafMaxRadius && isValidPlantingSpot; i++) {
					for (int k = pos.getZ() - leafMaxRadius; k <= pos.getZ() + leafMaxRadius
							&& isValidPlantingSpot; k++) {
						if (j >= 0 && j < 256) {
			...
			if (!isValidPlantingSpot) {
				...
				boolean isSoil = block2.canSustainPlant(world.getBlockState(pos.down()), world, pos.down(),
				...
				if (isSoil && pos.getY() < 256 - height - 1) {
					...
					for (int j = pos.getY() - leafMaxRadius + height; j <= pos.getY() + height; j++) {
						...
						for (int i = pos.getX() - leafGenRadius; i <= pos.getX() + leafGenRadius; i++) {
							...
							for (int k = pos.getZ() - leafGenRadius; k <= pos.getZ() + leafGenRadius; k++) {
								...
								if (Math.abs(leafGenDeltaX) != leafGenRadius || Math.abs(leafGenDeltaZ) != leafGenRadius
										|| random.nextInt(2) != 0 && treeHeightLayer != 0) {
									BlockPos leafPos = new BlockPos(i, j, k);
									IBlockState block1 = world.getBlockState(leafPos);

									if (block1.getBlock().isAir(block1, world, leafPos)
											|| block1.getBlock().isLeaves(block1, world, leafPos)) {
					...
					for (int j = 0; j < height; j++) {
						...
						if (block.getBlock().isAir(block, world, pos.add(0, j, 0))
								|| block.getBlock().isLeaves(block, world, pos.add(0, j, 0))) {
