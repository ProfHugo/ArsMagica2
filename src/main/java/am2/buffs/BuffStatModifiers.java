...
public class BuffStatModifiers {
	...
	public void applyStatModifiersBasedOnBuffs(EntityLivingBase entity) {
		// entangled
		applyOrRemoveModifiersForBuff(entity, PotionEffectsDefs.entangle,
				ResourceUtils.createHashMap(SharedMonsterAttributes.MOVEMENT_SPEED, entangled));
		// frost slow
		applyOrRemoveScalingModifiersForBuff(entity, PotionEffectsDefs.frostSlow,
				SharedMonsterAttributes.MOVEMENT_SPEED, frostSlow_Diminished, frostSlow_Normal, frostSlow_Augmented);
		// fury
	...
	private void applyOrRemoveModifiersForBuff(EntityLivingBase entity, Potion buffID,
			Map<IAttribute, AttributeModifier> modifiers) {
	...
	private void applyOrRemoveScalingModifiersForBuff(EntityLivingBase entity, Potion potionID, IAttribute attribute,
	...
	// * Fury *//
	private static final AttributeModifier furyMoveMod = (new AttributeModifier(IDDefs.furyMoveID, "Fury (Movement)", 2,
			2));

	private static final AttributeModifier furyDmgMod = (new AttributeModifier(IDDefs.furyDmgID, "Fury (Damage)", 5,
			2));
	...
	// * Frost Slow *//
	private static final AttributeModifier frostSlow_Diminished = (new AttributeModifier(IDDefs.frostSlowID,
			"Frost Slow (Diminished)", -0.2, 2));
	private static final AttributeModifier frostSlow_Normal = (new AttributeModifier(IDDefs.frostSlowID,
			"Frost Slow (Normal)", -0.5, 2));
	private static final AttributeModifier frostSlow_Augmented = (new AttributeModifier(IDDefs.frostSlowID,
			"Frost Slow (Augmented)", -0.8, 2));
}
