...
public class AMPacketProcessorServer {
	...
	public void onServerPacketData(ServerCustomPacketEvent event) {
		...
		try {
			...
			// handleSetMagiciansWorkbenchRecipe(remaining,
	...
	private void handleAbilityToggle(byte[] remaining, EntityPlayerMP player) {
		...
		String text = String.format(I18n.translateToLocal("am2.chat.activation"),
				I18n.translateToLocal("am2.chat.ability_" + str),
				I18n.translateToLocal(newState ? "am2.chat.enabled" : "am2.chat.disabled"));
	...
	// ExtendedProperties.For(player).performRemoteOp(new AMDataReader(data,
	// false).getInt());
	// }
	//
		...
		if (nom == 1) {
			...
			if (te != null && te instanceof IPowerNode) {
				AMNetHandler.INSTANCE.sendPowerResponseToClient(
						PowerNodeRegistry.For(player.worldObj).getDataCompoundForNode((IPowerNode<?>) te), player, te);
			}
		}
	}
	...
	// ((TileEntityMagiciansWorkbench)te).setRecipeLocked(rdr.getInt(),
	// rdr.getBoolean());
	...
	//
	// //open the GUI
	// player.openGui(AMCore.instance, ArsMagicaGuiIdList.GUI_RUNE_BAG,
	// player.worldObj, (int)player.posX, (int)player.posY, (int)player.posZ);
	// }
	//
	// private void handleSetMagiciansWorkbenchRecipe(byte[] data,
	// EntityPlayerMP player){
	// if (player.openContainer != null && player.openContainer instanceof
	// ContainerMagiciansWorkbench){
	// ((ContainerMagiciansWorkbench)player.openContainer).moveRecipeToCraftingGrid(new
	// AMDataReader(data, false).getInt());
	// }
	// }
	//
	...
	// if (add)
	// ItemsCommonProxy.keystone.addCombination(((ContainerKeystone)player.openContainer).getKeystoneStack(),
	// name, metas);
	// else
	// ItemsCommonProxy.keystone.removeCombination(((ContainerKeystone)player.openContainer).getKeystoneStack(),
	// name);
	// }
	// }
	//
	private void handleInscriptionTableUpdate(byte[] data, EntityPlayerMP player) {
		...
		world.markAndNotifyBlock(te.getPos(), te.getWorld().getChunkFromBlockCoords(te.getPos()),
	...
	private void handleDecoBlockUpdate(byte[] data, EntityPlayerMP player) {
		...
		world.markAndNotifyBlock(te.getPos(), world.getChunkFromBlockCoords(te.getPos()),
				world.getBlockState(te.getPos()), world.getBlockState(te.getPos()), 2);
	}

	// private void handleSyncSpellKnowledge(byte[] data, EntityPlayerMP
	// player){
	...
	//
	// private void handleRequestBetaParticles(byte[] data, EntityPlayerMP
	// player){
	// AMDataReader rdr = new AMDataReader(data, false);
	...
	//
	// if (player == null || entity == null || !(entity instanceof
	// EntityPlayer)) return;
	//
	...
	//
	// AMNetHandler.INSTANCE.sendPacketToClientPlayer(player,
	// AMPacketIDs.REQUEST_BETA_PARTICLES, writer.generate());
	// }
	//
	...
	//
	// ExtendedProperties.For(player).updateAuraData(index, behaviour, scale,
	// alpha, randomColor, defaultColor, color, delay, quantity, speed);
	// }
	//
	...
	//
	// if (AMCore.proxy.IncreaseEntityMagicLevel((EntityLiving)ent,
	// ent.worldObj)){
	...
	// Packet pkt = createArsMagicaClientPacket(AMPacketIDs.MAGIC_LEVEL_UP,
	// writer.generate());
	...
	public EntityLivingBase getEntityByID(int entityID) {
		for (EntityLivingBase ent : FMLCommonHandler.instance().getMinecraftServerInstance().getEntityWorld()
				.<EntityLivingBase>getEntities(EntityLivingBase.class, EntitySelectors.NOT_SPECTATING)) {
