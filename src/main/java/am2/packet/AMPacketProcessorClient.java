...
public class AMPacketProcessorClient extends AMPacketProcessorServer {
	...
	public void onPacketData(ClientCustomPacketEvent event) {
		...
		try {
			...
			// constant details all packets share: ID, player, and remaining
			// data
	...
	private void handleManaLinkUpdate(byte[] remaining) {
		...
		((EntityExtension) EntityExtension
				.For((EntityLivingBase) Minecraft.getMinecraft().theWorld.getEntityByID(reader.getInt())))
						.handleManaLinkUpdate(reader);
	}
	...
	private void handleObeliskData(byte[] remaining) {
		...
		TileEntity te = Minecraft.getMinecraft().theWorld
				.getTileEntity(new BlockPos(rdr.getInt(), rdr.getInt(), rdr.getInt()));
	...
	// AMDataReader rdr = new AMDataReader(remaining, false);
	// TileEntity te =
	// Minecraft.getMinecraft().theWorld.getTileEntity(rdr.getInt(),
	// rdr.getInt(), rdr.getInt());
	...
	private void handleRcvPowerPaths(byte[] data) {
		...
		if (bite == 0) {
			...
			if (te != null && te instanceof IPowerNode)
				PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld).setDataCompoundForNode((IPowerNode<?>) te,
						compound);
		}
	}
	...
	private void handleHecateDeath(byte[] data) {
		...
		for (int i = 0; i < 10 * ArsMagica2.config.getGFXLevel(); ++i) {
			world.spawnParticle(EnumParticleTypes.FLAME, x, y + 1, z, world.rand.nextDouble() - 0.5,
	...
	//
	// Entity caster =
	// Minecraft.getMinecraft().theWorld.getEntityByID(casterID);
	// Entity target =
	// Minecraft.getMinecraft().theWorld.getEntityByID(targetID);
	//
	// if (caster == null || target == null || !(caster instanceof
	// EntityLivingBase) || !(target instanceof EntityLivingBase))
	// return;
	//
	// SpellHelper.instance.applyStackStage(spellStack,
	// (EntityLivingBase)caster, (EntityLivingBase)target, x, y, z, 0,
	// Minecraft.getMinecraft().theWorld, false, false, 0);
	// }
	//
	private void handleStarFall(byte[] data) {
		...
		for (int i = 0; i < 360; i += ArsMagica2.config.FullGFX() ? 5 : ArsMagica2.config.LowGFX() ? 10 : 20) {
			AMParticle effect = (AMParticle) ArsMagica2.proxy.particleManager.spawn(Minecraft.getMinecraft().theWorld,
					"sparkle2", x, y + 1.5, z);
			if (effect != null) {
				...
				effect.AddParticleController(
						new ParticleFadeOut(effect, 1, false).setFadeSpeed(0.05f).setKillParticleOnFinish(true));
				effect.AddParticleController(new ParticleLeaveParticleTrail(effect, "sparkle2", false, 15, 1, false)
						.addControllerToParticleList(new ParticleChangeSize(effect, 1.2f, 0.01f, 15, 1, false))
						.setParticleRGB_I(finalColor).setChildAffectedByGravity().addRandomOffset(0.2f, 0.2f, 0.2f));
			}
		}

		// Minecraft.getMinecraft().theWorld.playSound(x, y, z,
		// "arsmagica2:spell.special.starfall", 2.0f, 1.0f, false);
	}
	...
	// }catch (IOException e){
	// e.printStackTrace();
	// player.addChatMessage(new ChatComponentText("An error occurred while
	// attempting to write the NBT, check the console for more information."));
	// return;
	// }
	//
	// player.addChatMessage(new ChatComponentText("NBT Saved to " +
	...
	// EntityPlayer player = Minecraft.getMinecraft().thePlayer;
	//
	// FMLNetworkHandler.openGui(player, AMCore.instance,
	// ArsMagicaGuiIdList.GUI_KEYSTONE_CHEST, Minecraft.getMinecraft().theWorld,
	// x, y, z);
	// }
	//
	...
	//
	// PotionEffect pe = BuffList.buffEffectFromPotionID(id, duration,
	// amplifier);
	// if (pe != null){
	...
	//
	// AMNetHandler.INSTANCE.sendPacketToServer(AMPacketIDs.SYNC_BETA_PARTICLES,
	// writer.generate());
	// }
	//
	...
	// }else if (buffID == BuffList.flight.id || buffID ==
	// BuffList.levitation.id){
	...
	// //this is only really required for the local player, as other entities
	// seem to work across the network.
	...
	// ExtendedProperties.For(player).setMagicLevelWithMana(ExtendedProperties.For(player).getMagicLevel()
	// + 1);
	// }
	// }
	//
	...
	// }
	//
	// SpellHelper.instance.applyStackStage(stack, (EntityLivingBase)caster,
	// (EntityLivingBase)target, x, y, z, side, world, false, false, ticksUsed);
	// }
	//
